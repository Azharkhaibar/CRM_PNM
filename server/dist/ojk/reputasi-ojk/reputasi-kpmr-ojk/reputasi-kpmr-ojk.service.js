...
var KpmrReputasiService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KpmrReputasiService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const class_transformer_1 = require("class-transformer");
const reputasi_kpmr_dto_1 = require("./dto/reputasi-kpmr.dto");
...
let KpmrReputasiService = KpmrReputasiService_1 = class KpmrReputasiService {
    kpmrRepository;
    aspekRepository;
    pertanyaanRepository;
    dataSource;
    logger = new common_1.Logger(KpmrReputasiService_1.name);
    MAX_BOBOT_TOTAL = 100;
    ...
    async getAspekWithRelations(id) {
        const aspek = await this.aspekRepository.findOne({
            where: { id },
            relations: ['kpmrReputasi', 'pertanyaanList'],
        });
        if (!aspek) {
    ...
    async getPertanyaanWithRelations(id) {
        const pertanyaan = await this.pertanyaanRepository.findOne({
            where: { id },
            relations: ['aspek', 'aspek.kpmrReputasi'],
        });
        if (!pertanyaan) {
    ...
    async validateTotalBobot(kpmrId, newBobot, excludeAspekId) {
        const query = this.aspekRepository
            ...
            .where('aspek.kpmrReputasiId = :kpmrId', { kpmrId });
        if (excludeAspekId) {
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(reputasi_kpmr_aspek_entity_1.KpmrAspekReputasi, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(reputasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanReputasi, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(reputasi_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    async createKpmr(createDto, createdBy = 'system') {
        ...
        const kpmr = this.kpmrRepository.create({
            ...
            notes: createDto.notes ||
                `KPMR Reputasi Produk ${createDto.year} Q${createDto.quarter}`,
            createdBy: createdBy,
    ...
    async createAspek(kpmrId, createDto) {
        try {
            ...
            const kpmrReputasi = await this.kpmrRepository.findOne({
                where: { id: kpmrId },
            });
            if (!kpmrReputasi) {
                throw new common_1.NotFoundException(`KPMR with ID ${kpmrId} not found`);
            }
            this.checkKpmrLocked(kpmrReputasi, 'menambah aspek');
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        this.checkKpmrLocked(aspek.kpmrReputasi, 'mengupdate aspek');
        if (updateDto.bobot !== undefined) {
            this.validateBobot(updateDto.bobot);
            await this.validateTotalBobot(aspek.kpmrReputasiId, updateDto.bobot, id);
        }
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(reputasi_kpmr_dto_1.FrontendAspekResponseDto, {
    ...
    async deleteAspek(id) {
        ...
        const aspek = await this.getAspekWithRelations(id);
        this.checkKpmrLocked(aspek.kpmrReputasi, 'menghapus aspek');
        try {
            await this.aspekRepository.remove(aspek);
            await this.reorderRemainingAspek(aspek.kpmrReputasiId);
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(reputasi_kpmr_aspek_entity_1.KpmrAspekReputasi, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async createPertanyaan(aspekId, createDto) {
        try {
            ...
            const aspek = await this.aspekRepository.findOne({
                where: { id: aspekId },
                relations: ['kpmrReputasi'],
            });
            if (!aspek) {
                throw new common_1.NotFoundException(`Aspek with ID ${aspekId} not found`);
            }
            this.checkKpmrLocked(aspek.kpmrReputasi, 'menambah pertanyaan');
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        this.checkKpmrLocked(pertanyaan.aspek.kpmrReputasi, 'mengupdate pertanyaan');
        if (updateDto.skor) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(reputasi_kpmr_dto_1.FrontendPertanyaanResponseDto, {
    ...
    async updateSkor(id, updateSkorDto) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrReputasi, 'mengupdate skor');
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(reputasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanReputasi)
                    .findOne({
                ...
                if (pertanyaan.aspek.kpmrReputasi.isLocked) {
    ...
    async deletePertanyaan(id) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrReputasi, 'menghapus pertanyaan');
        try {
    ...
    async reorderPertanyaan(aspekId, reorderDto) {
        ...
        this.checkKpmrLocked(aspek.kpmrReputasi, 'mengubah urutan pertanyaan');
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(reputasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanReputasi, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(reputasi_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrReputasiService = KpmrReputasiService;
exports.KpmrReputasiService = KpmrReputasiService = KpmrReputasiService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(reputasi_kpmr_ojk_entity_1.KpmrReputasiOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(reputasi_kpmr_aspek_entity_1.KpmrAspekReputasi)),
    __param(2, (0, typeorm_1.InjectRepository)(reputasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanReputasi)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KpmrReputasiService);
//# sourceMappingURL=reputasi-kpmr-ojk.service.js.map