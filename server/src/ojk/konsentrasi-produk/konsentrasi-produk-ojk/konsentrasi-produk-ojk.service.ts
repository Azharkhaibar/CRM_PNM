...
import {
  ...
  UpdateKonsentrasiProdukInherentDto,
  CreateParameterDto,
  UpdateParameterDto,
  CreateNilaiDto,
  UpdateNilaiDto,
...
@Injectable()
export class KonsentrasiProdukOjkService {
  private readonly logger = new Logger(KonsentrasiProdukOjkService.name);

  constructor(
    @InjectRepository(KonsentrasiProdukOjk)
    private inherentRepository: Repository<KonsentrasiProdukOjk>,
    ...
    @InjectRepository(InherentReferenceKonsentrasi)
    private referenceRepository: Repository<InherentReferenceKonsentrasi>,
    private dataSource: DataSource,
  ) {}

  // === CRUD UTAMA (KonsentrasiProdukOjk) ===

  async create(createDto: CreateKonsentrasiProdukInherentDto, userId: string) {
    try {
      ...
      this.logger.error(
        `Error creating KonsentrasiProdukOjk: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findActive(): Promise<KonsentrasiProdukOjk | null> {
  ...
  async findByYearQuarter(
    year: number,
    quarter: number,
  ): Promise<KonsentrasiProdukOjk | null> {
    this.logger.log(
  ...
  async update(
    id: number,
    updateDto: UpdateKonsentrasiProdukInherentDto,
    userId: string,
  ) {
  ...
  async remove(id: number) {
    ...
    try {
      ...
      // Hapus inherent
      await queryRunner.manager.delete(KonsentrasiProdukOjk, { id });
    ...
    try {
      for (let i = 0; i < reorderDto.parameterIds.length; i++) {
        ...
        await queryRunner.manager.update(
          ...
          { id: parameterId, konsentrasiProdukOjkId: inherentId },
          { orderIndex: i },
        );
      }
    ...
    try {
      ...
      if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
        const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
          ...
          return queryRunner.manager.save(KonsentrasiNilai, newNilai);
        });
      ...
      for (let i = 0; i < importData.parameters.length; i++) {
        ...
        if (paramData.nilaiList && Array.isArray(paramData.nilaiList)) {
          for (let j = 0; j < paramData.nilaiList.length; j++) {
            ...
            await queryRunner.manager.save(KonsentrasiNilai, nilai);
          }
        }
      }
