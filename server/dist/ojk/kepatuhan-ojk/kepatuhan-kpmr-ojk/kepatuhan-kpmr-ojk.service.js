...
var KpmrKepatuhanService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KpmrKepatuhanService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const class_transformer_1 = require("class-transformer");
const kepatuhan_kpmr_dto_1 = require("./dto/kepatuhan-kpmr.dto");
...
let KpmrKepatuhanService = KpmrKepatuhanService_1 = class KpmrKepatuhanService {
    kpmrRepository;
    aspekRepository;
    pertanyaanRepository;
    dataSource;
    logger = new common_1.Logger(KpmrKepatuhanService_1.name);
    MAX_BOBOT_TOTAL = 100;
    ...
    async getAspekWithRelations(id) {
        const aspek = await this.aspekRepository.findOne({
            where: { id },
            relations: ['kpmrKepatuhan', 'pertanyaanList'],
        });
        if (!aspek) {
    ...
    async getPertanyaanWithRelations(id) {
        const pertanyaan = await this.pertanyaanRepository.findOne({
            where: { id },
            relations: ['aspek', 'aspek.kpmrKepatuhan'],
        });
        if (!pertanyaan) {
    ...
    async validateTotalBobot(kpmrId, newBobot, excludeAspekId) {
        const query = this.aspekRepository
            ...
            .where('aspek.kpmrKepatuhanId = :kpmrId', { kpmrId });
        if (excludeAspekId) {
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(kepatuhan_kpmr_aspek_entity_1.KpmrAspekKepatuhan, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(kepatuhan_kpmr_pertanyaan_entity_1.KpmrPertanyaanKepatuhan, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(kepatuhan_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    async createKpmr(createDto, createdBy = 'system') {
        ...
        const kpmr = this.kpmrRepository.create({
            ...
            notes: createDto.notes ||
                `KPMR Kepatuhan Produk ${createDto.year} Q${createDto.quarter}`,
            createdBy: createdBy,
    ...
    async createAspek(kpmrId, createDto) {
        try {
            ...
            const kpmrKepatuhan = await this.kpmrRepository.findOne({
                where: { id: kpmrId },
            });
            if (!kpmrKepatuhan) {
                throw new common_1.NotFoundException(`KPMR with ID ${kpmrId} not found`);
            }
            this.checkKpmrLocked(kpmrKepatuhan, 'menambah aspek');
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        this.checkKpmrLocked(aspek.kpmrKepatuhan, 'mengupdate aspek');
        if (updateDto.bobot !== undefined) {
            this.validateBobot(updateDto.bobot);
            await this.validateTotalBobot(aspek.kpmrKepatuhanId, updateDto.bobot, id);
        }
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(kepatuhan_kpmr_dto_1.FrontendAspekResponseDto, {
    ...
    async deleteAspek(id) {
        ...
        const aspek = await this.getAspekWithRelations(id);
        this.checkKpmrLocked(aspek.kpmrKepatuhan, 'menghapus aspek');
        try {
            await this.aspekRepository.remove(aspek);
            await this.reorderRemainingAspek(aspek.kpmrKepatuhanId);
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(kepatuhan_kpmr_aspek_entity_1.KpmrAspekKepatuhan, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async createPertanyaan(aspekId, createDto) {
        try {
            ...
            const aspek = await this.aspekRepository.findOne({
                where: { id: aspekId },
                relations: ['kpmrKepatuhan'],
            });
            if (!aspek) {
                throw new common_1.NotFoundException(`Aspek with ID ${aspekId} not found`);
            }
            this.checkKpmrLocked(aspek.kpmrKepatuhan, 'menambah pertanyaan');
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKepatuhan, 'mengupdate pertanyaan');
        if (updateDto.skor) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(kepatuhan_kpmr_dto_1.FrontendPertanyaanResponseDto, {
    ...
    async updateSkor(id, updateSkorDto) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKepatuhan, 'mengupdate skor');
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(kepatuhan_kpmr_pertanyaan_entity_1.KpmrPertanyaanKepatuhan)
                    .findOne({
                ...
                if (pertanyaan.aspek.kpmrKepatuhan.isLocked) {
    ...
    async deletePertanyaan(id) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKepatuhan, 'menghapus pertanyaan');
        try {
    ...
    async reorderPertanyaan(aspekId, reorderDto) {
        ...
        this.checkKpmrLocked(aspek.kpmrKepatuhan, 'mengubah urutan pertanyaan');
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(kepatuhan_kpmr_pertanyaan_entity_1.KpmrPertanyaanKepatuhan, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(kepatuhan_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrKepatuhanService = KpmrKepatuhanService;
exports.KpmrKepatuhanService = KpmrKepatuhanService = KpmrKepatuhanService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(kepatuhan_kpmr_ojk_entity_1.KpmrKepatuhanOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(kepatuhan_kpmr_aspek_entity_1.KpmrAspekKepatuhan)),
    __param(2, (0, typeorm_1.InjectRepository)(kepatuhan_kpmr_pertanyaan_entity_1.KpmrPertanyaanKepatuhan)),
