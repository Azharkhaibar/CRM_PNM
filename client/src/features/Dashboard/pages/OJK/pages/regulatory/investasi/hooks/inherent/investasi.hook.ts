// src/ojk/investasi-produk/hooks/inherent/investasi.hook.ts
import { useState, useCallback, useRef, useEffect } from 'react';
import { useToast } from '../../components/use-toast';
import investasiService from '../../services/inherent/investasi.service';
import { InvestasiOjkEntity, ParameterEntity, NilaiEntity, CreateParameterDto, UpdateParameterDto, CreateNilaiDto, UpdateNilaiDto } from '../../services/inherent/investasi.service';

interface UseInvestasiInherentReturn {
  // State
  investasi: InvestasiOjkEntity | null;
  parameters: ParameterEntity[];
  loading: boolean;
  saving: boolean;
  error: string | null;
  currentInvestasiId: number | null;

  // Data operations
  loadByYearQuarter: (year: number, quarter: number) => Promise<void>;
  refreshData: () => Promise<ParameterEntity[]>;
  createInvestasi: (year: number, quarter: number) => Promise<InvestasiOjkEntity | null>;

  // Parameter operations
  addParameter: (investasiId: number, parameterData: CreateParameterDto) => Promise<ParameterEntity>;
  ...
  copyParameter: (investasiId: number, parameterId: number) => Promise<void>;
  reorderParameters: (investasiId: number, parameterIds: number[]) => Promise<void>;

  // Nilai operations
  addNilai: (investasiId: number, parameterId: number, nilaiData: CreateNilaiDto) => Promise<NilaiEntity>;
  updateNilai: (nilaiId: number, nilaiData: UpdateNilaiDto) => Promise<NilaiEntity>;
  deleteNilai: (nilaiId: number) => Promise<void>;
  copyNilai: (investasiId: number, parameterId: number, nilaiId: number) => Promise<void>;
  reorderNilai: (investasiId: number, parameterId: number, nilaiIds: number[]) => Promise<void>;

  // Utility
  getParametersByInvestasiId: (investasiId: number) => ParameterEntity[];
...
export function useInvestasiInherent(): UseInvestasiInherentReturn {
  const { toast } = useToast();

  const [investasi, setInvestasi] = useState<InvestasiOjkEntity | null>(null);
  const [parameters, setParameters] = useState<ParameterEntity[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [currentInvestasiId, setCurrentInvestasiId] = useState<number | null>(null);
  ...
  const loadByYearQuarter = useCallback(
    async (targetYear: number, targetQuarter: number) => {
      if (lastLoadedYearRef.current?.year === targetYear && lastLoadedYearRef.current?.quarter === targetQuarter && investasi !== null) {
      ...
      try {
        ...
        const data = await investasiService.loadOrCreate(targetYear, targetQuarter);

        if (!mountedRef.current) return;

        safeSet(setInvestasi, data);
        safeSet(setCurrentInvestasiId, data.id);
  ...
  // ========== CREATE INVESTASI ==========
  const createInvestasi = useCallback(
    async (year: number, quarter: number): Promise<InvestasiOjkEntity | null> => {
      if (loadingRef.current) {
      ...
      try {
        ...
        const data = await investasiService.create(payload);
  ...
  const refreshData = useCallback(async (): Promise<ParameterEntity[]> => {
    ...
    try {
      const freshData = await investasiService.getById(investasi.id);

      if (!mountedRef.current) return [];

      safeSet(setInvestasi, freshData);
  ...
  }, [investasi?.id, parameters, toast]);

  // ========== PARAMETER OPERATIONS ==========
  const addParameter = useCallback(
    async (investasiId: number, parameterData: CreateParameterDto): Promise<ParameterEntity> => {
      ...
      try {
        if (!investasiId) throw new Error('ID Investasi tidak boleh kosong');
        ...
        const updatedInvestasi = await investasiService.addParameter(investasiId, payload);

        if (!mountedRef.current) throw new Error('Component unmounted');

        safeSet(setInvestasi, updatedInvestasi);

        const params = Array.isArray(updatedInvestasi.parameters) ? updatedInvestasi.parameters : [];
        safeSet(setParameters, params);
  ...
  const updateParameter = useCallback(
    async (parameterId: number, parameterData: UpdateParameterDto): Promise<ParameterEntity> => {
      if (saving) throw new Error('Already saving');
      if (!investasi?.id) throw new Error('Tidak ada data investasi yang dipilih');
      ...
      try {
        ...
        const updatedInvestasi = await investasiService.updateParameter(investasi.id, parameterId, parameterData);
  ...
  const deleteParameter = useCallback(
    async (parameterId: number): Promise<void> => {
      ...
      try {
        const updatedInvestasi = await investasiService.removeParameter(investasi.id, parameterId);
  ...
  const copyParameter = useCallback(
    async (investasiId: number, parameterId: number): Promise<void> => {
      if (saving) throw new Error('Already saving');
      ...
      try {
        const updatedInvestasi = await investasiService.copyParameter(investasiId, parameterId);
  ...
  const reorderParameters = useCallback(
    async (investasiId: number, parameterIds: number[]): Promise<void> => {
      if (saving) throw new Error('Already saving');
      ...
      try {
        await investasiService.reorderParameters(investasiId, parameterIds);
  ...
  const addNilai = useCallback(
    async (investasiId: number, parameterId: number, nilaiData: CreateNilaiDto): Promise<NilaiEntity> => {
      if (saving) throw new Error('Already saving');
      ...
      try {
        ...
        const updatedInvestasi = await investasiService.addNilai(investasiId, parameterId, payload);
  ...
  const updateNilai = useCallback(
    async (nilaiId: number, nilaiData: UpdateNilaiDto): Promise<NilaiEntity> => {
      ...
      try {
        ...
        const updatedInvestasi = await investasiService.updateNilai(investasi.id, targetParameterId, nilaiId, nilaiData);
    ...
    [saving, investasi?.id, parameters, toast],
  );
  ...
  const deleteNilai = useCallback(
    async (nilaiId: number): Promise<void> => {
      ...
      try {
        ...
        const updatedInvestasi = await investasiService.removeNilai(investasi.id, targetParameterId, nilaiId);
  ...
  const copyNilai = useCallback(
    async (investasiId: number, parameterId: number, nilaiId: number): Promise<void> => {
      if (saving) throw new Error('Already saving');
      ...
      try {
        const updatedInvestasi = await investasiService.copyNilai(investasiId, parameterId, nilaiId);
  ...
  const reorderNilai = useCallback(
    async (investasiId: number, parameterId: number, nilaiIds: number[]): Promise<void> => {
      if (saving) throw new Error('Already saving');
      ...
      try {
        await investasiService.reorderNilai(investasiId, parameterId, nilaiIds);
  ...
  // ========== UTILITY ==========
  const getParametersByInvestasiId = useCallback(
    (investasiId: number): ParameterEntity[] => {
      if (investasi?.id === investasiId) {
        return parameters;
      }
      return [];
    },
    [investasi?.id, parameters],
  );
  ...
  return {
    ...
    getParametersByInvestasiId,
    getNilaiByParameterId,

    hasData: parameters.length > 0 && investasi !== null,
    isReady: !loading && !saving && investasi !== null,
    hasError: error !== null,
  };
}

export default useInvestasiInherent;
