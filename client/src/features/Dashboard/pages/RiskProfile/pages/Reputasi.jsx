...
const REPUTASI_SECTIONS_LS_V1 = "reputasi_sections_v1"; // Backup key for migration
const KPMR_REPUTASI_LS = "kpmr_reputasi_rows_v1";
const KPMR_ASPEK_LS = "kpmr_reputasi_aspek_v1";
const KPMR_PERTANYAAN_LS = "kpmr_reputasi_pertanyaan_v1";
const KPMR_TEMPLATE_LS = "kpmr_reputasi_template_v1";

// Year-Level Definitions: Shared across ALL quarters within the same year
const KPMR_DEFINITIONS_LS = "kpmr_reputasi_definitions_v1";

// Quarter-Level Scores: sectionSkor per quarter, linked by definitionId
const KPMR_SCORES_LS = "kpmr_reputasi_scores_v1";
...
const KPMR_EMPTY_FORM = {
  ...
  sectionNo: "1",
  sectionTitle:
    "Bagaimana perumusan tingkat risiko yang akan diambil (risk appetite) dan toleransi risiko (risk tolerance) terkait risiko reputasi?",
  sectionSkor: "",
...
// helper: pembulatan ke 2 desimal yang stabil
const round2 = (v) => {
  const n = Number(v) || 0;
...
const loadReputasiRows = () => {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem("reputasiRows");
    if (!raw) return [];
    ...
    console.warn("Gagal parse reputasiRows dari localStorage", e);
    return [];
  }
};
...
const makeRowKey = (r) => {
  const key = `${r.source || "REPUTASI"}|${r.year}|${r.quarter}|${r.no ?? ""}|${r.subNo ?? ""}|${r.sectionLabel ?? ""}|${r.indikator ?? ""}`;
...
export default function Reputasi() {
  ...
  // ----- SECTION LIST -----
  const [REPUTASI_sections, setREPUTASI_sections] = useState(() => {
    if (typeof window === "undefined") return [];
    try {
      // Try loading v2 format first
      const rawV2 = localStorage.getItem(REPUTASI_SECTIONS_LS);
      if (rawV2) {
      ...
      const rawV1 = localStorage.getItem(REPUTASI_SECTIONS_LS_V1);
      if (rawV1) {
        ...
        if (Array.isArray(v1Sections) && v1Sections.length > 0) {
          ...
          const migrated = migrateSectionsV1ToV2(v1Sections, currentYear, currentQuarter);

          // Save migrated data to v2 format
          localStorage.setItem(REPUTASI_SECTIONS_LS, JSON.stringify(migrated));
      ...
      console.warn('[REPUTASI] Failed to load sections:', e);
      return [];
    }
  });
  ...
  // State untuk menampilkan info inheritance (ketika data baru di-clone)
  ...
  // Get sections untuk periode saat ini (dengan inheritance)
  const visibleSections = useMemo(() => {
    return getSectionsForPeriod(viewYear, viewQuarter, REPUTASI_sections);
  }, [viewYear, viewQuarter, REPUTASI_sections]);
  ...
  function REPUTASI_addSection() {
    ...
    setREPUTASI_sections((prev) => {
      const newSections = addSectionToPeriod(viewYear, viewQuarter, sectionData, prev);
      localStorage.setItem(REPUTASI_SECTIONS_LS, JSON.stringify(newSections));
  ...
  function REPUTASI_saveSection() {
    if (!REPUTASI_sectionForm.id) return;

    setREPUTASI_sections((prev) => {
      const updated = updateSectionInPeriod(
        REPUTASI_sectionForm.id,
        viewYear,
        viewQuarter,
        {
          no: REPUTASI_sectionForm.no,
      ...
      try {
        localStorage.setItem(REPUTASI_SECTIONS_LS, JSON.stringify(updated));
      } catch (e) {
        console.warn('[REPUTASI_saveSection] Gagal simpan sections ke localStorage', e);
      }
  ...
  function REPUTASI_removeSection(id) {
    const sectionToDelete = visibleSections.find((s) => s.id === id);
    ...
    // Remove the section
    setREPUTASI_sections((prev) => {
      const updated = deleteSectionFromPeriod(id, viewYear, viewQuarter, prev);

      try {
        localStorage.setItem(REPUTASI_SECTIONS_LS, JSON.stringify(updated));
      } catch (e) {
        console.warn('[REPUTASI_removeSection] Gagal simpan sections ke localStorage', e);
      }
  ...
  // ===== PERSIST SECTION KE localStorage (backup/fallback) =====
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      // Simpan semua sections tanpa filter (consistency dengan auto-clone)
      // Catatan: di sistem baru, autoCloneSectionsIfNeeded membuat DIRECT sections (isVirtual: false)
      localStorage.setItem(
        REPUTASI_SECTIONS_LS,
        JSON.stringify(REPUTASI_sections)
      );
    } catch (e) {
      console.warn("Gagal simpan reputasi sections", e);
    }
  }, [REPUTASI_sections]);

  // ===== AUTO-CLONE SECTIONS SAAT GANTI PERIODE =====
  useEffect(() => {
    if (typeof window === "undefined") return;

    const updatedSections = autoCloneSectionsIfNeeded(viewYear, viewQuarter, REPUTASI_sections);

    // Hanya update jika ada perubahan (clone terjadi)
    if (updatedSections.length !== REPUTASI_sections.length) {
      setREPUTASI_sections(updatedSections);
      localStorage.setItem(REPUTASI_SECTIONS_LS, JSON.stringify(updatedSections));
  ...
  // 1) semua baris indikator reputasi (multi tahun & quarter)
  const [REPUTASI_rows, setREPUTASI_rows] = useState(loadReputasiRows);

  const latestRowsStrRef = useRef("");
  useEffect(() => {
    try {
      latestRowsStrRef.current = JSON.stringify(REPUTASI_rows);
    } catch {
  ...
  // ============================================================
  // DATA REPAIR: Fix weighted values
  // ============================================================
  const repairWeightedData = () => {

    setREPUTASI_rows(prev => {
      const repaired = prev.map(row => {
        // Pastikan semua field bertipe number
        const bobotSection = Number(row.bobotSection || 0);
        const bobotIndikator = Number(row.bobotIndikator || 0);
        const peringkat = Number(row.peringkat || 0);

        const recalculated = (bobotSection * bobotIndikator * peringkat) / 10000;

                return {
          ...row,
          bobotIndikator, // Pastikan number
          weighted: recalculated
        };
      });

      localStorage.setItem("reputasiRows", JSON.stringify(repaired));

      return repaired;
    });
  };

  // Jalankan repair saat load data pertama kali
  useEffect(() => {
    const hasData = REPUTASI_rows.length > 0;
    if (hasData) {
      const needsRepair = REPUTASI_rows.some(r =>
        typeof r.bobotIndikator === 'string' ||
        Math.abs(r.weighted - ((r.bobotSection * r.bobotIndikator * r.peringkat) / 10000)) > 0.0001
      );
  ...
  const [REPUTASI_form, setREPUTASI_form] = useState(invMakeRow());
  ...
  // ============================================================
  // AUTO-CLONE ROWS FROM PREVIOUS PERIOD
  // ============================================================
  useEffect(() => {
    const result = cloneFromPreviousPeriod({
      ...
      sourceLabel: "REPUTASI",
    });

    if (result.cloned) {
      setREPUTASI_rows(result.rows);
      setInheritInfo({
        from: result.from,
        count: result.count,
      });
  ...
  // ============================================================
  useEffect(() => {
    if (!REPUTASI_rows?.length || isProcessingWeighted.current) return;
    
    let hasChanges = false;
    
    const updatedRows = REPUTASI_rows.map(row => {
    ...
    if (hasChanges) {
      ...
      setREPUTASI_rows(updatedRows);
      // Reset flag setelah state update
      setTimeout(() => {
        isProcessingWeighted.current = false;
      }, 0);
    }
  }, [REPUTASI_rows.map(r => `${r.no}-${r.subNo}-${r.peringkat}-${r.bobotIndikator}`).join(',')]);

  // 
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const rowsToSave = REPUTASI_rows.map(r => ({
        ...r,
        weighted: r.weighted
      }));
      localStorage.setItem("reputasiRows", JSON.stringify(rowsToSave));

      // Notify RekapData/Rekap1 without re-triggering Reputasi sync
      window.dispatchEvent(
        new CustomEvent("reputasiRows:changed", { detail: { origin: "REPUTASI" } })
      );
    } catch (e) {
      console.warn("Gagal simpan reputasiRows ke localStorage", e);
    }
  }, [REPUTASI_rows]);

  // --- bentuk data ringkas untuk RekapData & simpan ke localStorage ---
  // ============================================================
  // SIMPAN DATA RINGKAS UNTUK REKAP1.JSX (REPUTASI)
  // ============================================================
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Map ALL data from REPUTASI_rows, not just filtered by viewYear/viewQuarter
    // This ensures we save data for ALL periods, not just the current view
    const rekapRows = REPUTASI_rows.map((r, idx) => ({
      // id stabil
      id: makeRowKey(r),
    ...
    try {
      // Save all data
      localStorage.setItem("rekap_reputasi", JSON.stringify(uniqueRows));
    } catch (e) {
      console.warn("Gagal simpan rekap_reputasi", e);
    }
  }, [REPUTASI_rows]);
  ...
  useEffect(() => {
    ...
    // Check if rekap_reputasi already exists and is up to date
    const existingRekap = localStorage.getItem("rekap_reputasi");
    const existingRekapData = existingRekap ? JSON.parse(existingRekap) : [];

    // Get all unique periods from REPUTASI_rows
    const periodsInRows = new Set(REPUTASI_rows.map(r => `${r.year}|${r.quarter}`));
    ...
    if (needsSync && REPUTASI_rows.length > 0) {

      // Re-use the same mapping logic
      const rekapRows = REPUTASI_rows.map((r) => ({
        id: makeRowKey(r),
        year: r.year,
        quarter: r.quarter,
        riskFormId: "reputasi",
        ...
        formula: r.formula || "",
        isPercent: !!r.isPercent,
        hasil: r.mode === "TEKS" ? "" : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeReputasiHasil(r)),
        hasilText: r.mode === "TEKS" ? (r.hasilText || "") : "",
  ...
  // ============================================================
  // LISTEN FOR REKAPDATA CHANGES (Bidirectional Sync) - FIXED
  // (includes percent-normalization for peringkat calculation)
  // ============================================================
  useEffect(() => {
    const handleRowsChange = (e) => {
      if (e?.detail?.origin === "REPUTASI") {
        return;
      }
      ...
      try {
        ...
        const repaired = parsed.map((row) => {
          try {
            ...
            // ----------------------
            // 1) compute hasil (raw) if missing
            // ----------------------
            const computedHasil = (r.mode === "TEKS")
              ? ""
              : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeReputasiHasil(r));

            // Keep original r.hasil for display / compatibility.
            // We'll use a normalized value only for peringkat calculation.
            const hasilRaw = computedHasil === "" || computedHasil == null ? "" : Number(computedHasil);

            // ----------------------
            // 2) NORMALIZE for percent-case:
            // If the indicator is percent (isPercent === true) but value > 1 (e.g. 20),
            // convert to decimal (20 -> 0.2) BEFORE passing to calculatePeringkat.
            // This prevents misclassification when Rekap saved percent as "20" instead of 0.2.
            // ----------------------
            const riskLevels = {
              low: r.low || "",
            ...
            const isPercentFlag = inferredIsPercent;

            // Jangan bagi 100 - biarkan calculatePeringkat handle konversi
            // const hasilForPeringkat = hasilRaw;

            // ----------------------
            // 4) compute peringkat (handle TEKS/numeric cases)
            // ----------------------
            let newPeringkat = 0;

            if ((r.mode || "RASIO") === "TEKS") {
              // TEKS mode: numeric-as-text or direct text match
              if (isNumericRiskLevels(riskLevels)) {
                const parsedNum = parseFloat(r.hasilText);
                if (!isNaN(parsedNum)) {
                  // Gunakan nilai asli, calculatePeringkat akan handle konversi
                  newPeringkat = calculatePeringkat(parsedNum, riskLevels, isPercentFlag);
                } else {
              ...
              // Numeric modes: use hasilForPeringkat (already normalized)
              if (hasilForPeringkat === "" || hasilForPeringkat == null || !Number.isFinite(hasilForPeringkat)) {
                // fallback to computeReputasiHasil (and normalize again)
                const fallback = computeReputasiHasil(r);
                if (fallback !== "" && fallback != null && Number.isFinite(fallback)) {
                  let fb = Number(fallback);
                  if (isPercentFlag && Math.abs(fb) > 1) fb = fb / 100;
                  newPeringkat = calculatePeringkat(fb, riskLevels, isPercentFlag);
                } else {
                  newPeringkat = 0;
                }
              } else {
                newPeringkat = calculatePeringkat(hasilForPeringkat, riskLevels, isPercentFlag);
              }
            }
            ...
            // ----------------------
            // 5) recalc weighted based on new peringkat
            // ----------------------
            ...
            // Do not overwrite display hasil (keputusan: leave as-is).
            // If you prefer to standardize stored hasil to decimal, uncomment below:
            // if (isPercentFlag && Number.isFinite(hasilRaw) && Math.abs(hasilRaw) > 1) r.hasil = hasilRaw / 100;

            return r;
          } catch (innerErr) {
            console.warn("[REPUTASI] repair row failed", innerErr);
            return row;
          }
        });

        // Skip if unchanged (prevents repeated recalculation loops)
        try {
        ...
        setREPUTASI_rows(repaired);
      } catch (err) {
        console.warn("[REPUTASI] gagal reload rekap data:", err);
      } finally {
        setTimeout(() => {
          isProcessingRekapChange.current = false;
        }, 100);
    ...
    const handleSectionsChange = () => {
      try {
        ...
        if (raw) setREPUTASI_sections(JSON.parse(raw));
      } catch (err) {
        console.warn('[REPUTASI] Failed to parse sections from localStorage', err);
      }
    };

    window.addEventListener("reputasiRows:changed", handleRowsChange);
    window.addEventListener("reputasiSections:changed", handleSectionsChange);

    return () => {
      window.removeEventListener("reputasiRows:changed", handleRowsChange);
      window.removeEventListener("reputasiSections:changed", handleSectionsChange);
    };
  }, []);

  // ============================================================
  // ONE-TIME RECALC ON MOUNT
  // Ensures imported Reputasi rows get peringkat/weighted updated
    ...
    try {
      ...
      if (newRowsStr && newRowsStr !== currentRowsStr) {
        ...
        try {
          ...
          console.warn("[REPUTASI] Failed to save repaired rows on mount", e);
        }
      }
    } catch {
      // silent
  ...
  }, [REPUTASI_rows, viewYear, viewQuarter, query]);

  const REPUTASI_totalWeighted = useMemo(
    () =>
      REPUTASI_filtered.reduce(
  ...
  const buildBaseRow = () => {
    const peringkatNum =
      REPUTASI_form.peringkat === "" || REPUTASI_form.peringkat == null
        ? 0
        : Number(REPUTASI_form.peringkat);
    ...
    return {
      ...
      numeratorLabel:
        REPUTASI_form.mode === "NILAI_TUNGGAL" || REPUTASI_form.mode === "TEKS"
          ? ""
          : REPUTASI_form.numeratorLabel,
      numeratorValue:
        REPUTASI_form.mode === "NILAI_TUNGGAL" || REPUTASI_form.mode === "TEKS"
          ? ""
          : REPUTASI_form.numeratorValue,
  ...
  const REPUTASI_addRow = () => {
    const baseRow = buildBaseRow();
    const rawHasil = computeReputasiHasil(baseRow);

    // Pastikan tipe data number
    const bobotSectionNum = Number(baseRow.bobotSection || 0);
    const bobotIndikatorNum = Number(baseRow.bobotIndikator || 0);
    const peringkatNum = Number(baseRow.peringkat || 0);

    const weightedAuto = round2((bobotSectionNum * bobotIndikatorNum * peringkatNum) / 10000);
    ...
    const newRow = {
      ...
      inheritedFrom: null,
      inheritedAt: null,
      bobotIndikator: bobotIndikatorNum, // Simpan sebagai number
    ...
    const newRowKey = makeRowKey({ ...newRow, source: "REPUTASI" });

    setREPUTASI_rows((prev) => {
      const isDuplicate = prev.some(r =>
        makeRowKey({ ...r, source: "REPUTASI" }) === newRowKey
      );

      if (isDuplicate) {
        console.warn('[REPUTASI_addRow] DUPLICATE DETECTED! Row already exists:', newRowKey);
  ...
  const REPUTASI_saveEdit = () => {
    ...
    const bobotSectionNum = Number(baseRow.bobotSection || 0);
    const bobotIndikatorNum = Number(baseRow.bobotIndikator || 0);
    const peringkatNum = newPeringkat;  // ✅ GUNAKAN newPeringkat

    const weightedAuto = round2((bobotSectionNum * bobotIndikatorNum * peringkatNum) / 10000);

    setREPUTASI_rows((prev) =>
      prev.map((r) =>
        r === REPUTASI_editingRow
          ? {
            ...baseRow,
            year: REPUTASI_editingRow.year,
            quarter: REPUTASI_editingRow.quarter,
            source: "REPUTASI",
            isFinal: true,
            inheritedFrom: null,
            inheritedAt: null,
  ...
  const REPUTASI_removeRow = (row) => {
    if (!row) return;
    setREPUTASI_rows((arr) => arr.filter((r) => r !== row));
    if (REPUTASI_editingRow === row) REPUTASI_resetForm();
  };

  const REPUTASI_exportExcel = () =>
    exportInvestasiToExcel
      ? exportInvestasiToExcel(REPUTASI_filtered, viewYear, viewQuarter)
      : null;
  ...
  const loadKPMRRows = () => {
    if (typeof window === "undefined") return [];
    try {
      const raw = localStorage.getItem(KPMR_REPUTASI_LS);
      return raw ? JSON.parse(raw) : [];
    } catch (e) {
      console.warn("Gagal parse KPMR Reputasi dari localStorage", e);
      return [];
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('reputasiKPMR:changed', {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Gagal simpan KPMR Reputasi ke localStorage", e);
    }
  }, [KPMR_rows]);
      ...
      <div className="mb-6 border-b border-gray-200">
        <nav className="flex space-x-8">
          <button
            ...
            className={`py-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === "reputasi"
            ...
            <header className="px-4 py-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-end gap-4">
                ...
                <div className="flex items-end gap-2">
                  {/* Button Tambah Data */}
                  <button
                    onClick={() => setShowReputasiForm(true)}
                  ...
                  {/* <button
                    onClick={repairWeightedData}
                    className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-yellow-500 text-white hover:bg-yellow-600 ml-2"
                  >
                    🔧 Repair Data
                  </button> */}
                ...
                <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-white font-semibold text-lg sm:text-xl">
                      Form Section – Reputasi
                    </h2>

                    <button
                      onClick={() => setShowReputasiForm(false)}
                  ...
                  <div className="flex items-end gap-4 mb-4">
                    ...
                    <div className="flex gap-2 self-end">
                      ...
                      <button
                        ...
                        onClick={() =>
                          REPUTASI_sectionForm.id &&
                          REPUTASI_removeSection(REPUTASI_sectionForm.id)
                        }
                        title="Hapus Section"
                      >
                        <Trash2 size={20} />
                  ...
                  <div className="grid grid-cols-12 gap-4 mb-4">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        value={REPUTASI_form.subNo}
                        onChange={(e) =>
                          setREPUTASI_form((f) => ({ ...f, subNo: e.target.value }))
                        }
                        placeholder="1.1"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={REPUTASI_form.mode === "RASIO" ? "10000" : "5"}
                        />
                      </div>
                    </div>
                  )}
                      ...
                      <div className="col-span-6 space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setREPUTASI_form((f) => ({ ...f, dampak: e.target.value }))
                            }
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={(() => {
                                const peringkatNum =
                                  REPUTASI_form.peringkat === "" ||
                                    REPUTASI_form.peringkat == null
                                    ? 0
                                    : Number(REPUTASI_form.peringkat);
                          ...
                          <div>
                            ...
                            <input
                              className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                              value={(() => {
                                if (REPUTASI_form.mode === "TEKS") {
                                  return REPUTASI_form.hasilText || "";
                                }
                                ...
                                if (REPUTASI_form.mode === "NILAI_TUNGGAL") {
              ...
              <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
                ...
                <div className="relative h-[350px]">
                  <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
                    <DataTable
                      rows={REPUTASI_filtered}
                      totalWeighted={REPUTASI_totalWeighted}
                      viewYear={viewYear}
                      viewQuarter={viewQuarter}
                      startEdit={REPUTASI_startEdit}
                      removeRow={REPUTASI_removeRow}
                    />
                  </div>
                </div>
