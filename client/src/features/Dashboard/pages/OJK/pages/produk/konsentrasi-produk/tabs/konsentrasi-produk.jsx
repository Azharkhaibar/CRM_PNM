...
import useKpmrKonsentrasi from '../hook/kpmr/konsentrasi-produk-kpmr.hook';
...
export default function KonsentrasiProdukOJK() {
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrKonsentrasi();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [KonsentrasiProduk] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [KonsentrasiProduk] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [KonsentrasiProduk] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [KonsentrasiProduk] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [KonsentrasiProduk] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [KonsentrasiProduk] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [KonsentrasiProduk] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [KonsentrasiProduk] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [KonsentrasiProduk] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [KonsentrasiProduk] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [KonsentrasiProduk] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [KonsentrasiProduk] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [KonsentrasiProduk] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [KonsentrasiProduk] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <KonsentrasiProdukKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [KonsentrasiProduk] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [KonsentrasiProduk] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [KonsentrasiProduk] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'inherent') {
      exportInherent({
        ...
        year,
        quarter: activeQuarter,
        categoryLabel: 'KonsentrasiProduk',
      });
    }
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <KonsentrasiProdukInherentWrapper rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
