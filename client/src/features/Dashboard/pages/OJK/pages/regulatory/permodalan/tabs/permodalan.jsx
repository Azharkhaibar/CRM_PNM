...
import PermodalanInherent from './permodalan-inherent';
import PermodalanKpmrPage from './permodalan-kpmr';
...
import useKpmrPermodalan from '../hooks/kpmr/permodalan-kpmr.hook';
...
export default function PermodalanOJK() {
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrPermodalan();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [Permodalan] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [Permodalan] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [Permodalan] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [Permodalan] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [Permodalan] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [Permodalan] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [Permodalan] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [Permodalan] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [Permodalan] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [Permodalan] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [Permodalan] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [Permodalan] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [Permodalan] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [Permodalan] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <PermodalanKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [Permodalan] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [Permodalan] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [Permodalan] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Permodalan" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <PermodalanInherent rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
