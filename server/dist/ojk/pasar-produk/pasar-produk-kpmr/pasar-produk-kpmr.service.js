...
let KpmrPasarService = KpmrPasarService_1 = class KpmrPasarService {
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(pasar_produk_kpmr_aspek_entity_1.KpmrAspekPasar, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(pasar_produk_kpmr_pertanyaan_entity_1.KpmrPertanyaanPasar, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(pasar_produk_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(pasar_produk_kpmr_dto_1.FrontendAspekResponseDto, {
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(pasar_produk_kpmr_aspek_entity_1.KpmrAspekPasar, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(pasar_produk_kpmr_dto_1.FrontendPertanyaanResponseDto, {
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(pasar_produk_kpmr_pertanyaan_entity_1.KpmrPertanyaanPasar)
                    .findOne({
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(pasar_produk_kpmr_pertanyaan_entity_1.KpmrPertanyaanPasar, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(pasar_produk_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrPasarService = KpmrPasarService = KpmrPasarService_1 = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(pasar_produk_ojk_entity_1.KpmrPasarOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(pasar_produk_kpmr_aspek_entity_1.KpmrAspekPasar)),
    __param(2, (0, typeorm_1.InjectRepository)(pasar_produk_kpmr_pertanyaan_entity_1.KpmrPertanyaanPasar)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KpmrPasarService);
//# sourceMappingURL=pasar-produk-kpmr.service.js.map