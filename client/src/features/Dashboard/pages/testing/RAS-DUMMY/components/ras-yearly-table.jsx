    ...
    <section className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200 flex flex-col w-full h-full">
      <div className="p-3 bg-gray-50 border-b border-gray-200 flex justify-end gap-3">
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${showNumDen ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-100'}`}
        >
                        ...
                        <React.Fragment key={item.id}>
                          <tr className="hover:bg-blue-50/50 transition-colors border-b border-gray-300">
                            ...
                            <td rowSpan={itemSpan} className="px-2 py-3 text-center text-xs align-top border border-gray-400">
                              {formatRasDisplayValue(dataYearMinus2?.rkapTarget, item.unitType)}
                            </td>
                            <td rowSpan={itemSpan} className="px-2 py-3 text-center text-xs align-top border border-gray-400">
                              {formatRasDisplayValue(dataYearMinus2?.rasLimit, item.unitType)}
                            </td>
                            <td rowSpan={itemSpan} className="px-2 py-3 text-center text-xs align-top border border-gray-400">
                              {formatRasDisplayValue(dataYearMinus1?.rkapTarget, item.unitType)}
                            </td>
                            <td rowSpan={itemSpan} className="px-2 py-3 text-center text-xs align-top border border-gray-400">
                              {formatRasDisplayValue(dataYearMinus1?.rasLimit, item.unitType)}
                            </td>
                            {showStats && (
                              <>
                          ...
                          {showNumDen && hasCalc && (
                            <>
                              <tr className="bg-indigo-50/50 text-xs text-gray-600">
                                <td className="px-4 py-2 border border-gray-400 italic text-black bg-indigo-50">{item.numeratorLabel}</td>
                              </tr>
                              <tr className="bg-indigo-50/50 text-xs text-gray-600">
                                <td className="px-4 py-2 border border-gray-400 italic text-black bg-indigo-50">{item.denominatorLabel}</td>
                              </tr>
                            </>
                          )}
                        </React.Fragment>
