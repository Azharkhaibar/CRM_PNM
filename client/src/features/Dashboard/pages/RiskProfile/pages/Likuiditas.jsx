...
import { exportLikuiditasToExcel } from "../utils/exportExcel";

// ==== Utils Export KPMR ====
import { exportKPMRLikuiditasToExcel } from "../utils/exportExcelKPMR";
...
const LIKUIDITAS_SECTIONS_LS = "likuiditas_sections_v2";
const LIKUIDITAS_SECTIONS_LS_V1 = "likuiditas_sections_v1"; // Backup key for migration
const KPMR_LIKUIDITAS_LS = "kpmr_likuiditas_rows_v1";
const KPMR_ASPEK_LS = "kpmr_likuiditas_aspek_v1";
const KPMR_PERTANYAAN_LS = "kpmr_likuiditas_pertanyaan_v1";
const KPMR_TEMPLATE_LS = "kpmr_likuiditas_template_v1";

// Year-Level Definitions: Shared across ALL quarters within the same year
const KPMR_DEFINITIONS_LS = "kpmr_likuiditas_definitions_v1";

// Quarter-Level Scores: sectionSkor per quarter, linked by definitionId
const KPMR_SCORES_LS = "kpmr_likuiditas_scores_v1";
...
const likuiditasFallbackEmpty = (year, quarter) => ({
  year,
  quarter,
  no: "1",
  subNo: "1.1",
...
const loadLikuiditasRows = () => {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem("likuiditasRows");
    return raw ? JSON.parse(raw) : [];
  } catch (e) {
    console.warn("Gagal parse likuiditasRows dari localStorage", e);
    return [];
  }
};
...
const makeRowKey = (r) => {
  const key = `${r.source || "LIKUIDITAS"}|${r.year}|${r.quarter}|${r.no ?? ""}|${r.subNo ?? ""}|${r.sectionLabel ?? ""}|${r.indikator ?? ""}`;
  return key;
};
...
export default function Likuiditas() {
  ...
  // ----- SECTION LIST -----
  const [LIKUIDITAS_sections, setLIKUIDITAS_sections] = useState(() => {
    if (typeof window === "undefined") return [];
    try {
      // Try loading v2 first
      const rawV2 = localStorage.getItem(LIKUIDITAS_SECTIONS_LS);
      if (rawV2) {
        return JSON.parse(rawV2);
      }

      // Try migrating from v1
      const rawV1 = localStorage.getItem(LIKUIDITAS_SECTIONS_LS_V1);
      if (rawV1) {
        ...
        localStorage.setItem(LIKUIDITAS_SECTIONS_LS_V1 + "_backup", rawV1);
        localStorage.setItem(LIKUIDITAS_SECTIONS_LS, JSON.stringify(v2Sections));
        return v2Sections;
      }
  ...
  const currentPeriodSections = useMemo(() => {
    return getSectionsForPeriod(viewYear, viewQuarter, LIKUIDITAS_sections);
  }, [viewYear, viewQuarter, LIKUIDITAS_sections]);
  ...
  function LIKUIDITAS_addSection() {
    const sectionData = {
    ...
    setLIKUIDITAS_sections((prev) => {
      const newSections = addSectionToPeriod(viewYear, viewQuarter, sectionData, prev);
      localStorage.setItem(LIKUIDITAS_SECTIONS_LS, JSON.stringify(newSections));
  ...
  function LIKUIDITAS_removeSection(id) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        LIKUIDITAS_SECTIONS_LS,
        JSON.stringify(LIKUIDITAS_sections)
      );
    } catch (e) {
      console.warn("Gagal simpan likuiditas sections", e);
    }
  }, [LIKUIDITAS_sections]);
  ...
  useEffect(() => {
    ...
    const updatedSections = autoCloneSectionsIfNeeded(viewYear, viewQuarter, LIKUIDITAS_sections);

    // Hanya update jika ada perubahan (clone terjadi)
    if (updatedSections.length !== LIKUIDITAS_sections.length) {
      setLIKUIDITAS_sections(updatedSections);
      localStorage.setItem(LIKUIDITAS_SECTIONS_LS, JSON.stringify(updatedSections));
    }
  }, [viewYear, viewQuarter]);


  // ----- ROW & FORM -----
  const likuiditasMakeRow = () =>
    typeof makeEmptyRow === "function"
      ...
      : likuiditasFallbackEmpty(viewYear, viewQuarter);

  // 1) semua baris indikator likuiditas (multi tahun & quarter)
  const [LIKUIDITAS_rows, setLIKUIDITAS_rows] = useState(loadLikuiditasRows);

  // 2) form indikator yang sedang diisi
  const [LIKUIDITAS_form, setLIKUIDITAS_form] = useState(likuiditasMakeRow());
  ...
  useEffect(() => {
    try {
      latestRowsStrRef.current = JSON.stringify(LIKUIDITAS_rows);
    } catch {
  ...
  useEffect(() => {
    const result = cloneFromPreviousPeriod({
      rows: LIKUIDITAS_rows,
      targetYear: viewYear,
      targetQuarter: viewQuarter,
      sourceLabel: "LIKUIDITAS",
    });

    if (result.cloned) {
      setLIKUIDITAS_rows(result.rows);
      setInheritInfo({
        from: result.from,
        count: result.count,
      });
  ...
  // ============================================================
  useEffect(() => {
    if (!LIKUIDITAS_rows?.length || isProcessingWeighted.current) return;
    
    let hasChanges = false;
    
    const updatedRows = LIKUIDITAS_rows.map(row => {
    ...
    if (hasChanges) {
      ...
      setLIKUIDITAS_rows(updatedRows);
      // Reset flag setelah state update
      setTimeout(() => {
        isProcessingWeighted.current = false;
      }, 0);
    }
  }, [LIKUIDITAS_rows.map(r => `${r.no}-${r.subNo}-${r.peringkat}-${r.bobotIndikator}`).join(',')]);

  // --- sinkronkan semua baris ke localStorage (supaya tidak hilang) ---
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      localStorage.setItem("likuiditasRows", JSON.stringify(LIKUIDITAS_rows));

      // Notify RekapData/Rekap1 without re-triggering Likuiditas sync
      window.dispatchEvent(
        new CustomEvent("likuiditasRows:changed", { detail: { origin: "LIKUIDITAS" } })
      );
    } catch (e) {
      console.warn("Gagal simpan likuiditasRows ke localStorage", e);
    }
  }, [LIKUIDITAS_rows]);

  // --- bentuk data ringkas untuk RekapData & simpan ke localStorage ---
  // ============================================================
  // SIMPAN DATA RINGKAS UNTUK REKAP1.JSX (LIKUIDITAS)
  // ============================================================
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Map ALL data from LIKUIDITAS_rows, not just filtered by viewYear/viewQuarter
    // This ensures we save data for ALL periods, not just the current view
    const rekapRows = LIKUIDITAS_rows.map((r, idx) => ({
      // id stabil
      id: makeRowKey(r),
      ...
      // identitas risk form
      riskFormId: "likuiditas",
      riskFormLabel: "Likuiditas",
    ...
    try {
      // Save all data
      localStorage.setItem("rekap_likuiditas", JSON.stringify(uniqueRows));
    } catch (e) {
      console.warn("Gagal simpan rekap_likuiditas", e);
    }
  }, [LIKUIDITAS_rows]);
  ...
  useEffect(() => {
    ...
    // Check if rekap_likuiditas already exists and is up to date
    const existingRekap = localStorage.getItem("rekap_likuiditas");
    const existingRekapData = existingRekap ? JSON.parse(existingRekap) : [];

    // Get all unique periods from LIKUIDITAS_rows
    const periodsInRows = new Set(LIKUIDITAS_rows.map(r => `${r.year}|${r.quarter}`));
    ...
    if (needsSync && LIKUIDITAS_rows.length > 0) {
      // Re-use the same mapping logic
      const rekapRows = LIKUIDITAS_rows.map((r) => ({
        id: makeRowKey(r),
        year: r.year,
        quarter: r.quarter,
        riskFormId: "likuiditas",
        ...
        formula: r.formula || "",
        isPercent: !!r.isPercent,
        hasil: r.mode === "TEKS" ? "" : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeLikuiditasHasil(r)),
        hasilText: r.mode === "TEKS" ? (r.hasilText || "") : "",
  ...
  useEffect(() => {
    const handleRekapDataChange = (e) => {
      ...
      try {
        ...
        const repaired = parsed.map((row) => {
          try {
            ...
            const computedHasil = (r.mode === "TEKS")
              ? ""
              : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeLikuiditasHasil(r));
            ...
            if ((r.mode || "RASIO") === "TEKS") {
              ...
              if (hasilForPeringkat === "" || hasilForPeringkat == null || !Number.isFinite(hasilForPeringkat)) {
                const fallback = computeLikuiditasHasil(r);
        ...
        setLIKUIDITAS_rows(repaired);
      } catch (err) {
        console.warn("[LIKUIDITAS] gagal reload rekap data:", err);
      } finally {
        setTimeout(() => {
    ...
    window.addEventListener("likuiditasRows:changed", handleRekapDataChange);
    return () => window.removeEventListener("likuiditasRows:changed", handleRekapDataChange);
  }, []);

  // ============================================================
  // ONE-TIME RECALC ON MOUNT
  // Ensures imported Likuiditas rows get peringkat/weighted updated
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (newRowsStr && newRowsStr !== currentRowsStr) {
        setLIKUIDITAS_rows(repaired);
        try {
          localStorage.setItem("likuiditasRows", JSON.stringify(repaired));
        } catch (e) {
          console.warn("[LIKUIDITAS] Failed to save repaired rows on mount", e);
        }
      }
    } catch {
      // silent
  ...
  }, [LIKUIDITAS_rows, viewYear, viewQuarter, query]);

  const LIKUIDITAS_totalWeighted = useMemo(
    () =>
      LIKUIDITAS_filtered.reduce(
  ...
  const LIKUIDITAS_resetForm = () => {
    setLIKUIDITAS_form(likuiditasMakeRow());
    setLIKUIDITAS_editingRow(null);
  };

  const buildBaseRow = () => {
    const peringkatNum =
      LIKUIDITAS_form.peringkat === "" || LIKUIDITAS_form.peringkat == null
        ? 0
        : Number(LIKUIDITAS_form.peringkat);
    ...
    return {
      ...
      numeratorLabel:
        LIKUIDITAS_form.mode === "NILAI_TUNGGAL" || LIKUIDITAS_form.mode === "TEKS"
          ? ""
          : LIKUIDITAS_form.numeratorLabel,
      numeratorValue:
        LIKUIDITAS_form.mode === "NILAI_TUNGGAL" || LIKUIDITAS_form.mode === "TEKS"
          ? ""
          : LIKUIDITAS_form.numeratorValue,
  ...
  const LIKUIDITAS_addRow = () => {
    ...
    const newRowKey = makeRowKey({ ...newRow, source: "LIKUIDITAS" });

    setLIKUIDITAS_rows((prev) => {
      const isDuplicate = prev.some(r =>
        makeRowKey({ ...r, source: "LIKUIDITAS" }) === newRowKey
      );

      if (isDuplicate) {
        console.warn('[LIKUIDITAS_addRow] DUPLICATE DETECTED! Row already exists:', newRowKey);
    ...
    // ✅ Hapus baris duplikat setLIKUIDITAS_rows
  ...
  const LIKUIDITAS_saveEdit = () => {
    ...
    setLIKUIDITAS_rows((prev) =>
      prev.map((r) =>
        r === LIKUIDITAS_editingRow
          ? {
            ...baseRow,
            // ✅ PERTAHANKAN year dan quarter yang asli
            year: LIKUIDITAS_editingRow.year,
            quarter: LIKUIDITAS_editingRow.quarter,
            hasil: baseRow.mode === "TEKS" ? "" : (rawHasil === "" || rawHasil == null ? 0 : rawHasil),
            hasilText: baseRow.mode === "TEKS" ? (baseRow.hasilText || "") : "",
  ...
  const LIKUIDITAS_removeRow = (row) => {
    if (!row) return;
    setLIKUIDITAS_rows((arr) => arr.filter((r) => r !== row));
    if (LIKUIDITAS_editingRow === row) LIKUIDITAS_resetForm();
  };
  ...
  const loadKPMRRows = () => {
    if (typeof window === "undefined") return [];
    try {
      const raw = localStorage.getItem(KPMR_LIKUIDITAS_LS);
      return raw ? JSON.parse(raw) : [];
    } catch (e) {
      console.warn("Gagal parse KPMR Likuiditas dari localStorage", e);
      return [];
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('likuiditasKPMR:changed', {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Gagal simpan KPMR Likuiditas ke localStorage", e);
    }
  }, [KPMR_rows]);
            ...
            <header className="px-4 py-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-end gap-4">
                ...
                <div className="flex items-end gap-2">
                  {/* Button Tambah Data */}
                  <button
                    onClick={() => setShowLikuiditasForm(true)}
                  ...
                  <button
                    onClick={LIKUIDITAS_exportExcel}
              ...
              <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
                ...
                <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() => setShowLikuiditasForm(false)}
                      ...
                      {!isAddingNewSection && !isEditingSection && (
                        <>
                          ...
                          <button
                            ...
                            onClick={() =>
                              LIKUIDITAS_sectionForm.id &&
                              LIKUIDITAS_removeSection(LIKUIDITAS_sectionForm.id)
                            }
                            title="Hapus Section"
                          >
                            <Trash2 size={20} />
                    ...
                    >
                      ...
                      {currentPeriodSections.map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.inheritedFrom && "🔄 "} {s.no} - {s.parameter}
                        </option>
                      ))}
                      <option value="ADD_NEW_SECTION">
                        + Tambah Section Baru
                      </option>
                    </select>
                    <ChevronDown
                  ...
                  <div className="grid grid-cols-12 gap-4 mb-4">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        value={LIKUIDITAS_form.subNo}
                        onChange={(e) =>
                          setLIKUIDITAS_form((f) => ({ ...f, subNo: e.target.value }))
                        }
                        placeholder="1.1"
                      />
                    </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setRawNumeratorInput(raw);
                            setLIKUIDITAS_form((f) => ({
                              ...f,
                              numeratorValue: raw, // SIMPAN RAW STRING untuk flexible input
                            }));
                          }}
                          onBlur={(e) => {
                            const val = e.target.value;
                          ...
                          placeholder="250, -100, 1.000,50, dll"
                        />
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const raw = e.target.value;
                            setRawDenominatorInput(raw);
                            setLIKUIDITAS_form((f) => ({
                              ...f,
                              denominatorValue: raw, // SIMPAN RAW STRING untuk flexible input
                            }));
                          }}
                          onBlur={(e) => {
                            const val = e.target.value;
                          ...
                          placeholder={LIKUIDITAS_form.mode === "RASIO" ? "10000, -5000, dll" : "5, -3, dll"}
                        />
                      </div>
                    </div>
                  )}
                      ...
                      <div className="col-span-6 space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setLIKUIDITAS_form((f) => ({ ...f, dampak: e.target.value }))
                            }
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              value={LIKUIDITAS_form.peringkat ?? ""}
                              readOnly
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={(() => {
                                const peringkatNum =
                                  LIKUIDITAS_form.peringkat === "" ||
                                    LIKUIDITAS_form.peringkat == null
                                    ? 0
                                    : Number(LIKUIDITAS_form.peringkat);
                          ...
                          <div>
                            ...
                            <input
                              className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                              value={(() => {
                                if (LIKUIDITAS_form.mode === "TEKS") {
                                  return LIKUIDITAS_form.hasilText || "";
                                }
                                ...
                                if (LIKUIDITAS_form.mode === "NILAI_TUNGGAL") {
              ...
              <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
                ...
                <div className="relative h-[350px]">
                  <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
                    <DataTable
                      rows={LIKUIDITAS_filtered}
                      totalWeighted={LIKUIDITAS_totalWeighted}
                      viewYear={viewYear}
                      viewQuarter={viewQuarter}
                      startEdit={LIKUIDITAS_startEdit}
                      removeRow={LIKUIDITAS_removeRow}
                    />
                  </div>
                </div>
