...
export class KpmrPasarService {
  ...
  constructor(
    @InjectRepository(KpmrPasarOjk)
    private readonly kpmrRepository: Repository<KpmrPasarOjk>,

    @InjectRepository(KpmrAspekPasar)
    private readonly aspekRepository: Repository<KpmrAspekPasar>,

    @InjectRepository(KpmrPertanyaanPasar)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanPasar>,
  ...
  private async getKpmrWithRelations(id: number): Promise<KpmrPasarOjk> {
  ...
  private async getKpmrEntity(id: number): Promise<KpmrPasarOjk> {
    const kpmr = await this.kpmrRepository.findOne({
      where: { id },
    });
  ...
  private async getAspekWithRelations(id: number): Promise<KpmrAspekPasar> {
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekPasar> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanEntity(id: number): Promise<KpmrPertanyaanPasar> {
    const pertanyaan = await this.pertanyaanRepository.findOne({
      where: { id },
    });
  ...
  private checkKpmrLocked(kpmr: KpmrPasarOjk, action: string): void {
    if (kpmr.isLocked) {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekPasar, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanPasar, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private convertToFrontendFormat(kpmr: KpmrPasarOjk): FrontendKpmrResponseDto {
    if (!kpmr) {
  ...
  async createKpmr(
    createDto: CreateKpmrPasarOjkDto,
  ...
  async findOneEntity(id: number): Promise<KpmrPasarOjk> {
  ...
  async updateKpmr(
    id: number,
    updateDto: UpdateKpmrPasarOjkDto,
    updatedBy?: string,
  ): Promise<FrontendKpmrResponseDto> {
  ...
  async createAspek(
    kpmrId: number,
    createDto: CreateKpmrAspekPasarDto,
    ...
    try {
      ...
      const aspekData = {
        nomor: createDto.nomor || undefined, // null → undefined
        judul: createDto.judul.trim(),
        bobot: createDto.bobot,
        deskripsi: createDto.deskripsi || undefined, // null → undefined
        kpmrOjkId: kpmrId,
        orderIndex: createDto.orderIndex ?? 0,
        averageScore: createDto.averageScore ?? undefined, // null → undefined
        rating: createDto.rating || undefined, // null → undefined
        updatedBy: createDto.updatedBy || undefined, // null → undefined
        notes: createDto.notes || undefined, // null → undefined
      };
      ...
      if (createDto.pertanyaanList?.length) {
        ...
        const pertanyaanEntities = createDto.pertanyaanList.map((q) => {
          ...
          const pertanyaanData = {
            nomor: q.nomor || undefined, // null → undefined
            pertanyaan: q.pertanyaan,
            skor: q.skor || {},
            indicator: q.indicator || {},
            evidence: q.evidence || undefined, // null → undefined
            catatan: q.catatan || undefined, // null → undefined
            aspekId: savedAspek.id,
      ...
      return {
        ...
        // updatedBy: aspekWithRelations.updatedBy, // HAPUS jika tidak ada di DTO
        // notes: aspekWithRelations.notes || '', // HAPUS jika tidak ada di DTO
        pertanyaanList:
  ...
  async updateAspek(
    id: number,
    updateDto: UpdateKpmrAspekPasarDto,
    updatedBy?: string,
  ): Promise<FrontendAspekResponseDto> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekPasar, reorderDto.aspekIds[i], {
            orderIndex: i,
          });
        }
      });
  ...
  async createPertanyaan(
    aspekId: number,
    createDto: CreateKpmrPertanyaanPasarDto,
    ...
    try {
      ...
      const pertanyaanData = {
        ...
        evidence: createDto.evidence || undefined, // null → undefined
        catatan: createDto.catatan || undefined, // null → undefined
        aspekId: aspekId,
        orderIndex: createDto.orderIndex ?? 0,
      };
  ...
  async updatePertanyaan(
    id: number,
    updateDto: UpdateKpmrPertanyaanPasarDto,
  ): Promise<FrontendPertanyaanResponseDto> {
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanPasar)
          .findOne({
