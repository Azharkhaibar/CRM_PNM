...
export function exportKPMRInvestasiToExcel({ year, quarter, rows }) {
    ...
    // Iterasi baris konten (mulai baris ke-3 = index 2)
    for (let R = 2; R <= range.e.r; R++) {
        ...
        // Deteksi baris ASPEK: kolom A string yang mengandung "Aspek "
        const isAspekRow =
            typeof aVal === "string" && aVal.toLowerCase().includes("aspek");

        // Deteksi baris FOOTER: kolom A dan B kosong, ada nilai di kolom C
        const isFooterRow =
            (ws[aAddr]?.v === "" || ws[aAddr] == null) &&
            (ws[bAddr]?.v === "" || ws[bAddr] == null) &&
            ws[cAddr] &&
            ws[cAddr].v !== "" &&
            R === range.e.r; // baris terakhir

        if (isAspekRow) {
            // Merge A..B untuk baris aspek
            ws["!merges"].push({ s: { r: R, c: 0 }, e: { r: R, c: 1 } });

            // Style A & B (hijau muda, bold, left)
            const aspekStyle = {
            ...
            // Style skor (C) – hijau terang, bold, center
            if (ws[cAddr]) {
                ws[cAddr].s = {
                    ...(ws[cAddr].s || {}),
        ...
        if (isFooterRow) {
            // Warnai seluruh baris footer biru muda
            for (let C = 0; C <= 8; C++) {
                const addr = XLSX.utils.encode_cell({ r: R, c: C });
                if (!ws[addr]) continue;
                ws[addr].s = {
            ...
            // Sel skor (C) tetap hijau terang + bold agar sama dengan web
            if (ws[cAddr]) {
                ws[cAddr].s = {
                    ...(ws[cAddr].s || {}),
                    fill: { patternType: "solid", fgColor: { rgb: SKOR_CELL_FILL } },
