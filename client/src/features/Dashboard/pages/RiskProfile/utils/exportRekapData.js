...
export function exportRekapDataToExcel(
    visibleGroups,
    viewYear,
    viewQuarter,
    mode = "triwulan",
    ...
    const SOURCE_ORDER = [
        "INVESTASI", "PASAR", "LIKUIDITAS", "OPERASIONAL",
    ...
    if (mode === "triwulan") {
        ...
        quartersToShow = (selectedQuarters && selectedQuarters.length > 0)
            ? QUARTER_ORDER.filter(q => selectedQuarters.includes(q))
            : QUARTER_ORDER;
    }
    ...
    const periodeLabel = mode === "triwulan"
        ? `${QUARTER_LABEL[viewQuarter]}-${viewYear}`
        : `${viewYear}-Tahunan`;
