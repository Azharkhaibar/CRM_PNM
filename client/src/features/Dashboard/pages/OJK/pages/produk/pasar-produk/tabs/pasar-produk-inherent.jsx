...
export default function PasarProdukInherentWrapper() {
  ...
  const handleResetData = async () => {
    if (!confirm('Apakah Anda yakin ingin menghapus/reset semua data Pasar Produk untuk periode ini? Semua parameter dan nilai akan terhapus secara permanen.')) {
      return;
    }
    try {
      await rekapApiService.undoClonePeriodData({
          ...
          <table className="min-w-max text-sm table-fixed">
            <thead>
              <tr>
                ...
                <th className="border border-black px-2 py-2 bg-blue-900 text-white w-16">Bobot</th>
                <th className="border border-black px-2 py-2 bg-blue-900 text-white w-32">% Portofolio</th>
                    ...
                    for (let subIndex = 0; subIndex < rowsForThisNilai; subIndex++) {
                      ...
                      rows.push(
                        <tr key={`${param.id}-${nilai.id}-${subIndex}`}>
                          ...
                          <td className={`border px-2 py-2 text-center ${isMainRow ? 'bg-[#E8F5FA]' : 'bg-white'} break-words max-w-[130px]`}>{isMainRow ? (nilai.portofolio ?? '-') : ''}</td>
