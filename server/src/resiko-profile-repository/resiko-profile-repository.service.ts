// src/resiko-profile-repository/resiko-profile-repository.service.ts
...
@Injectable()
export class ResikoProfileRepositoryService
  implements IRiskProfileRepositoryService
{
  constructor(
    @InjectRepository(RiskProfileRepositoryView)
    private readonly repositoryView: Repository<RiskProfileRepositoryView>,
  ) {}
  ...
  async getRepositoryData(
    ...
    try {
      ...
      if (filters.startDate && filters.endDate) {
        console.log(
          `Applying date range filter: ${filters.startDate} to ${filters.endDate}`,
        );
        queryBuilder.andWhere(
          'view.createdAt BETWEEN :startDate AND :endDate',
          {
      ...
      // Jika error dari database, cek view entity
  ...
  async getRepositoryStatistics(
    ...
    try {
      ...
      // Group by module
      const byModule: Record<ModuleType, RiskProfileRepositoryView[]> =
        this.groupByModule(data);

      // Group by quarter
      const byQuarter: Record<Quarter, RiskProfileRepositoryView[]> =
        this.groupByQuarter(data);

      // Validation status
      const validated = data.filter((item) => item.isValidated).length;
      const notValidated = data.length - validated;

      // Konversi byModule ke array
      const byModuleArray = Object.entries(byModule).map(([module, items]) => {
        const moduleItems = items;
        const totalWeighted = moduleItems.reduce(
          (sum, item) => sum + (item.weighted || 0),
          0,
        );
        return {
          module: this.getModuleName(module as ModuleType),
      ...
      // Konversi byQuarter ke array
      const byQuarterArray = Object.entries(byQuarter).map(
        ([quarter, items]) => {
          const quarterItems = items;
          return {
            quarter,
            count: quarterItems.length,
            totalWeighted: quarterItems.reduce(
              (sum, item) => sum + (item.weighted || 0),
              0,
            ),
          };
      ...
      throw new BadRequestException('Failed to fetch repository statistics');
    }
  }
  ...
  async exportRepositoryData(
    ...
    const pagination: PaginationOptions = { page: 1, limit: 10000 };
    ...
    switch (format) {
      ...
      default:
        throw new BadRequestException(
          `Export format ${format} is not yet supported. Please use CSV.`,
        );
    }
  }

  async getAvailablePeriods(): Promise<
    { year: number; quarters: Quarter[] }[]
  > {
    try {
  ...
  // ========== DEBUG METHOD ==========
  async debugModuleData(
    module: ModuleType,
    year: number = 2026,
    quarter: string = 'Q1', // <-- DIUBAH: string bukan Quarter
  ): Promise<any> {
    try {
      // Validasi quarter
      if (!['Q1', 'Q2', 'Q3', 'Q4'].includes(quarter)) {
        throw new BadRequestException('Invalid quarter value');
      }

      // Langsung query ke database untuk debug
      const query = `
        SELECT 
          COUNT(*) as total,
      ...
      throw new BadRequestException(`Failed to debug module ${module}`);
    }
  }

  // Private helper methods
  private transformToDto(
    entity: RiskProfileRepositoryView,
  ): RiskProfileRepositoryDto {
    return {
      id: entity.id,
  ...
  private calculateStatistics(data: RiskProfileRepositoryView[]) {
    if (data.length === 0) {
      return {
        totalModules: 0,
        totalIndicators: 0,
    ...
    const moduleBreakdown = data.reduce(
      (acc, item) => {
      ...
      {} as Record<ModuleType, { count: number; totalWeighted: number }>,
    );
    ...
    return {
      ...
      totalIndicators: data.length,
      totalWeighted,
      moduleBreakdown: Object.entries(moduleBreakdown).map(
        ([module, stats]) => ({
          module: this.getModuleName(module as ModuleType),
          count: stats.count,
  ...
  private groupByModule(
    data: RiskProfileRepositoryView[],
  ): Record<ModuleType, RiskProfileRepositoryView[]> {
    return data.reduce(
      (acc, item) => {
        const module = item.moduleType;
        if (!acc[module]) {
      ...
      {} as Record<ModuleType, RiskProfileRepositoryView[]>,
    );
  }

  private groupByQuarter(
    data: RiskProfileRepositoryView[],
  ): Record<Quarter, RiskProfileRepositoryView[]> {
    return data.reduce(
      (acc, item) => {
      ...
      {} as Record<Quarter, RiskProfileRepositoryView[]>,
    );
  }
  ...
  // Export methods - Simple CSV export only
  private exportToCsv(data: RiskProfileRepositoryDto[]): Buffer {
    const headers = [
      'Module',
      'Year',
      'Quarter',
