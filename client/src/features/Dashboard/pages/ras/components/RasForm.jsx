...
import { RISK_ATTITUDE_INFO, UNIT_TYPES } from '../utils/rasConstant';

// --- Internal Styled Components (Agar konsisten dengan tema Gradient) ---
...
const Input = ({ className = "", ...props }) => (
  <input
    className={`w-full rounded-xl border-0 bg-white/90 text-gray-800 px-4 py-2.5 focus:ring-2 focus:ring-yellow-400 focus:bg-white transition-all shadow-sm placeholder:text-gray-400 text-sm ${className}`}
    {...props}
  />
);
...
const Select = ({ className = "", children, ...props }) => (
  <select
    className={`w-full rounded-xl border-0 bg-white/90 text-gray-800 px-4 py-2.5 focus:ring-2 focus:ring-yellow-400 focus:bg-white transition-all shadow-sm text-sm ${className}`}
    {...props}
  >
    {children}
  </select>
...
const TextArea = ({ className = "", ...props }) => (
  <textarea
    className={`w-full rounded-xl border-0 bg-white/90 text-gray-800 px-4 py-2.5 focus:ring-2 focus:ring-yellow-400 focus:bg-white transition-all shadow-sm resize-none text-sm ${className}`}
    {...props}
  />
);
...
export default function RasForm({
  existingCategories = [],
  onSubmit,
  onCancel,
  initialData = null,
    ...
    <section className="relative rounded-2xl overflow-hidden mb-8 shadow-lg bg-gradient-to-r from-[#0076C6]/95 via-[#00A3DA]/95 to-[#33C2B5]/95 animate-fade-in">
      <div className="p-6 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-6 border-b border-white/20 pb-4">
          ...
          <div className="text-white/80 text-sm italic bg-white/10 px-3 py-1 rounded-lg">
            * Field wajib diisi
          </div>
        </div>
              ...
              <div className="bg-white/10 p-5 rounded-xl border border-white/10 backdrop-blur-sm">
                  ...
                  {!form.isNewCategory ? (
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        <option value="__NEW__" className="font-bold text-blue-600 bg-blue-50">+ Tambah Kategori Baru...</option>
                      </Select>
                    </div>
                  ) : (
                  ...
                  <div className="col-span-3">
                    <FormGroup label="Parameter" required>
                      <TextArea 
                        ...
                        onChange={handleChange} 
                        rows={2} 
                        placeholder="Deskripsi parameter risiko..."
                      />
                    </FormGroup>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 mb-4 bg-white/10 p-3 rounded-lg border border-white/10">
                  <input 
                    type="checkbox" 
                    id="hasCalc" 
                    name="hasNumeratorDenominator" 
                    checked={form.hasNumeratorDenominator} 
                    onChange={handleChange}
                    className="w-5 h-5 text-blue-600 rounded focus:ring-yellow-400 cursor-pointer"
                  />
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white/10 p-5 rounded-xl border border-white/10 backdrop-blur-sm">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <FormGroup 
                    label="Value Triwulan" 
                    subLabel={form.unitType !== 'X' && form.hasNumeratorDenominator ? "(Auto Calc)" : "(Manual Input)"}
                  >
                    {form.unitType !== 'X' && form.hasNumeratorDenominator ? (
                      // Read Only Auto Calculated Field
                      <div className="w-full rounded-xl border-0 bg-black/20 text-white px-4 py-2.5 font-bold text-lg text-center border border-white/10 shadow-inner">
                        {calculatedResult || '-'}
                      </div>
                    ) : (
                      // Manual Input Field
                      <Input 
                        name="manualQuarterValue" 
                        value={form.manualQuarterValue} 
                        onChange={handleChange} 
                        placeholder={form.unitType === 'X' ? "Input Manual (X)..." : "Masukkan nilai..."}
              ...
              <div className="bg-white/10 p-5 rounded-xl border border-white/10 backdrop-blur-sm">
                ...
                <FormGroup label="Sikap Terhadap Risiko">
                  <Select name="riskStance" value={form.riskStance} onChange={handleChange}>
                    {RISK_ATTITUDE_INFO.map(r => <option key={r.label} value={r.label}>{r.label}</option>)}
                  </Select>
                </FormGroup>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/20 mt-8">
                <button 
                  type="button" 
                  onClick={onCancel} 
                  className="px-6 py-3 rounded-xl bg-white/10 text-white font-semibold hover:bg-white/20 border border-white/30 transition-all backdrop-blur-sm"
                >
                  Batal
                </button>
