...
@Injectable()
export class KpmrKreditService {
  private readonly logger = new Logger(KpmrKreditService.name);
  ...
  private async getAspekWithRelations(id: number): Promise<KpmrAspekKredit> {
  ...
  private async getPertanyaanEntity(id: number): Promise<KpmrPertanyaanKredit> {
    const pertanyaan = await this.pertanyaanRepository.findOne({
      where: { id },
    });
  ...
  private convertToFrontendFormat(kpmr: KpmrKreditOjk): FrontendKpmrResponseDto {
    if (!kpmr) {
