// src/ojk/kepatuhan-produk/kepatuhan-produk-ojk/kepatuhan-produk-ojk.service.tsx
...
import useKepatuhanInherent from '../hooks/inherent/kepatuhan.hook';
...
export default function KepatuhanInherentWrapper() {
  ...
  // PERBAIKAN: Data kepatuhan untuk info
  const currentKepatuhanData = kepatuhan;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      loading,
      currentKepatuhanId,
      currentKepatuhanData: currentKepatuhanData ? `ID: ${currentKepatuhanData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentKepatuhanId, currentKepatuhanData, error]);
  ...
  const backendHandlers = useMemo(
    ...
    [currentKepatuhanId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Kepatuhan - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentKepatuhanId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentKepatuhanId).substring(0, 8)}...</span>}
            </p>
            {kepatuhan?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(kepatuhan.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {kepatuhan?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{kepatuhan.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
...
// Komponen utama
function KepatuhanInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Kepatuhan - Inherent</h1>
