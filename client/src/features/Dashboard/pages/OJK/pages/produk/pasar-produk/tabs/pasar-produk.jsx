...
import PasarProdukInherent from './pasar-produk-inherent';
...
export default function PasarProdukOJK() {
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrPasar();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [PasarProduk] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [PasarProduk] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [PasarProduk] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [PasarProduk] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [PasarProduk] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [PasarProduk] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [PasarProduk] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [PasarProduk] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [PasarProduk] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [PasarProduk] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [PasarProduk] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [PasarProduk] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [PasarProduk] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [PasarProduk] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <PasarProdukKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [PasarProduk] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [PasarProduk] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [PasarProduk] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <PasarProdukInherent rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
