...
import { computeWeighted } from '../../utils/pasar/calc';
...
export default function FormSectionPasar({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Pasar', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    if (!form.sectionLabel?.trim()) validationErrors.push('Nama Parameter harus diisi');
    if (!form.indikator?.trim()) validationErrors.push('Indikator Kinerja harus diisi');
    if (!form.no || isNaN(Number(form.no))) validationErrors.push('Nomor harus berupa angka yang valid');
    if (!form.subNo?.trim()) validationErrors.push('Sub Nomor harus diisi');
    else {
      const hierarchicalRegex = /^\d+(\.\d+)*$/;
      if (!hierarchicalRegex.test(form.subNo)) validationErrors.push('Sub Nomor harus dalam format hierarchical seperti 1.1, 1.2.1, 2.3.4');
    }
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('PASAR', `Menambahkan data pasar baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/pasar',
          isSuccess: true,
          metadata: { no_indikator: form.subNo, parameter: form.sectionLabel, indikator: form.indikator, investment_data: serviceData },
        });
      }
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('PASAR', `Gagal menambahkan data pasar - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/pasar',
          isSuccess: false,
          metadata: { error: error.message, error_details: error.response?.data || error.toString() },
        });
      }
      alert('Gagal menambahkan data pasar: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('PASAR', `Memperbarui data pasar - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/pasar/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: { no_indikator: form.subNo, parameter: form.sectionLabel, indikator: form.indikator },
        });
      }

      alert('Data pasar berhasil diperbarui!');
    } catch (error) {
      console.error('Error saving pasar:', error);
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('PASAR', `Gagal memperbarui data pasar - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/pasar/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: { error: error.message },
        });
      }
      alert('Gagal memperbarui data pasar: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data pasar dan analisis risiko secara komprehensif</p>
            </div>
          </div>
        ...
        <div className="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
          {!editing ? (
            ...
            >
              ...
              {loading ? 'Menyimpan...' : 'Tambah Data Pasar'}
            </button>
          ) : (
            <>
              <button
