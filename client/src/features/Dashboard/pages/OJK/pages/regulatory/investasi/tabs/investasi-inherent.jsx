// src/ojk/investasi-produk/investasi-produk-ojk/investasi-produk-ojk.service.tsx
...
import useInvestasiInherent from '../hooks/inherent/investasi.hook';
...
export default function InvestasiInherentWrapper() {
  ...
  // PERBAIKAN: Gunakan useInvestasiInherent, bukan useInvestasiIntegration
  const { investasi, parameters, loading, saving, error, currentInvestasiId, loadByYearQuarter, refreshData, addParameter, updateParameter, deleteParameter, copyParameter, addNilai, updateNilai, deleteNilai, copyNilai } =
    useInvestasiInherent();
  ...
  // PERBAIKAN: Data investasi untuk info
  const currentInvestasiData = investasi;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      loading,
      currentInvestasiId,
      currentInvestasiData: currentInvestasiData ? `ID: ${currentInvestasiData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentInvestasiId, currentInvestasiData, error]);
  ...
  const backendHandlers = useMemo(
    () => ({
      addAspek: async (createAspekDto) => {
        try {
          ...
          if (!currentInvestasiId) {
            throw new Error('Tidak ada data investasi yang dipilih');
          }
          ...
          // Panggil API
          await addParameter(currentInvestasiId, createAspekDto);
      ...
      addPertanyaan: async (aspekId, createPertanyaanDto) => {
        try {
          ...
          await addNilai(currentInvestasiId, aspekId, createPertanyaanDto);
          await handleRefresh();
      ...
      copyAspek: async (aspekId) => {
        try {
          ...
          await copyParameter(currentInvestasiId, aspekId);
          await handleRefresh();
      ...
      copyPertanyaan: async (aspekId, pertanyaanId) => {
        try {
          ...
          await copyNilai(currentInvestasiId, aspekId, pertanyaanId);
          await handleRefresh();
    ...
    [currentInvestasiId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Investasi - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentInvestasiId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentInvestasiId).substring(0, 8)}...</span>}
            </p>
            {investasi?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(investasi.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {investasi?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{investasi.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <InvestasiInherent rows={safeRows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={loading || saving || isRefreshing} isLocked={investasi?.isLocked || false} />
    </div>
  );
}

// Komponen utama
function InvestasiInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Investasi - Inherent</h1>
