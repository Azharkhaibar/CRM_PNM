...
export function exportKPMRInvestasiToExcel({ year, quarter, rows }) {
  ...
  for (const [k, items] of by.entries()) {
    ...
    const nums = items.map((it) => (it.sectionSkor === '' ? null : Number(it.sectionSkor))).filter((v) => v != null && !isNaN(v));
    ...
    for (const it of items) {
      aoa.push([it.sectionNo, it.sectionTitle, it.sectionSkor === '' ? '' : Number(it.sectionSkor), it.level1, it.level2, it.level3, it.level4, it.level5, it.evidence]);
    }
  }
