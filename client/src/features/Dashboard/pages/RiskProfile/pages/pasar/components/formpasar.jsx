...
*/
export default function FormSectionPasar({ sections = [], setSections, indicatorDraft, setIndicatorDraft, selectedSectionId, setSelectedSectionId, onAddIndicator, onUpdateIndicator, editingIndicatorIndex, onCancelEdit }) {
    ...
    <section className="rounded-xl bg-[#DBDBDB] p-4">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <select value={selectedSectionId || ''} onChange={(e) => setSelectedSectionId(e.target.value || null)} className="rounded-xl border px-3 py-2 bg-white flex-1">
      ...
      <div className="rounded-lg p-4 bg-white">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div>
            ...
            <input className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.title || ''} onChange={(e) => handleChangeDraft('title', e.target.value)} />

            <div className="grid grid-cols-2 gap-3 mt-3">
              <div>
                <label className="block text-sm text-gray-600">Bobot Indikator (%)</label>
                <input type="number" className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.bobotIndikator || ''} onChange={(e) => handleChangeDraft('bobotIndikator', e.target.value === '' ? '' : Number(e.target.value))} />
              </div>

              <div>
                <label className="block text-sm text-gray-600">Peringkat (1-5)</label>
                <input type="number" min={1} max={5} className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.peringkat || 1} onChange={(e) => handleChangeDraft('peringkat', Number(e.target.value || 1))} />
              </div>
            </div>

            <div className="mt-3">
              <label className="block text-sm text-gray-600">Faktor Pembilang (label)</label>
              <input className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.numeratorLabel || ''} onChange={(e) => handleChangeDraft('numeratorLabel', e.target.value)} />
              <label className="block text-sm text-gray-600 mt-2">Total Pembilang (value)</label>
              <input type="number" className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.numeratorValue || ''} onChange={(e) => handleChangeDraft('numeratorValue', e.target.value)} />
            </div>

            <div className="mt-3">
              <label className="block text-sm text-gray-600">Faktor Penyebut (label)</label>
              <input className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.denominatorLabel || ''} onChange={(e) => handleChangeDraft('denominatorLabel', e.target.value)} />
              <label className="block text-sm text-gray-600 mt-2">Total Penyebut (value)</label>
              <input type="number" className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.denominatorValue || ''} onChange={(e) => handleChangeDraft('denominatorValue', e.target.value)} />
            </div>

            <div className="mt-3">
              <label className="block text-sm text-gray-600">Keterangan</label>
              <textarea className="w-full rounded-xl border px-3 py-2" value={indicatorDraft?.keterangan || ''} onChange={(e) => handleChangeDraft('keterangan', e.target.value)} />
            </div>
          </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-2">
              ...
              <div className="flex gap-2 mt-3">
                {editingIndicatorIndex == null ? (
                  ...
                  <>
                    <button className="bg-blue-600 text-white px-4 py-2 rounded-xl" onClick={() => onUpdateIndicator()}>
                      Simpan Perubahan
                    </button>
