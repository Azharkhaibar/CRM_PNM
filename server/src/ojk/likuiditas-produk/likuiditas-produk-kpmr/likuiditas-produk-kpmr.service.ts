...
export class KpmrLikuiditasService {
  ...
  constructor(
    ...
    @InjectRepository(KpmrPertanyaanLikuiditas)
    private readonly pertanyaanRepository: Repository<KpmrPertanyaanLikuiditas>,
  ...
  private async getAspekEntity(id: number): Promise<KpmrAspekLikuiditas> {
    const aspek = await this.aspekRepository.findOne({
      where: { id },
    });
  ...
  private async getPertanyaanWithRelations(
    id: number,
  ): Promise<KpmrPertanyaanLikuiditas> {
  ...
  private async reorderRemainingAspek(kpmrId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < aspekList.length; i++) {
        await manager.update(KpmrAspekLikuiditas, aspekList[i].id, {
          orderIndex: i,
        });
      }
    });
  ...
  private async reorderRemainingPertanyaan(aspekId: number): Promise<void> {
    ...
    await this.dataSource.transaction(async (manager) => {
      for (let i = 0; i < pertanyaanList.length; i++) {
        await manager.update(KpmrPertanyaanLikuiditas, pertanyaanList[i].id, {
          orderIndex: i,
        });
      }
    });
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekLikuiditas, reorderDto.aspekIds[i], {
            orderIndex: i,
          });
        }
      });
    ...
    try {
      for (const update of bulkDto.updates) {
        const pertanyaan = await queryRunner.manager
          .getRepository(KpmrPertanyaanLikuiditas)
          .findOne({
