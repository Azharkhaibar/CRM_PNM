  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let headerRowIndex = rawRows.findIndex((row) => row && row.some((cell) => typeof cell === 'string' && (cell.includes('Risiko') || cell.includes('No'))));
        ...
        headers.forEach((h, idx) => {
          ...
          if (cleanHeader.includes('risiko') && !cleanHeader.includes('sikap') && !cleanHeader.includes('sumber')) colIdx.riskCategory = idx;
          else if (cleanHeader === 'No') colIdx.no = idx;
          else if (cleanHeader.includes('Parameter') || cleanHeader.includes('indikator')) colIdx.parameter = idx;
          ...
          else if (cleanHeader.includes('Realisasi') || ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'].some((m) => cleanHeader.includes(m)))
            colIdx.actualValue = idx;
        });
        ...
        for (let i = headerRowIndex + 1; i < rawRows.length; i++) {
          ...
          const isChild = !row[colIdx.no] && rawParam && (String(rawParam).includes('(Pembilang)') || String(rawParam).includes('(Penyebut)') || String(rawParam).includes('Jumlah'));
          ...
          if (!isChild) {
            ...
            if (currentParent) {
              ...
              const label = String(rawParam)
                .replace(/\(Pembilang\)|\(Penyebut\)/gi, '')
                .trim();
