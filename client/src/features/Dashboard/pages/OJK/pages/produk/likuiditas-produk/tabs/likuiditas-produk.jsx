...
export default function LikuiditasProdukOJK() {
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrLikuiditas();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [LikuiditasProduk] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [LikuiditasProduk] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [LikuiditasProduk] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [LikuiditasProduk] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [LikuiditasProduk] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [LikuiditasProduk] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [LikuiditasProduk] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [LikuiditasProduk] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [LikuiditasProduk] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [LikuiditasProduk] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [LikuiditasProduk] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [LikuiditasProduk] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [LikuiditasProduk] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [LikuiditasProduk] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <LikuiditasProdukKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [LikuiditasProduk] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [LikuiditasProduk] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [LikuiditasProduk] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <LikuiditasProdukInherentWrapper rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
