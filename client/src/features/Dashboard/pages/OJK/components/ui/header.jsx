  ...
  return (
    <div className="w-full bg-gradient-to-r from-blue-700 to-sky-600 text-white p-6 rounded-xl flex items-center justify-between">
      ...
      <div>
        ...
        <span className="text-xs">Silakan isi risk profile untuk OJK</span>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-full max-w-sm">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 text-black/70" size={18} />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="pl-8 bg-white hover:bg-gray-200 text-black" />
        </div>

        {/* YEAR SELECT */}
        <DropdownMenu open={dropdownOpen} onOpenChange={handleDropdownOpenChange}>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="bg-white text-gray-950 w-[150px] justify-between">
              <span>{year}</span>
              <ChevronDown size={16} />
            </Button>
          </DropdownMenuTrigger>
              ...
              {availableYears
                ...
                .map((y) => (
                  <button
                    ...
                    className={`w-full px-2 py-1.5 text-sm flex items-center justify-between hover:bg-gray-100 ${y === year ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-700'}`}
                  >
                    <span>{y}</span>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-[100px] bg-white p-0">
            {quarterOptions.map((q) => (
              <button key={q.value} onClick={() => setActiveQuarter(q.value)} className="w-full px-3 py-2 flex items-center justify-between hover:bg-gray-100 text-gray-900">
                <span>{q.label}</span>
                {activeQuarter === q.value && <Check className="h-4 w-4 text-blue-600" />}
              </button>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        {/* EXPORT */}
        <Button className="bg-black transition-transform hover:scale-110 text-white flex gap-2" onClick={requestExport}>
          <Upload size={16} />
          Export Excel
        </Button>
      </div>
