// src/features/Dashboard/pages/RiskProfile/pages/Operasional/components/OperasionalTabs.jsx
...
import { exportKPMRPasarToExcel as exportKPMROperationalToExcel } from '../../pasar/utils/excelexportpasar.jsx';
import { useOperasional } from '../hooks/operational/operasional.hook.js';
import { exportOperasionalToExcel } from '../components/exportoperationaltoexcel.jsx';
// ===================== Brand =====================
const PNM_BRAND = {
  primary: '#0068B3',
  primarySoft: '#E6F1FA',
...
// Transform functions untuk operational - SESUAI dengan service
...
const transformSectionToBackendOperational = (sectionData, year, quarter) => {
  return {
    no: sectionData.no,
...
const computeHasil = (ind) => {
  ...
  if (formula.trim() !== '') {
    try {
      ...
      console.warn('Invalid formula (Operational):', formula, e);
      return null;
    }
  }
...
// ===================== MAIN COMPONENT =====================
export default function OperasionalTabs() {
  // ====== STATES UTAMA ======
  ...
  const loadData = useCallback(async () => {
    console.log('🔄 [OPERATIONAL] Loading data for:', viewYear, viewQuarter);
    ...
    try {
      ...
      console.log('📥 [OPERATIONAL] Data loaded:', data);

      // Pastikan data ada dan formatnya benar
      if (!data || !Array.isArray(data)) {
        console.warn('[OPERATIONAL] Data tidak valid:', data);
        setSections([]);
      } else {
        setSections(data);
      }
    } catch (err) {
      console.error('❌ [OPERATIONAL] Error loading data:', err);
      setLocalError(err.message || 'Gagal memuat data operational');
      showNotification('error', err.message || 'Gagal memuat data operational');
      setSections([]);
    } finally {
  ...
  const loadAllSections = useCallback(async () => {
    ...
    try {
      console.log('🔄 [OPERATIONAL] Loading all sections...');
      ...
      console.log('✅ [OPERATIONAL] All sections loaded:', transformedSections.length);
    } catch (err) {
      console.error('❌ [OPERATIONAL] Error loading all sections:', err);
  ...
  const getUniqueSections = useMemo(() => {
    ...
    console.log('📊 [OPERATIONAL] Sections for current period:', sectionsForCurrentPeriod.length);
  ...
  const selectSection = (sectionId) => {
    console.log('📝 [OPERATIONAL] Selecting section ID:', sectionId);
    ...
    if (!section) {
      console.log('❌ [OPERATIONAL] Section not found in current period, checking all sections...');

      // Coba cari di allSections
      const sectionFromAll = allSections.find((s) => s.id === sectionId);
      if (sectionFromAll) {
        console.log('✅ [OPERATIONAL] Found in allSections:', sectionFromAll);
        setSectionForm({
        ...
        console.log('❌ [OPERATIONAL] Section not found anywhere');
        showNotification('error', 'Section tidak ditemukan');
      }
      return;
    }

    console.log('✅ [OPERATIONAL] Section found:', section);
    ...
    console.log('✅ [OPERATIONAL] Section form set:', section.parameter);
  };
  ...
  const handleEditIndicator = (indikator) => {
    console.log('📝 [OPERATIONAL] Editing indicator:', indikator);
    ...
    console.log('✅ [OPERATIONAL] Form set for editing:', formData.indikator);
  };
  ...
  const handleCreateSection = async () => {
    ...
    setIsCreating(true);
    try {
      const sectionData = transformSectionToBackendOperational(sectionForm, viewYear, viewQuarter);

      console.log('📤 [OPERATIONAL] Creating section with data:', sectionData);
      ...
      console.error('❌ [OPERATIONAL] Error creating section:', err);
  ...
  const handleUpdateSection = async () => {
    ...
    setIsUpdating(true);
    try {
      const updateData = transformSectionToBackendOperational(sectionForm, viewYear, viewQuarter);

      console.log('📤 [OPERATIONAL] Updating section with data:', updateData);
      ...
      console.error('❌ [OPERATIONAL] Error updating section:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Terjadi kesalahan';
  ...
  const handleDeleteSection = async (sectionId) => {
    ...
    try {
      ...
      console.error('❌ [OPERATIONAL] Error deleting section:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Terjadi kesalahan';
  ...
  const handleCreateIndicator = async () => {
    ...
    try {
      ...
      console.log('📤 [OPERATIONAL] Creating indicator with data:', indicatorData);
      ...
      console.error('❌ [OPERATIONAL] Error creating indicator:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Terjadi kesalahan';
  ...
  const handleUpdateIndicator = async () => {
    ...
    try {
      ...
      console.log('📤 [OPERATIONAL] Updating indicator with data:', updateData);
      ...
      console.error('❌ [OPERATIONAL] Error updating indicator:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Terjadi kesalahan';
  ...
  const handleDeleteIndicator = async (indikatorId) => {
    ...
    try {
      ...
      console.error('❌ [OPERATIONAL] Error deleting indicator:', err);
  ...
  const handleExportOperational = () => {
    exportOperasionalToExcel({
      year: viewYear,
      quarter: viewQuarter,
      sections: sections,
      filename: `Operasional_${viewYear}_${viewQuarter}`,
    });
  };
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl">
        ...
        <header className="flex items-center justify-between gap-3 px-4 py-4">
          <h2 className="text-xl sm:text-2xl font-semibold">Form – Operational</h2>
          <div className="flex items-end gap-4">
            ...
            <div className="flex items-end gap-2">
              ...
              <button onClick={handleExportOperational} disabled={loadingData || sections.length === 0} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black disabled:opacity-50">
                <Download size={18} />
        ...
        <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
          ...
          <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
            <h2 className="text-white font-semibold text-lg sm:text-xl mb-6">Form Section – Operational</h2>
            ...
            <div className="flex items-end gap-4 mb-4">
              ...
              <div className="flex-1 flex flex-col">
                ...
                <input
                  ...
                  placeholder="Uraian section risiko operational"
            ...
            <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
              ...
              <div className="grid grid-cols-12 gap-4 mb-4">
                <div className="col-span-2">
                  ...
                  <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={indicatorForm.subNo} onChange={(e) => setIndicatorField('subNo', e.target.value)} placeholder="1.1.1" />
                </div>
                <div className="col-span-7">
                  ...
                  <input
                    ...
                    placeholder="Teks indikator risiko operational…"
                  />
                </div>
                  ...
                  {indicatorForm.mode === CalculationMode.TEKS ? (
                    <>
                      ...
                      <input
                        ...
                        placeholder="misal: sesuai standar operasional"
                      />
                    </>
                  ) : (
                    <>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Actual, pencapaian real, dll."
                    />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Target, standar yang ditetapkan, dll."
                    />
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Contoh: prosedur operasional yang tidak sesuai, dll."
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Contoh: gangguan proses bisnis, penurunan produktivitas, dll."
                  />
                </div>
              </div>
