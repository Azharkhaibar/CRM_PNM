// ===================== Export Function for Strategik Table =====================
...
const transformIndicatorToFrontendStrategik = (indikator = {}) => ({
  id: indikator.id,
...
export const exportStrategikToExcel = async ({ year, quarter, sections = [], totalWeighted = 0, filename = `Strategik_${year}_${quarter}` }) => {
  try {
    // Validasi data
    ...
    // ===================== PALET WARNA SESUAI FILE EXCEL =====================
    const COLORS = {
      // ===== HEADER UTAMA =====
      HEADER: '4472C4', // Biru Excel untuk header utama
    ...
    sections.forEach((section) => {
      ...
      indicators.forEach((indicator, idx) => {
        ...
        const transformed = transformIndicatorToFrontendStrategik(indicator);
        ...
        calculatedTotalWeighted += weightedVal;
        ...
        if (transformed.mode !== CalculationMode.TEKS) {
          ...
          wsData.push([
            ...
            '',
            '',
            transformed.penyebutValue ? fmtNumber(transformed.penyebutValue) : '', // ⬅ NILAI tepat di bawah HASIL
            '', // Peringkat
            '', // Weighted
            '', // Aksi
          ]);
          ...
          if (transformed.mode === CalculationMode.RASIO) {
            wsData.push([
              ...
              '',
              '',
              transformed.pembilangValue ? fmtNumber(transformed.pembilangValue) : '', // ⬅ NILAI tepat di bawah HASIL
              '',
              '',
              '',
            ]);
    ...
    // Gunakan totalWeighted yang dihitung jika tidak ada parameter
    const finalTotalWeighted = totalWeighted || calculatedTotalWeighted;

    // ===== FOOTER =====
    wsData.push(['', '', '', '', '', '', '', '', '', '', '', '', 'Summary', '', finalTotalWeighted.toFixed(2), '']);
    ...
    sections.forEach((section) => {
      ...
      indicators.forEach((indicator, idx) => {
        ...
        const hasilPeringkatStyles = {
          ...
          14: COLORS.HASIL, // O - Peringkat (akan diubah sesuai nilai)
          15: COLORS.HASIL, // P - Weighted (Abu-abu)
        };
        ...
        Object.entries(hasilPeringkatStyles).forEach(([col, color]) => {
          ...
          // Warna khusus untuk kolom Peringkat berdasarkan nilai
          if (colNum === 14) {
            // Kolom O - Peringkat
            let ratingColor = COLORS.HASIL; // Default abu-abu
            let textColor = '000000'; // Default hitam

            // Tentukan warna berdasarkan nilai peringkat - SESUAI HEADER RISK LEVEL
            if (peringkat === 1) {
              ratingColor = COLORS.HEADER_LOW; // Hijau muda (C6E0B4)
            } else if (peringkat === 2) {
              ratingColor = COLORS.HEADER_LOW_TO_MODERATE; // Biru muda (BDD7EE)
            } else if (peringkat === 3) {
              ratingColor = COLORS.HEADER_MODERATE; // Kuning pastel (FFE699)
            } else if (peringkat === 4) {
              ratingColor = COLORS.HEADER_MODERATE_TO_HIGH; // Oranye kemerahan (F4B084)
              textColor = '000000'; // Hitam
            } else if (peringkat >= 5) {
              ratingColor = COLORS.HEADER_HIGH; // Merah tua (C00000)
              textColor = 'FFFFFF'; // Putih untuk kontras dengan merah tua
            }
    ...
    // ===================== SHEET RINGKASAN =====================
    const summaryData = [['LAPORAN RISIKO STRATEGIK BANK'], [`Periode: ${year} ${quarter}`], [''], ['No', 'Section', 'Bobot Section', 'Jumlah Indikator', 'Total Weighted']];
    ...
    // Total keseluruhan
    summaryData.push(['', '', 'TOTAL:', '', finalTotalWeighted.toFixed(2)]);
    ...
    // ===== SIMPAN KE FILE =====
    XLSX.utils.book_append_sheet(wb, ws, 'Data Strategik');
