...
const PASAR_SECTIONS_LS = "pasar_sections_v2";
const PASAR_SECTIONS_LS_V1 = "pasar_sections_v1"; // Backup key for migration
const KPMR_PASAR_LS = "kpmr_pasar_rows_v1";
const KPMR_ASPEK_LS = "kpmr_pasar_aspek_v1";
const KPMR_PERTANYAAN_LS = "kpmr_pasar_pertanyaan_v1";
const KPMR_TEMPLATE_LS = "kpmr_pasar_template_v1";

// Year-Level Definitions: Shared across ALL quarters within the same year
const KPMR_DEFINITIONS_LS = "kpmr_pasar_definitions_v1";

// Quarter-Level Scores: sectionSkor per quarter, linked by definitionId
const KPMR_SCORES_LS = "kpmr_pasar_scores_v1";
...
const KPMR_EMPTY_FORM = {
  ...
  // Aspek
  aspekNo: "Aspek 1",
  aspekTitle: "Tata Kelola Risiko Pasar",
  aspekBobot: 30,
...
const loadPasarRows = () => {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem("pasarRows");
    return raw ? JSON.parse(raw) : [];
  } catch (e) {
    console.warn("Gagal parse pasarRows dari localStorage", e);
    return [];
  }
};
...
const makeRowKey = (r) => {
  const key = `${r.source || "PASAR"}|${r.year}|${r.quarter}|${r.no ?? ""}|${r.subNo ?? ""}|${r.sectionLabel ?? ""}|${r.indikator ?? ""}`;
  return key;
};
...
export default function Pasar() {
  ...
  // ----- SECTION LIST -----
  const [PASAR_sections, setPASAR_sections] = useState(() => {
    if (typeof window === "undefined") return [];
    try {
      // Try loading v2 first
      const rawV2 = localStorage.getItem(PASAR_SECTIONS_LS);
      if (rawV2) {
        return JSON.parse(rawV2);
      }

      // Try migrating from v1
      const rawV1 = localStorage.getItem(PASAR_SECTIONS_LS_V1);
      if (rawV1) {
        ...
        localStorage.setItem(PASAR_SECTIONS_LS_V1 + "_backup", rawV1);
        localStorage.setItem(PASAR_SECTIONS_LS, JSON.stringify(v2Sections));
        return v2Sections;
      }
  ...
  const currentPeriodSections = useMemo(() => {
    return getSectionsForPeriod(viewYear, viewQuarter, PASAR_sections);
  }, [viewYear, viewQuarter, PASAR_sections]);
  ...
  function PASAR_addSection() {
    ...
    setPASAR_sections((prev) => {
      const newSections = addSectionToPeriod(viewYear, viewQuarter, sectionData, prev);
      localStorage.setItem(PASAR_SECTIONS_LS, JSON.stringify(newSections));
  ...
  function PASAR_removeSection(id) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        PASAR_SECTIONS_LS,
        JSON.stringify(PASAR_sections)
      );
    } catch (e) {
      console.warn("Gagal simpan pasar sections", e);
    }
  }, [PASAR_sections]);

  // ===== AUTO-CLONE SECTIONS ON PERIOD CHANGE =====
  // Hanya clone jika quarter KOSONG (belum ada direct sections)
  useEffect(() => {
    if (typeof window === "undefined") return;

    const updatedSections = autoCloneSectionsIfNeeded(viewYear, viewQuarter, PASAR_sections);

    // Update jika ada perubahan
    if (updatedSections !== PASAR_sections) {
      setPASAR_sections(updatedSections);
      localStorage.setItem(PASAR_SECTIONS_LS, JSON.stringify(updatedSections));
    }
  ...
  // 1) semua baris indikator pasar (multi tahun & quarter)
  const [PASAR_rows, setPASAR_rows] = useState(loadPasarRows);
  ...
  useEffect(() => {
    try {
      latestRowsStrRef.current = JSON.stringify(PASAR_rows);
    } catch {
  ...
  // ============================================================
  useEffect(() => {
    if (!PASAR_rows?.length || isProcessingWeighted.current) return;
    
    let hasChanges = false;
    
    const updatedRows = PASAR_rows.map(row => {
    ...
    if (hasChanges) {
      ...
      setPASAR_rows(updatedRows);
      // Reset flag setelah state update
      setTimeout(() => {
        isProcessingWeighted.current = false;
      }, 0);
    }
  }, [PASAR_rows.map(r => `${r.no}-${r.subNo}-${r.peringkat}-${r.bobotIndikator}`).join(',')]);

  // --- sinkronkan semua baris ke localStorage (supaya tidak hilang) ---
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      localStorage.setItem("pasarRows", JSON.stringify(PASAR_rows));

      // Notify RekapData/Rekap1 without re-triggering Pasar sync
      window.dispatchEvent(
        new CustomEvent("pasarRows:changed", { detail: { origin: "PASAR" } })
      );
    } catch (e) {
      console.warn("Gagal simpan pasarRows ke localStorage", e);
    }
  }, [PASAR_rows]);

  // --- bentuk data ringkas untuk RekapData & simpan ke localStorage ---
  // ============================================================
  // SIMPAN DATA RINGKAS UNTUK REKAP1.JSX (PASAR)
  // ============================================================
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Map ALL data from PASAR_rows, not just filtered by viewYear/viewQuarter
    // This ensures we save data for ALL periods, not just the current view
    const rekapRows = PASAR_rows.map((r, idx) => ({
      // id stabil
      id: makeRowKey(r),
    ...
    try {
      // Save all data
      localStorage.setItem("rekap_pasar", JSON.stringify(uniqueRows));
    } catch (e) {
      console.warn("Gagal simpan rekap_pasar", e);
    }
  }, [PASAR_rows]);
  ...
  useEffect(() => {
    ...
    // Check if rekap_pasar already exists and is up to date
    const existingRekap = localStorage.getItem("rekap_pasar");
    const existingRekapData = existingRekap ? JSON.parse(existingRekap) : [];

    // Get all unique periods from PASAR_rows
    const periodsInRows = new Set(PASAR_rows.map(r => `${r.year}|${r.quarter}`));
    ...
    if (needsSync && PASAR_rows.length > 0) {
      // Re-use the same mapping logic
      const rekapRows = PASAR_rows.map((r) => ({
        id: makeRowKey(r),
        year: r.year,
        quarter: r.quarter,
        riskFormId: "pasar",
        ...
        formula: r.formula || "",
        isPercent: !!r.isPercent,
        hasil: r.mode === "TEKS" ? "" : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computePasarHasil(r)),
        hasilText: r.mode === "TEKS" ? (r.hasilText || "") : "",
  ...
  useEffect(() => {
    const handleRekapDataChange = (e) => {
      ...
      try {
        ...
        const repaired = parsed.map((row) => {
          try {
            ...
            const computedHasil = (r.mode === "TEKS")
              ? ""
              : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computePasarHasil(r));
            ...
            if ((r.mode || "RASIO") === "TEKS") {
              ...
              if (hasilForPeringkat === "" || hasilForPeringkat == null || !Number.isFinite(hasilForPeringkat)) {
                const fallback = computePasarHasil(r);
        ...
        setPASAR_rows(repaired);
      } catch (err) {
        console.warn("[PASAR] gagal reload rekap data:", err);
      } finally {
        setTimeout(() => {
    ...
    window.addEventListener("pasarRows:changed", handleRekapDataChange);
    return () => window.removeEventListener("pasarRows:changed", handleRekapDataChange);
  }, []);

  // ============================================================
  // ONE-TIME RECALC ON MOUNT
  // Ensures imported Pasar rows get peringkat/weighted updated
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (newRowsStr && newRowsStr !== currentRowsStr) {
        setPASAR_rows(repaired);
        try {
          localStorage.setItem("pasarRows", JSON.stringify(repaired));
        } catch (e) {
          console.warn("[PASAR] Failed to save repaired rows on mount", e);
        }
      }
    } catch {
      // silent
  ...
  }, [PASAR_rows, viewYear, viewQuarter, query]);

  const PASAR_totalWeighted = useMemo(
    () =>
      PASAR_filtered.reduce(
  ...
  const buildBaseRow = () => {
    const peringkatNum =
      PASAR_form.peringkat === "" || PASAR_form.peringkat == null
        ? 0
        : Number(PASAR_form.peringkat);
    ...
    return {
      ...
      numeratorLabel:
        PASAR_form.mode === "NILAI_TUNGGAL" || PASAR_form.mode === "TEKS"
          ? ""
          : PASAR_form.numeratorLabel,
      numeratorValue:
        PASAR_form.mode === "NILAI_TUNGGAL" || PASAR_form.mode === "TEKS"
          ? ""
          : PASAR_form.numeratorValue,
  ...
  const PASAR_addRow = () => {
    ...
    const newRowKey = makeRowKey({ ...newRow, source: "PASAR" });

    setPASAR_rows((prev) => {
      const isDuplicate = prev.some(r =>
        makeRowKey({ ...r, source: "PASAR" }) === newRowKey
      );

      if (isDuplicate) {
        console.warn('[PASAR_addRow] DUPLICATE DETECTED! Row already exists:', newRowKey);
    ...
    // ✅ Hapus baris duplikat setPASAR_rows
    PASAR_resetForm();
      ...
      prev.map((r) =>
        r === PASAR_editingRow
          ? {
            ...
            year: PASAR_editingRow.year,
            quarter: PASAR_editingRow.quarter,
            hasil: baseRow.mode === "TEKS" ? "" : (rawHasil === "" || rawHasil == null ? 0 : rawHasil),
            hasilText: baseRow.mode === "TEKS" ? (baseRow.hasilText || "") : "",
  ...
  const PASAR_removeRow = (row) => {
    if (!row) return;
    setPASAR_rows((arr) => arr.filter((r) => r !== row));
    if (PASAR_editingRow === row) PASAR_resetForm();
  };

  const PASAR_exportExcel = () =>
    exportInvestasiToExcel
      ? exportInvestasiToExcel(PASAR_filtered, viewYear, viewQuarter)
      : null;
  ...
  const loadKPMRRows = () => {
    if (typeof window === "undefined") return [];
    try {
      const raw = localStorage.getItem(KPMR_PASAR_LS);
      return raw ? JSON.parse(raw) : [];
    } catch (e) {
      console.warn("Gagal parse KPMR Pasar dari localStorage", e);
      return [];
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('pasarKPMR:changed', {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Gagal simpan KPMR Pasar ke localStorage", e);
    }
  }, [KPMR_rows]);
              ...
              <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
                ...
                <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() => setShowPasarForm(false)}
                      ...
                      {!isAddingNewSection && !isEditingSection && (
                        <>
                          ...
                          <button
                            ...
                            onClick={() =>
                              PASAR_sectionForm.id &&
                              PASAR_removeSection(PASAR_sectionForm.id)
                            }
                            title="Hapus Section"
                          >
                            <Trash2 size={20} />
                  ...
                  <div className="grid grid-cols-12 gap-4 mb-4">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        value={PASAR_form.subNo}
                        onChange={(e) =>
                          setPASAR_form((f) => ({ ...f, subNo: e.target.value }))
                        }
                        placeholder="1.1"
                      />
                    </div>
                      ...
                      <div className="col-span-6 space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setPASAR_form((f) => ({ ...f, dampak: e.target.value }))
                            }
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={(() => {
                                const peringkatNum =
                                  PASAR_form.peringkat === "" ||
                                    PASAR_form.peringkat == null
                                    ? 0
                                    : Number(PASAR_form.peringkat);
                          ...
                          <div>
                            ...
                            <input
                              className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                              value={(() => {
                                if (PASAR_form.mode === "TEKS") {
                                  return PASAR_form.hasilText || "";
                                }
                                ...
                                if (PASAR_form.mode === "NILAI_TUNGGAL") {
              ...
              <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
                ...
                <div className="relative h-[350px]">
                  <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
                    <DataTable
                      rows={PASAR_filtered}
                      totalWeighted={PASAR_totalWeighted}
                      viewYear={viewYear}
                      viewQuarter={viewQuarter}
                      startEdit={PASAR_startEdit}
                      removeRow={PASAR_removeRow}
                    />
                  </div>
                </div>
