...
let KpmrHukumService = KpmrHukumService_1 = class KpmrHukumService {
    kpmrRepository;
    aspekRepository;
    pertanyaanRepository;
    dataSource;
    logger = new common_1.Logger(KpmrHukumService_1.name);
    MAX_BOBOT_TOTAL = 100;
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(hukum_kpmr_aspek_entity_1.KpmrAspekHukum, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(hukum_kpmr_pertanyaan_entity_1.KpmrPertanyaanHukum, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(hukum_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(hukum_kpmr_dto_1.FrontendAspekResponseDto, {
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(hukum_kpmr_aspek_entity_1.KpmrAspekHukum, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(hukum_kpmr_dto_1.FrontendPertanyaanResponseDto, {
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(hukum_kpmr_pertanyaan_entity_1.KpmrPertanyaanHukum)
                    .findOne({
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(hukum_kpmr_pertanyaan_entity_1.KpmrPertanyaanHukum, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(hukum_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrHukumService = KpmrHukumService = KpmrHukumService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(hukum_kpmr_ojk_entity_1.KpmrHukum)),
    __param(1, (0, typeorm_1.InjectRepository)(hukum_kpmr_aspek_entity_1.KpmrAspekHukum)),
    __param(2, (0, typeorm_1.InjectRepository)(hukum_kpmr_pertanyaan_entity_1.KpmrPertanyaanHukum)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KpmrHukumService);
//# sourceMappingURL=hukum-kpmr-ojk.service.js.map