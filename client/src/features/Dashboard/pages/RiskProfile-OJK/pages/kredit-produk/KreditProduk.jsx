...
// ==================== FUNGSI EXPORT INHERENT KE EXCEL (TANPA % DALAM PORTOFOLIO) ====================
async function exportInherentToExcel({
  ...
  try {
    ...
    // Tambahkan worksheet
    const worksheet = workbook.addWorksheet(`3. Inherent ${categoryLabel}`, {
...
async function exportKpmrToExcel({
  ...
  try {
    ...
    // Tambahkan worksheet
    const worksheet = workbook.addWorksheet(`3. KPMR ${categoryLabel}`, {
      views: [{ showGridLines: true }],
    });
...
export default function KreditProduk() {
  ...
  // Handler untuk ekspor sesuai tab aktif
  ...
  // Handle export request dari header (via store)
  useEffect(() => {
  ...
  return (
    <div className="w-full space-y-4">
      <Header title="Risk Profile – Kredit Produk" onExportClick={handleExportToExcel} showQuarterActions={true}/>
