...
export default function OperasionalOJK() {
  ...
  const CATEGORY_ID = 'operasional-produk';
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrOperasional();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [OperasionalProduk] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [OperasionalProduk] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [OperasionalProduk] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [OperasionalProduk] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [OperasionalProduk] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [OperasionalProduk] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [OperasionalProduk] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [OperasionalProduk] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [OperasionalProduk] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [OperasionalProduk] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [OperasionalProduk] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [OperasionalProduk] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [OperasionalProduk] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [OperasionalProduk] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <OperasionalProdukKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [OperasionalProduk] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [OperasionalProduk] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [OperasionalProduk] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'inherent') {
      exportInherent({
        ...
        year,
        quarter: activeQuarter,
        categoryLabel: 'OperasionalProduk',
      });
    }
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <OperasionalProdukInherentWrapper rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
