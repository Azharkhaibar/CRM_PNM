// src/ojk/rentabilitas-produk/rentabilitas-produk-ojk/rentabilitas-produk-ojk.service.tsx
...
import useRentabilitasInherent from '../hooks/inherent/rentabilitas.hook';
...
export default function RentabilitasInherentWrapper() {
  ...
  // PERBAIKAN: Gunakan useRentabilitasInherent, bukan useRentabilitasIntegration
  const { rentabilitas, parameters, loading, saving, error, currentRentabilitasId, loadByYearQuarter, refreshData, addParameter, updateParameter, deleteParameter, copyParameter, addNilai, updateNilai, deleteNilai, copyNilai } =
    useRentabilitasInherent();
  ...
  // PERBAIKAN: Data rentabilitas untuk info
  const currentRentabilitasData = rentabilitas;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      currentRentabilitasData: currentRentabilitasData ? `ID: ${currentRentabilitasData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentRentabilitasId, currentRentabilitasData, error]);
  ...
  const backendHandlers = useMemo(
    () => ({
      addAspek: async (createAspekDto) => {
        try {
          ...
          if (!currentRentabilitasId) {
            throw new Error('Tidak ada data rentabilitas yang dipilih');
          }
          ...
          // Panggil API
          await addParameter(currentRentabilitasId, createAspekDto);
      ...
      addPertanyaan: async (aspekId, createPertanyaanDto) => {
        try {
          ...
          await addNilai(currentRentabilitasId, aspekId, createPertanyaanDto);
          await handleRefresh();
      ...
      copyAspek: async (aspekId) => {
        try {
          ...
          await copyParameter(currentRentabilitasId, aspekId);
          await handleRefresh();
      ...
      copyPertanyaan: async (aspekId, pertanyaanId) => {
        try {
          ...
          await copyNilai(currentRentabilitasId, aspekId, pertanyaanId);
          await handleRefresh();
    ...
    [currentRentabilitasId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Rentabilitas - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentRentabilitasId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentRentabilitasId).substring(0, 8)}...</span>}
            </p>
            {rentabilitas?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(rentabilitas.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {rentabilitas?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{rentabilitas.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <RentabilitasInherent rows={safeRows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={loading || saving || isRefreshing} isLocked={rentabilitas?.isLocked || false} />
    </div>
  );
}

// Komponen utama
function RentabilitasInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Rentabilitas - Inherent</h1>
