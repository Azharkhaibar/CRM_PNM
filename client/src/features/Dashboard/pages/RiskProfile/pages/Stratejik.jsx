...
// ==== Utils Export KPMR ====
import { exportKPMRStratejikToExcel } from "../utils/exportExcelKPMR";
...
const STRATEJIK_SECTIONS_LS_V1 = "stratejik_sections_v1"; // Backup key for migration
const KPMR_STRATEJIK_LS = "kpmr_stratejik_rows_v1";
const KPMR_ASPEK_LS = "kpmr_stratejik_aspek_v1";
const KPMR_PERTANYAAN_LS = "kpmr_stratejik_pertanyaan_v1";
const KPMR_TEMPLATE_LS = "kpmr_stratejik_template_v1";

// Year-Level Definitions: Shared across ALL quarters within the same year
const KPMR_DEFINITIONS_LS = "kpmr_stratejik_definitions_v1";

// Quarter-Level Scores: sectionSkor per quarter, linked by definitionId
const KPMR_SCORES_LS = "kpmr_stratejik_scores_v1";
...
const stratejikFallbackEmpty = (year, quarter) => ({
  year,
  quarter,
  no: "1",
  subNo: "1.1",
...
const loadStratejikRows = () => {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem("stratejikRows");
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    // Recalculate weighted values to fix old data
    return parsed.map(row => ({
      ...row,
      weighted: computeWeightedLocal(row.bobotSection, row.bobotIndikator, row.peringkat)
    }));
  } catch (e) {
    console.warn("Gagal parse stratejikRows dari localStorage", e);
    return [];
  }
};
...
const makeRowKey = (r) => {
  const key = `${r.source || "STRATEJIK"}|${r.year}|${r.quarter}|${r.no ?? ""}|${r.subNo ?? ""}|${r.sectionLabel ?? ""}|${r.indikator ?? ""}`;
  return key;
};
...
export default function Stratejik() {
  ...
  // ----- SECTION LIST -----
  const [STRATEJIK_sections, setSTRATEJIK_sections] = useState(() => {
    if (typeof window === "undefined") return [];
    try {
      // Try loading v2 first
      const rawV2 = localStorage.getItem(STRATEJIK_SECTIONS_LS);
      if (rawV2) {
        return JSON.parse(rawV2);
      }

      // Try migrating from v1
      const rawV1 = localStorage.getItem(STRATEJIK_SECTIONS_LS_V1);
      if (rawV1) {
        ...
        localStorage.setItem(STRATEJIK_SECTIONS_LS_V1 + "_backup", rawV1);
        localStorage.setItem(STRATEJIK_SECTIONS_LS, JSON.stringify(v2Sections));
        return v2Sections;
      }

      return [];
    } catch (e) {
      console.warn('[STRATEJIK] Failed to load sections:', e);
      return [];
    }
  });
  ...
  // Get sections for current viewing period (with inheritance)
  const currentPeriodSections = useMemo(() => {
    return getSectionsForPeriod(viewYear, viewQuarter, STRATEJIK_sections);
  }, [viewYear, viewQuarter, STRATEJIK_sections]);
  ...
  function STRATEJIK_addSection() {
    ...
    setSTRATEJIK_sections((prev) => {
      const newSections = addSectionToPeriod(viewYear, viewQuarter, sectionData, prev);
      localStorage.setItem(STRATEJIK_SECTIONS_LS, JSON.stringify(newSections));
  ...
  function STRATEJIK_removeSection(id) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        STRATEJIK_SECTIONS_LS,
        JSON.stringify(STRATEJIK_sections)
      );
    } catch (e) {
      console.warn("Gagal simpan stratejik sections", e);
    }
  }, [STRATEJIK_sections]);
  ...
  useEffect(() => {
    ...
    const updatedSections = autoCloneSectionsIfNeeded(viewYear, viewQuarter, STRATEJIK_sections);

    // Hanya update jika ada perubahan (clone terjadi)
    if (updatedSections.length !== STRATEJIK_sections.length) {
      setSTRATEJIK_sections(updatedSections);
      localStorage.setItem(STRATEJIK_SECTIONS_LS, JSON.stringify(updatedSections));
    }
  ...
  const handleUndoInheritance = () => {
    ...
    setSTRATEJIK_rows(cleaned);
    setInheritInfo(null);
  };


  // ----- ROW & FORM -----
  const stratejikMakeRow = () =>
    typeof makeEmptyRow === "function"
      ...
      : stratejikFallbackEmpty(viewYear, viewQuarter);

  // 1) semua baris indikator stratejik (multi tahun & quarter)
  const [STRATEJIK_rows, setSTRATEJIK_rows] = useState(loadStratejikRows);

  // 2) form indikator yang sedang diisi
  const [STRATEJIK_form, setSTRATEJIK_form] = useState(stratejikMakeRow());
  ...
  useEffect(() => {
    try {
      latestRowsStrRef.current = JSON.stringify(STRATEJIK_rows);
    } catch {
  ...
  useEffect(() => {
    const result = cloneFromPreviousPeriod({
      ...
      sourceLabel: "STRATEJIK",
    });

    if (result.cloned) {
      setSTRATEJIK_rows(result.rows);
      setInheritInfo({
        from: result.from,
        count: result.count,
      });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isNumericRiskLevels(riskLevels)) {
        ...
        if (!isNaN(hasilNum)) {
          // ✅ FIXED: TEKS mode, pass mode='TEKS' agar tidak dikalikan 100 lagi
  ...
  // ============================================================
  useEffect(() => {
    if (!STRATEJIK_rows?.length || isProcessingWeighted.current) return;
    
    let hasChanges = false;
    
    const updatedRows = STRATEJIK_rows.map(row => {
    ...
    if (hasChanges) {
      ...
      setSTRATEJIK_rows(updatedRows);
      // Reset flag setelah state update
      setTimeout(() => {
        isProcessingWeighted.current = false;
      }, 0);
    }
  }, [STRATEJIK_rows.map(r => `${r.no}-${r.subNo}-${r.peringkat}-${r.bobotIndikator}`).join(',')]);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("stratejikRows", JSON.stringify(STRATEJIK_rows));

      // Notify RekapData/Rekap1 without re-triggering Stratejik sync
      window.dispatchEvent(
        new CustomEvent("stratejikRows:changed", { detail: { origin: "STRATEJIK" } })
      );
    } catch (e) {
      console.warn("Gagal simpan stratejikRows ke localStorage", e);
    }
  }, [STRATEJIK_rows]);

  // --- bentuk data ringkas untuk RekapData & simpan ke localStorage ---
  // ============================================================
  // SIMPAN DATA RINGKAS UNTUK REKAP1.JSX (STRATEJIK)
  // ============================================================
  useEffect(() => {
    if (typeof window === "undefined") return;

    // Map ALL data from STRATEJIK_rows, not just filtered by viewYear/viewQuarter
    // This ensures we save data for ALL periods, not just the current view
    const rekapRows = STRATEJIK_rows.map((r, idx) => ({
      // id stabil
      id: makeRowKey(r),
      ...
      // identitas risk form
      riskFormId: "stratejik",
      riskFormLabel: "Stratejik",
      ...
      weighted:
        r.weighted !== "" && r.weighted != null
          ? Number(Number(r.weighted).toFixed(2)) // ROUND to 2 decimals
          : 0,
    ...
    try {
      // Save all data
      localStorage.setItem("rekap_stratejik", JSON.stringify(uniqueRows));
    } catch (e) {
      console.warn("Gagal simpan rekap_stratejik", e);
    }
  }, [STRATEJIK_rows]);
  ...
  useEffect(() => {
    ...
    // Check if rekap_stratejik already exists and is up to date
    const existingRekap = localStorage.getItem("rekap_stratejik");
    const existingRekapData = existingRekap ? JSON.parse(existingRekap) : [];

    // Get all unique periods from STRATEJIK_rows
    const periodsInRows = new Set(STRATEJIK_rows.map(r => `${r.year}|${r.quarter}`));
    ...
    if (needsSync && STRATEJIK_rows.length > 0) {
      // Re-use the same mapping logic
      const rekapRows = STRATEJIK_rows.map((r) => ({
        id: makeRowKey(r),
        year: r.year,
        quarter: r.quarter,
        riskFormId: "stratejik",
        ...
        formula: r.formula || "",
        isPercent: !!r.isPercent,
        hasil: r.mode === "TEKS" ? "" : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeStratejikHasil(r)),
        hasilText: r.mode === "TEKS" ? (r.hasilText || "") : "",
  ...
  useEffect(() => {
    const handleRowsChange = (e) => {
      ...
      try {
        ...
        const repaired = parsed.map((row) => {
          try {
            ...
            const computedHasil = (r.mode === "TEKS")
              ? ""
              : (r.hasil !== "" && r.hasil != null ? Number(r.hasil) : computeStratejikHasil(r));

            const hasilRaw = computedHasil === "" || computedHasil == null ? "" : Number(computedHasil);

            // ✅ FIX: Untuk persentase, gunakan nilai desimal asli (1.52 untuk 152%)
            // calculatePeringkat akan mengkalikan dengan 100 secara otomatis
            ...
            if ((r.mode || "RASIO") === "TEKS") {
              if (isNumericRiskLevels(riskLevels)) {
                ...
                if (!Number.isNaN(parsedNum)) {
                  // ✅ FIX: Untuk TEKS mode, pass mode='TEKS' agar tidak dikalikan 100 lagi
                  newPeringkat = calculatePeringkat(parsedNum, riskLevels, inferredIsPercent, 'TEKS');
                } else {
              ...
              if (hasilForPeringkat === "" || hasilForPeringkat == null || !Number.isFinite(hasilForPeringkat)) {
                const fallback = computeStratejikHasil(r);
                if (fallback !== "" && fallback != null && Number.isFinite(fallback)) {
                  // ✅ FIX: Mode RASIO/NILAI_TUNGGAL
                  newPeringkat = calculatePeringkat(Number(fallback), riskLevels, inferredIsPercent, r.mode || 'RASIO');
                } else {
                  newPeringkat = 0;
                }
              } else {
                // ✅ FIX: Mode RASIO/NILAI_TUNGGAL
                newPeringkat = calculatePeringkat(hasilForPeringkat, riskLevels, inferredIsPercent, r.mode || 'RASIO');
              }
            }
            ...
            r.weighted = Number.isFinite(Number(recalcWeighted))
              ? Number(Number(recalcWeighted).toFixed(2))
              : 0;
        ...
        // 🔍 DEBUG: Log sample data untuk melihat apa yang terjadi
        if (repaired.length > 0) {
          const sample = repaired[0];
          console.log('%c=== STRATEJIK IMPORT REKAPDATA DEBUG ===', 'color: #F59E0B; font-weight: bold');
          console.log('📊 Sample row dari localStorage setelah import:', {
            indikator: sample.indikator,
            hasil: sample.hasil,
            peringkat: sample.peringkat,
        ...
        try {
          ...
          console.log('📋 Comparison check:', {
            newRowsLength: newRowsStr.length,
            currentRowsLength: currentRowsStr?.length || 0,
            areEqual: newRowsStr === currentRowsStr,
          });

          if (currentRowsStr && newRowsStr === currentRowsStr) {
            console.warn('%c⚠️ SKIP UPDATE: Data sama dengan reference', 'color: #EF4444; font-weight: bold');
            return;
          }
          console.log('%c✅ PROCEED UPDATE: Data berbeda, akan update state', 'color: #10B981; font-weight: bold');
        } catch {
          // ignore
        }

        setSTRATEJIK_rows(repaired);

        // ✅ Update reference SEGERA setelah state update
        // untuk mencegah stale reference pada event listener berikutnya
        try {
          latestRowsStrRef.current = JSON.stringify(repaired);
          console.log('🔄 Reference updated');
        } catch {
          latestRowsStrRef.current = "";
        }
      } catch (err) {
        console.warn("[STRATEJIK] gagal reload rekap data:", err);
      } finally {
        setTimeout(() => {
    ...
    const handleSectionsChange = (e) => {
      ...
      if (raw) {
        try {
          setSTRATEJIK_sections(JSON.parse(raw));
        } catch (e) {
          console.warn('[STRATEJIK] Failed to parse sections from localStorage', e);
        }
      }
    };

    window.addEventListener("stratejikRows:changed", handleRowsChange);
    window.addEventListener("stratejikSections:changed", handleSectionsChange);
    return () => {
      window.removeEventListener("stratejikRows:changed", handleRowsChange);
      window.removeEventListener("stratejikSections:changed", handleSectionsChange);
    };
  }, []);

  // ============================================================
  // ONE-TIME RECALC ON MOUNT
  // Ensures imported Stratejik rows get peringkat/weighted updated
    ...
    try {
      ...
      const repaired = parsed.map((row) => {
        try {
          ...
          // ✅ FIXED: Jangan bagi 100 - biarkan calculatePeringkat handle konversi
          // calculatePeringkat akan mengkalikan hasil dengan 100 jika risk levels menggunakan %
          ...
          if ((r.mode || "RASIO") === "TEKS") {
            ...
            if (hasilForPeringkat === "" || hasilForPeringkat == null || !Number.isFinite(hasilForPeringkat)) {
              const fallback = computeStratejikHasil(r);
              if (fallback !== "" && fallback != null && Number.isFinite(fallback)) {
                // ✅ FIXED: Mode RASIO/NILAI_TUNGGAL
                newPeringkat = calculatePeringkat(Number(fallback), riskLevels, inferredIsPercent, r.mode || 'RASIO');
              } else {
                newPeringkat = 0;
              }
      ...
      // 🔍 DEBUG: Log one-time recalc
      console.log('%c=== STRATEJIK ONE-TIME RECALC ===', 'color: #8B5CF6; font-weight: bold');
      console.log('📊 Comparison:', {
        newRowsLength: newRowsStr.length,
        currentRowsLength: currentRowsStr.length,
        willUpdate: newRowsStr && newRowsStr !== currentRowsStr,
      });
      if (repaired.length > 0) {
        console.log('📊 Sample repaired row:', {
          indikator: repaired[0].indikator,
          hasil: repaired[0].hasil,
          peringkat: repaired[0].peringkat,
          weighted: repaired[0].weighted,
        });
      }

      if (newRowsStr && newRowsStr !== currentRowsStr) {
        console.log('%c✅ ONE-TIME RECALC: Updating state and localStorage', 'color: #10B981; font-weight: bold');
        setSTRATEJIK_rows(repaired);
        try {
          localStorage.setItem("stratejikRows", JSON.stringify(repaired));
        } catch (e) {
          console.warn("[STRATEJIK] Failed to save repaired rows on mount", e);
        }
      } else {
        console.log('%c⚠️ ONE-TIME RECALC: Skipping update (data unchanged)', 'color: #F59E0B; font-weight: bold');
      }
    } catch {
      // silent
    }
  ...
  }, [STRATEJIK_rows, viewYear, viewQuarter, query]);

  const STRATEJIK_totalWeighted = useMemo(
    () =>
      STRATEJIK_filtered.reduce(
  ...
  const STRATEJIK_resetForm = () => {
    setSTRATEJIK_form(stratejikMakeRow());
  ...
  const buildBaseRow = () => {
    const peringkatNum =
      STRATEJIK_form.peringkat === "" || STRATEJIK_form.peringkat == null
        ? 0
        : Number(STRATEJIK_form.peringkat);
    ...
    return {
      ...
      numeratorLabel:
        STRATEJIK_form.mode === "NILAI_TUNGGAL" || STRATEJIK_form.mode === "TEKS"
          ? ""
          : STRATEJIK_form.numeratorLabel,
      numeratorValue:
        STRATEJIK_form.mode === "NILAI_TUNGGAL" || STRATEJIK_form.mode === "TEKS"
          ? ""
          : STRATEJIK_form.numeratorValue,
  ...
  const STRATEJIK_addRow = () => {
    ...
    const newRowKey = makeRowKey({ ...newRow, source: "STRATEJIK" });

    setSTRATEJIK_rows((prev) => {
      const isDuplicate = prev.some(r =>
        makeRowKey({ ...r, source: "STRATEJIK" }) === newRowKey
      );

      if (isDuplicate) {
        console.warn('[STRATEJIK_addRow] DUPLICATE DETECTED! Row already exists:', newRowKey);
    ...
    // ✅ Hapus baris duplikat setSTRATEJIK_rows
    STRATEJIK_resetForm();
  ...
  const STRATEJIK_saveEdit = () => {
    ...
    setSTRATEJIK_rows((prev) =>
      prev.map((r) =>
        r === STRATEJIK_editingRow
          ? {
            ...baseRow,
            // ✅ PERTAHANKAN year dan quarter yang asli
            year: STRATEJIK_editingRow.year,
            quarter: STRATEJIK_editingRow.quarter,
            hasil: baseRow.mode === "TEKS" ? "" : (rawHasil === "" || rawHasil == null ? 0 : rawHasil),
            hasilText: baseRow.mode === "TEKS" ? (baseRow.hasilText || "") : "",
  ...
  const STRATEJIK_removeRow = (row) => {
    if (!row) return;
    setSTRATEJIK_rows((arr) => arr.filter((r) => r !== row));
    if (STRATEJIK_editingRow === row) STRATEJIK_resetForm();
  };

  const STRATEJIK_exportExcel = () =>
    exportStratejikToExcel
  ...
  const loadKPMRRows = () => {
    if (typeof window === "undefined") return [];
    try {
      const raw = localStorage.getItem(KPMR_STRATEJIK_LS);
      return raw ? JSON.parse(raw) : [];
    } catch (e) {
      console.warn("Gagal parse KPMR Stratejik dari localStorage", e);
      return [];
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('stratejikKPMR:changed', {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Gagal simpan KPMR Stratejik ke localStorage", e);
    }
  }, [KPMR_rows]);
            ...
            <header className="px-4 py-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-end gap-4">
                ...
                <div className="flex items-end gap-2">
                  {/* Button Tambah Data */}
                  <button
                    onClick={() => setShowStratejikForm(true)}
              ...
              <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
                ...
                <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() => setShowStratejikForm(false)}
                  ...
                  <div className="flex items-end gap-4 mb-4">
                    ...
                    <div className="flex gap-2 self-end">
                      ...
                      <button
                        ...
                        onClick={() =>
                          STRATEJIK_sectionForm.id &&
                          STRATEJIK_removeSection(STRATEJIK_sectionForm.id)
                        }
                        title="Hapus Section"
                      >
                        <Trash2 size={20} />
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      value={STRATEJIK_sectionForm.id || ""}
                      onChange={(e) => {
                        const selectedId = e.target.value;
                        if (selectedId) STRATEJIK_selectSection(selectedId);
                      }}
                    >
                  ...
                  <div className="grid grid-cols-12 gap-4 mb-4">
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        value={STRATEJIK_form.subNo}
                        onChange={(e) =>
                          setSTRATEJIK_form((f) => ({ ...f, subNo: e.target.value }))
                        }
                        placeholder="1.1"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={STRATEJIK_form.mode === "RASIO" ? "10000" : "5"}
                        />
                      </div>
                    </div>
                  )}
                      ...
                      <div className="col-span-6 space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setSTRATEJIK_form((f) => ({ ...f, dampak: e.target.value }))
                            }
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              value={STRATEJIK_form.peringkat ?? ""}
                              readOnly
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              value={(() => {
                                const peringkatNum =
                                  STRATEJIK_form.peringkat === "" ||
                                    STRATEJIK_form.peringkat == null
                                    ? 0
                                    : Number(STRATEJIK_form.peringkat);
                          ...
                          <div>
                            ...
                            <input
                              className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-gray-50"
                              value={(() => {
                                if (STRATEJIK_form.mode === "TEKS") {
                                  return STRATEJIK_form.hasilText || "";
                                }
                                ...
                                if (STRATEJIK_form.mode === "NILAI_TUNGGAL") {
                        ...
                        <div>
                          ...
                          <textarea
                            rows={3}
                            className="w-full rounded-lg border-2 border-gray-300 px-4 py-3 text-sm bg-white"
                            value={STRATEJIK_form.keterangan}
                            onChange={(e) =>
                              setSTRATEJIK_form((f) => ({
                                ...f,
              ...
              <div className="bg-white rounded-2xl shadow overflow-hidden border border-gray-200">
                ...
                <div className="relative h-[350px]">
                  <div className="absolute inset-0 overflow-x-auto overflow-y-auto">
                    <DataTable
                      rows={STRATEJIK_filtered}
                      totalWeighted={STRATEJIK_totalWeighted}
                      viewYear={viewYear}
                      viewQuarter={viewQuarter}
                      startEdit={STRATEJIK_startEdit}
                      removeRow={STRATEJIK_removeRow}
                    />
                  </div>
                </div>
