...
export interface InvestasiOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
...
export interface ParameterEntity {
  ...
  orderIndex: number;
  investasiId: number; // ✅ BUKAN kpmrInvestasiId
  createdAt: Date;
  updatedAt: Date;
}
...
export interface UpdateInvestasiDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class InvestasiService {
  ...
  async findOrCreate(
    ...
    try {
      ...
      const createDto: CreateInvestasiDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
   ...
   */
  async ensureDataExists(year: number, quarter: number): Promise<InvestasiOjkEntity> {
   ...
   */
  async loadOrCreateData(year: number, quarter: number): Promise<InvestasiOjkEntity> {
   ...
   */
  private formatToFrontend(entity: InvestasiOjkEntity | null): any[] {
    ...
    const result = parameters.map((parameter, index) => {
      ...
      const formattedParameter = {
        ...
        // Metadata dari investasi
        metadata: {
          investasiId: entity.id,
          year: entity.year,
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[InvestasiService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<InvestasiOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_investasi_produk.get<InvestasiOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<InvestasiOjkEntity | null> {
  ...
  async getById(id: number): Promise<InvestasiOjkEntity> {
  ...
  async create(createDto: CreateInvestasiDto): Promise<InvestasiOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_investasi_produk.post<InvestasiOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateInvestasiDto): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_investasi_produk.put<InvestasiOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_investasi_produk.put<InvestasiOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateInvestasiDto['summary']): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_investasi_produk.put<InvestasiOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async getParameters(investasiId: number): Promise<ParameterEntity[]> {
    const url = `${this.baseUrl}/${investasiId}/parameters`;
    this.logApiCall('GET', url);

    try {
      const response = await api_investasi_produk.get<ParameterEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addParameter(investasiId: number, dto: CreateParameterDto): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters`;

    console.log('[Service] addParameter:', { url, investasiId, dto });
    ...
    try {
      ...
      const response = await api_investasi_produk.post<InvestasiOjkEntity>(url, payload);
  ...
  async updateParameter(investasiId: number, parameterId: number, dto: UpdateParameterDto): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}`;
    ...
    try {
      ...
      const response = await api_investasi_produk.put<InvestasiOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(investasiId: number, parameterId: number): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_investasi_produk.post<InvestasiOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(investasiId: number, parameterId: number): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_investasi_produk.delete<InvestasiOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(investasiId: number, parameterIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${investasiId}/parameters/reorder`;
  ...
  async getNilai(investasiId: number, parameterId: number): Promise<NilaiEntity[]> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai`;
    this.logApiCall('GET', url);

    try {
      const response = await api_investasi_produk.get<NilaiEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addNilai(investasiId: number, parameterId: number, dto: CreateNilaiDto): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai`;
  ...
  async updateNilai(investasiId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai/${nilaiId}`;
  ...
  async copyNilai(investasiId: number, parameterId: number, nilaiId: number): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai/${nilaiId}/copy`;
    this.logApiCall('POST', url);
  ...
  async removeNilai(investasiId: number, parameterId: number, nilaiId: number): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(investasiId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${investasiId}/parameters/${parameterId}/nilai/reorder`;
  ...
  async exportToExcel(investasiId: number): Promise<any> {
    const url = `${this.baseUrl}/${investasiId}/export`;
    this.logApiCall('GET', url);
  ...
  async importFromExcel(importData: any): Promise<InvestasiOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_investasi_produk.post<InvestasiOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: InvestasiOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<InvestasiOjkEntity> {
    ...
    try {
      ...
      const fallbackData: InvestasiOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: InvestasiOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_investasi_produk.get(this.baseUrl, {
        timeout: 5000,
      });
...
export default investasiService;
