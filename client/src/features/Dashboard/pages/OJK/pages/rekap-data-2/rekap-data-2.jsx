...
import { StoreIcon, HandCoins, BanknoteArrowUp, BrainCircuit, Scale, Cog, ClipboardCheck, CircleStar, BrainCog, Handshake, Sprout, TrendingUpDown, Earth, FileText, Grid3x3, Circle } from 'lucide-react';
      ...
      <div className="bg-white rounded-lg shadow border p-4 h-full">
                ...
                {RISK_MATRIX.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    ...
                    {row.map((cell, cellIndex) => {
                      ...
                      return (
                        <td key={cellIndex} className={`border border-black p-3 text-center font-bold text-lg relative ${isActive ? 'ring-2 ring-blue-500 ring-offset-1' : ''}`} style={{ backgroundColor: cellIndicator.color }}>
                          {cell}
                          {isActive && (
        ...
        <div className="space-y-2">
          {showPosition ? (
            ...
            <div className="text-lg font-semibold text-gray-950 text-center p-4 bg-gray-100 rounded-lg">Data tidak cukup untuk menampilkan posisi risiko</div>
          )}
        </div>
      </div>
    );
    ...
    <div className="w-full min-h-screen p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-4 mt-6">
        ...
        <div className="lg:col-span-4">
          <RiskMatrix inherentScore={footerDisplay.inherentScoreForMatrix} kpmrScore={footerDisplay.kpmrScoreForMatrix} hasInherentData={footerDisplay.hasInherentData} hasKpmrData={footerDisplay.hasKpmrData} />
        </div>
      </div>
      ...
      <div className="mt-6 rounded-lg border border-gray-200 bg-white p-3">
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-base font-semibold"></div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-base font-semibold text-gray-950">NET RISK :</span>
            {INHERENT_RISK_INDICATORS.map((i, idx) => (
              <div key={idx} className="flex items-center gap-1.5 text-base font-semibold">
                <span className="w-5 h-5 rounded border" style={{ backgroundColor: i.color }} />
                <span className="text-gray-950">
