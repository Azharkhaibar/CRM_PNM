...
export function exportInvestasiToExcel(filteredRows, viewYear, viewQuarter) {
  ...
  const sorted = filteredRows.slice().sort((a, b) => a.subNo.localeCompare(b.subNo, undefined, { numeric: true }));
  ...
  sorted.forEach((r) => {
    ...
    // pembilang
    dataRows.push(['', '', '', '', String(r.numeratorLabel || 'Pembilang'), '', '', '', '', '', '', '', '', r.numeratorValue === '' ? '' : Number(r.numeratorValue || 0), '', '', '', '']);
    // penyebut
    dataRows.push(['', '', '', '', String(r.denominatorLabel || 'Penyebut'), '', '', '', '', '', '', '', '', r.denominatorValue === '' ? '' : Number(r.denominatorValue || 0), '', '', '', '']);
  });
