      ...
      <div className="flex-1 overflow-x-auto w-full max-w-[calc(100vw-2rem)] lg:max-w-[calc(100vw-20rem)] h-[600px]">
        <table className="w-full min-w-[2000px] text-sm text-left border-collapse separate border-spacing-0">
          <thead className="bg-[#1f4e79] text-white">
            <tr>
              <th rowSpan={2} className="py-3 px-4 border border-black text-center align-middle w-32 bg-[#1f4e79]">
                Risiko
              </th>

              <th rowSpan={2} className="py-3 px-2 border border-black text-center align-middle w-12 bg-[#1f4e79]">
                No
              </th>

              <th rowSpan={2} className="py-3 px-4 border border-black text-center align-middle min-w-[200px] bg-[#1f4e79] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.3)]">
                Parameter
              </th>

              <th rowSpan={2} className="py-3 px-2 border border-black text-center align-middle w-24">
                RKAP
                <br />
                {year}
              </th>
              ...
              {showDetailColumns && (
                <th rowSpan={2} className="py-3 px-2 border border-black text-center align-middle w-32">
                  Penjelasan
                  <br />
                  Tipe Data
                </th>
              ...
              <th colSpan={showDetailColumns ? 3 : 2} className="py-2 px-2 border border-black text-center bg-[#163a5c]">
                RAS {year}
              </th>

              {showDetailColumns && (
                <th rowSpan={2} className="py-3 px-4 border border-black text-center align-middle w-40">
                  Keterangan
                </th>
              )}

              {selectedMonths.map((mIdx) => (
                <th key={mIdx} rowSpan={2} className="py-3 px-4 border border-black text-center align-middle w-28 bg-[#163a5c]">
                  {getMonthName(mIdx)}
                </th>
              ))}
              <th rowSpan={2} className="py-3 px-2 border border-black text-center align-middle w-24">
          ...
          <tbody className="text-gray-800">
            {Object.keys(groupedData).length === 0 ? (
              <tr>
                <td colSpan={100} className="p-8 text-center text-gray-500 bg-gray-50">
                      ...
                      return (
                        <React.Fragment key={item.id}>
                          <tr className="hover:bg-blue-50/50">
                            ...
                            <td className="px-2 py-3 text-center align-top border border-gray-400">
                              <span className={`px-2 py-1 rounded text-[10px] font-bold border ${getStanceBadgeColor(item.riskStance)}`}>{item.riskStance}</span>
                            </td>

                            {showDetailColumns && (
                              <>
                                <td className="px-3 py-3 text-xs align-top border border-gray-400 text-justify">{item.statement}</td>
                                <td rowSpan={itemSpan} className="px-2 py-3 text-xs align-top border border-gray-400 text-gray-600">
                                  {item.notes}
                                </td>
                              </>
                            )}
                          ...
                          {showNumDenom && hasCalc && (
                            <>
                              <tr className="bg-indigo-50/50 text-xs text-gray-600">
                                ...
                                {selectedMonths.map((mIdx) => (
                                  <td key={mIdx} className="px-3 py-2 text-center border border-gray-400 font-mono bg-[#c6d9a7]">
                                    {fmtNumber(getValueForMonth(item, mIdx).num)}
                                  </td>
                                ))}
                              </tr>
                              <tr className="bg-indigo-50/50 text-xs text-gray-600">
                                ...
                                {selectedMonths.map((mIdx) => (
                                  <td key={mIdx} className="px-3 py-2 text-center border border-gray-400 font-mono bg-[#c6d9a7]">
                                    {fmtNumber(getValueForMonth(item, mIdx).den)}
                                  </td>
                                ))}
                              </tr>
                            </>
