...
async function exportInherentToExcel({
  ...
  year,
  quarter,
  categoryLabel = "InvestasiRegulatory",
}) {
  if (!Array.isArray(rows) || rows.length === 0) {
...
export default function InvestasiRegulatory() {
  const { year, activeQuarter, search, exportRequestId, resetExport } = useHeaderStore();

  const CATEGORY_ID = "investasi-regulatory";
  ...
  const handleExportToExcel = useCallback(async () => {
    if (activeTab === "inherent") {
      await exportInherentToExcel({
        ...
        year,
        quarter: activeQuarter,
        categoryLabel: "Investasi Regulatory",
      });
  ...
  return (
    <div className="w-full space-y-4">
      <Header title="Risk Profile – Investasi" onExportClick={handleExportToExcel} showQuarterActions={true}/>
