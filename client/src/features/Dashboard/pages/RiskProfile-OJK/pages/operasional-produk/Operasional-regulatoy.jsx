...
// ===================== EXPORT INHERENT =====================
async function exportInherentToExcel({ rows = [], year, quarter, categoryLabel = "OperasionalRegulatory" }) {
  if (!Array.isArray(rows) || rows.length === 0) {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet(`5. Inherent ${categoryLabel}`, {
      views: [{ showGridLines: true }]
    });

    // Set kolom widths (sesuai permintaan, kolom terakhir dihapus)
    worksheet.columns = [
      { width: 5 },    // 1. No Parameter
      { width: 7 },    // 2. Bobot Parameter
      { width: 26 },   // 3. Parameter
      { width: 5 },    // 4. No Indikator (dulu Nilai)
      { width: 30 },   // 5. Indikator (dulu Nilai)
      { width: 7 },    // 6. Bobot Indikator (dulu Bobot Nilai)
      { width: 25 },   // 7. Keterangan (dulu % Dalam Portofolio)
      { width: 15 },   // 8. Low
      ...
      { width: 15 },   // 12. High
      { width: 15 },   // 13. Hasil
      { width: 15 },   // 14. Peringkat
      { width: 15 },   // 15. Weighted
      // Kolom 16 (Keterangan) dihapus
    ];

    // Judul
    const titleRow = worksheet.addRow([`Risk Profile OJK - ${categoryLabel}`]);
    titleRow.font = { size: 16, bold: true };
    worksheet.mergeCells('A1:O1'); // O karena kolom 15
    ...
    // Header tabel (tanpa kolom Keterangan terakhir)
    const headerRow = worksheet.addRow([
      'No', 'Bobot', 'Parameter', 'No', 'Indikator', 'Bobot', 'Keterangan',
      'Low', 'Low To Moderate', 'Moderate', 'Moderate To High', 'High',
      'Hasil', 'Peringkat', 'Weighted'
    ]);

    // Styling header (sama seperti PasarProduk, hanya jumlah kolom berkurang)
    headerRow.eachCell((cell, colNumber) => {
      ...
      if (colNumber <= 3) {
        ...
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colors[colNumber - 8] } };
        
        // Atur warna teks (putih untuk Low dan High, hitam untuk lainnya)
        if (colNumber === 12) { // High
          cell.font.color = { argb: 'FFFFFFFF' };
        } else if (colNumber === 8) { // Low
          cell.font.color = { argb: 'FFFFFFFF' };
        } else {
    ...
    // Format persentase (sama seperti PasarProduk)
    ...
    // Fungsi warna background peringkat (sama)
    ...
    rows.forEach((param, pi) => {
      ...
      if (nilaiList.length === 0) {
        // Baris kosong untuk parameter tanpa nilai (kolom terakhir dihilangkan)
        const emptyRow = [
          param.nomor || "-",
      ...
      // Hitung total baris untuk parameter ini (sama seperti PasarProduk)
      ...
      nilaiList.forEach((nilai, ni) => {
        ...
        for (let subIndex = 0; subIndex < rowsForThisNilai; subIndex++) {
          ...
          // Kolom parameter (No, Bobot, Parameter) hanya di baris pertama parameter
          ...
          // Kolom Indikator (teks)
          rowData.push(nilaiText);

          // Kolom Bobot Indikator
          if (isMainRow) {
          ...
          // Kolom Keterangan (dulu portofolio)
          if (isMainRow) {
          ...
          // Kolom risk indicator (hanya di baris pertama)
          ...
          // Kolom Peringkat dan Weighted (hanya di baris pertama)
          if (isFirstRowOfNilai) {
            rowData.push(Number.isFinite(peringkat) ? peringkat : "-");
            rowData.push(weightedDisplay || "");
            // Kolom Keterangan dihapus, jadi tidak ada tambahan data
          } else {
            rowData.push("", "");
          }

          // Tambahkan baris (sekarang hanya 15 kolom)
          ...
          row.eachCell((cell, colNumber) => {
            ...
            // Background colors (sesuaikan dengan jumlah kolom)
            if (colNumber <= 3) {
              cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8F5FA' } };
            } else if (colNumber === 4) {
              if (isMainRow) {
              ...
              if (isFirstRowOfNilai && Number.isFinite(peringkat)) {
                ...
                cell.font = { bold: true, color: { argb: peringkat === 1 || peringkat === 5 ? 'FFFFFFFF' : 'FF000000' } };
                cell.alignment.horizontal = 'center';
              }
        ...
        if (rowsForThisNilai === 1) {
          mergeRanges.push({ start: nilaiStart, end: nilaiEnd, col: 4 }); // No Indikator
          mergeRanges.push({ start: nilaiStart, end: nilaiEnd, col: 14 }); // Peringkat
          mergeRanges.push({ start: nilaiStart, end: nilaiEnd, col: 15 }); // Weighted
        } else {
          mergeRanges.push({ start: nilaiStart, end: nilaiEnd, col: 14 }); // Peringkat
          mergeRanges.push({ start: nilaiStart, end: nilaiEnd, col: 15 }); // Weighted
          // No Indikator tidak di-merge
        }
      });

      // Merge parameter (No, Bobot, Parameter)
    ...
    mergeRanges.forEach(range => {
      if (range.start !== range.end) {
        ...
        if (range.col !== 16) { // kolom 16 sudah tidak ada, sesuaikan
    ...
    summaryRow.eachCell((cell, colNumber) => {
      ...
      if (colNumber === 13 || colNumber === 14) {
        ...
        cell.font = { bold: true, color: { argb: summaryBgColor === '4F6228' || summaryBgColor === 'FF0000' ? 'FFFFFFFF' : 'FF000000' } };
      }
    });
...
// ===================== EXPORT KPMR =====================
async function exportKpmrToExcel({ rows = [], year, quarter, categoryLabel = "OperasionalRegulatory", selectedQuarters = ["Q1", "Q2", "Q3", "Q4"] }) {
  if (!Array.isArray(rows) || rows.length === 0) {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet(`5. KPMR ${categoryLabel}`, {
      views: [{ showGridLines: true }]
    });
    ...
    const calculateGlobalSummary = () => {
      ...
      selectedQuarters.forEach(q => {
        if (summary[q].length === 0) result[q] = "-";
          ...
          const total = summary[q].reduce((a, b) => a + b, 0);
          result[q] = (total / summary[q].length).toFixed(2);
        }
      });
      return result;
    };
    ...
    const createAspekHeader = (rowNumber, aspek) => {
      ...
      selectedQuarters.forEach((q, qIdx) => {
        const avg = getQuarterAvg(aspek, q);
        const col = 4 + qIdx;
    ...
    normalizedRows.forEach(aspek => {
      ...
      if (list.length === 0) {
        ...
        list.forEach((pertanyaan, qi) => {
          const row = worksheet.getRow(currentRow);
          row.getCell(2).value = pertanyaan.nomor || (qi + 1);
          row.getCell(3).value = pertanyaan.pertanyaan || "-";
          selectedQuarters.forEach((q, qIdx) => {
            const col = 4 + qIdx;
            const skorValue = pertanyaan.skor?.[q] || pertanyaan.skor?.[q.toLowerCase()];
            const hasSkor = skorValue != null && skorValue !== "";
            row.getCell(col).value = hasSkor ? skorValue : "-";
            if (hasSkor) {
              row.getCell(col).font = { color: { argb: 'FF000000' } };
            }
          });
          const descKeys = ["strong", "satisfactory", "fair", "marginal", "unsatisfactory"];
          descKeys.forEach((key, idx) => {
            row.getCell(8 + idx).value = pertanyaan.indicator?.[key] || "-";
          });
          row.getCell(13).value = pertanyaan.evidence || "-";
          row.height = 75;
          row.eachCell((cell, colNumber) => {
            if (colNumber >= 2 && colNumber <= 13) {
              cell.alignment = { vertical: 'middle', horizontal: (colNumber === 3 || colNumber >= 8) ? 'left' : 'center', wrapText: true };
              cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
              if (colNumber === 2 || colNumber === 3) {
                cell.fill = qi % 2 === 0
    ...
    if (selectedQuarters.length > 0) {
      ...
      worksheet.mergeCells(`B${currentRow}:C${currentRow}`);
      selectedQuarters.forEach((q, idx) => {
        const col = 4 + idx;
        const value = globalSummary[q];
    ...
    for (let i = 1; i <= currentRow; i++) {
      ...
      if (row) {
        if (i === 1) row.height = 30;
        else if (i === 2) row.height = 25;
        else if (i === 3) row.height = 10;
        else if (i === 4) row.height = 30;
      }
    }
    ...
    console.error('Error exporting KPMR:', error);
    alert('Gagal mengexport data KPMR ke Excel.\nError: ' + error.message);
  }
}
...
export default function OperasionalRegulatory() {
  ...
  const saveInherentData = useCallback(() => {
    if (!isDataReady || !initialLoadDone || inherentRows.length === 0) return false;
    const currentSignature = getInherentSignature(inherentRows);
    if (currentSignature === lastSavedSignature && !isSaving) return true;
    setIsSaving(true);
    try {
      saveInherent({ categoryId: CATEGORY_ID, year, quarter: activeQuarter, rows: inherentRows });
      const derivedValues = inherentRows.flatMap(param => (param.nilaiList || []).map(nilai => computeDerived(nilai, param)));
      const snapshot = computeInherentSnapshot(inherentRows);
      saveDerived({ categoryId: CATEGORY_ID, year, quarter: activeQuarter, snapshot, values: derivedValues });
      notifyRiskUpdated();
  ...
  useEffect(() => {
    if (!isDataReady || !initialLoadDone || activeTab !== "inherent") return;
    if (saveTimeoutRef.current) clearTimeout(saveTimeoutRef.current);
    saveTimeoutRef.current = setTimeout(() => {
      const currentSignature = getInherentSignature(inherentRows);
      if (currentSignature !== lastSavedSignature) saveInherentData();
    }, 2000);
  ...
  useEffect(() => {
    ...
    return () => { delete window.saveInherentData; };
  }, [saveInherentData, activeTab, isSaving]);

  useEffect(() => {
    setIsDataReady(false);
    setInitialLoadDone(false);
    const inh = loadInherent({ categoryId: CATEGORY_ID, year, quarter: activeQuarter });
    const kpmr = loadKpmr({ categoryId: CATEGORY_ID, year });
  ...
  const saveKpmrDataImmediate = useCallback((rowsToSave = null) => {
    ...
    try {
      saveKpmr({ categoryId: CATEGORY_ID, year, rows });
      return true;
    } catch (error) {
  ...
  const saveInherentDataImmediate = useCallback((rowsToSave = null) => {
    ...
    if (!isDataReady || !initialLoadDone) return false;
    setIsSaving(true);
    try {
      saveInherent({ categoryId: CATEGORY_ID, year, quarter: activeQuarter, rows });
      const derivedValues = rows.flatMap(param => (param.nilaiList || []).map(nilai => computeDerived(nilai, param)));
  ...
  return (
    <div className="w-full space-y-4">
      <Header
        title="Risk Profile – Operasional"
