...
import { computeWeighted } from '../../utils/likuiditas/calc';
...
export default function FormSection({ form: incomingForm, setForm = () => {}, onAdd = () => {}, onSave = () => {}, onReset = () => {}, editing = false, title = 'Form Likuiditas', loading = false }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (logCreate && typeof logCreate === 'function') {
        await logCreate('LIKUIDITAS', `Menambahkan data likuiditas baru - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: '/api/likuiditas/investments',
          isSuccess: true,
          metadata: {
      ...
      onReset();
      alert('Data likuiditas berhasil ditambahkan!');
    } catch (error) {
      console.error('Error adding likuiditas:', error);

      if (logCreate && typeof logCreate === 'function') {
        await logCreate('LIKUIDITAS', `Gagal menambahkan data likuiditas - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: '/api/likuiditas/investments',
          isSuccess: false,
          metadata: {
            error: error.message,
      ...
      alert('Gagal menambahkan data likuiditas: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('LIKUIDITAS', `Memperbarui data likuiditas - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}", Sub No: ${form.subNo}`, {
          endpoint: `/api/likuiditas/investments/${form.id || serviceData.no_indikator}`,
          isSuccess: true,
          metadata: {
            no_indikator: form.subNo,
            parameter: form.sectionLabel,
      ...
      alert('Data likuiditas berhasil diperbarui!');
    } catch (error) {
      console.error('Error saving likuiditas:', error);

      if (logUpdate && typeof logUpdate === 'function') {
        await logUpdate('LIKUIDITAS', `Gagal memperbarui data likuiditas - Parameter: "${form.sectionLabel}", Indikator: "${form.indikator}"`, {
          endpoint: `/api/likuiditas/investments/${form.id || 'unknown'}`,
          isSuccess: false,
          metadata: {
            error: error.message,
          },
      ...
      alert('Gagal memperbarui data likuiditas: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black mt-1">Kelola data likuiditas dan analisis risiko secara komprehensif</p>
            </div>
          </div>
