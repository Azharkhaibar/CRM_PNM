...
import { rekapDataAPI } from "../rekapdata1/service/rekap-data-api.service";
...
export default function Komposit() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const response = await rekapDataAPI.getKompositSummary(year, quarter);
                if (isMounted) {
                    setApiData(response.data);
                }
            } catch (err) {
                console.error("[KOMPOSIT] Error fetching summary:", err);
                if (isMounted) {
    ...
    const data = useMemo(() => {
        ...
        try {
            const { kompositA, kompositB, risks, bhzConfig } = apiData;

            // Build rows dengan BHz
            const rows = risks.map(r => {
                const bhzKey = r.label.toLowerCase();
                const bhz = Number(bhzConfig?.[bhzKey] ?? 0);
            ...
            console.error("[KOMPOSIT] Error parsing apiData:", err);
            return null;
        }
    }, [apiData]);
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center py-12 text-[#0068B3] font-semibold text-lg">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#0068B3] mr-3"></div>
                Memuat data komposit...
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="p-6 text-red-600 bg-red-50 border border-red-200 rounded-lg max-w-xl mx-auto my-8">
    ...
    if (!data || rows.length === 0) {
        return (
            <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
                ...
                <div className="bg-white rounded-xl shadow-xl p-8 border border-gray-200 text-center text-gray-500 font-medium text-lg">
                    Data belum tersedia untuk periode tahun {year} {quarter}.
                </div>
            </div>
        );
    }
