// src/ojk/rentabilitas-produk/rentabilitas-produk-ojk/rentabilitas-produk-ojk.service.ts
import {
  Injectable,
  NotFoundException,
  BadRequestException,
...
import { InherentReferenceRentabilitas } from './entities/rentabilitas-inherent-references.entity';
import {
  CreateRentabilitasInherentDto,
  UpdateRentabilitasInherentDto,
  CreateParameterDto,
  UpdateParameterDto,
  CreateNilaiDto,
  UpdateNilaiDto,
...
@Injectable()
export class RentabilitasProdukOjkService {
  private readonly logger = new Logger(RentabilitasProdukOjkService.name);

  constructor(
    @InjectRepository(RentabilitasProdukOjk)
    private inherentRepository: Repository<RentabilitasProdukOjk>,
    ...
    @InjectRepository(InherentReferenceRentabilitas)
    private referenceRepository: Repository<InherentReferenceRentabilitas>,
    private dataSource: DataSource,
  ) {}

  // === CRUD UTAMA (RentabilitasProdukOjk) ===

  async create(createDto: CreateRentabilitasInherentDto, userId: string) {
    try {
      ...
      this.logger.error(
        `Error creating RentabilitasProdukOjk: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findActive(): Promise<RentabilitasProdukOjk | null> {
  ...
  async findByYearQuarter(
    year: number,
    quarter: number,
  ): Promise<RentabilitasProdukOjk | null> {
    this.logger.log(
  ...
  async update(
    id: number,
    updateDto: UpdateRentabilitasInherentDto,
    userId: string,
  ) {
  ...
  async remove(id: number) {
    ...
    try {
      ...
      // Hapus inherent
      await queryRunner.manager.delete(RentabilitasProdukOjk, { id });
    ...
    try {
      for (let i = 0; i < reorderDto.parameterIds.length; i++) {
        ...
        await queryRunner.manager.update(
          ...
          { id: parameterId, rentabilitasProdukOjkId: inherentId },
          { orderIndex: i },
        );
      }
    ...
    try {
      ...
      if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
        const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
          ...
          return queryRunner.manager.save(RentabilitasNilai, newNilai);
        });
      ...
      for (let i = 0; i < importData.parameters.length; i++) {
        ...
        if (paramData.nilaiList && Array.isArray(paramData.nilaiList)) {
          for (let j = 0; j < paramData.nilaiList.length; j++) {
            ...
            await queryRunner.manager.save(RentabilitasNilai, nilai);
          }
        }
      }
