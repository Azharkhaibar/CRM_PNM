...
var PasarProdukOjkService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PasarProdukOjkService = void 0;
...
let PasarProdukOjkService = PasarProdukOjkService_1 = class PasarProdukOjkService {
    inherentRepository;
    parameterRepository;
    nilaiRepository;
    referenceRepository;
    dataSource;
    logger = new common_1.Logger(PasarProdukOjkService_1.name);
    ...
    async create(createDto, userId) {
        try {
            ...
            this.logger.error(`Error creating PasarProdukOjk: ${error.message}`, error.stack);
            throw error;
        }
    }
    async findActive() {
    ...
    async updateActiveStatus(id, isActive, userId) {
        ...
        if (isActive) {
            ...
            await this.inherentRepository
                .createQueryBuilder()
                .update(pasar_produk_ojk_entity_1.PasarProdukOjk)
                .set({ isActive: false })
                .execute();
        }
    ...
    async remove(id) {
        ...
        try {
            for (const parameter of inherent.parameters || []) {
                await queryRunner.manager.delete(pasar_produk_nilai_entity_1.PasarNilai, {
                    parameterId: parameter.id,
                });
            }
            await queryRunner.manager.delete(pasar_produk_parameter_entity_1.PasarParameter, {
                pasarProdukOjkId: id,
            });
            await queryRunner.manager.delete(pasar_produk_ojk_entity_1.PasarProdukOjk, { id });
    ...
    async addParameter(inherentId, createParamDto, userId) {
        ...
        if (createParamDto.kategori) {
            ...
            if (kategori.model === pasar_produk_inherent_dto_1.KategoriModel.OPEN_END) {
                if (!kategori.jenis) {
            ...
            if (kategori.model === pasar_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR) {
                if (kategori.jenis) {
            ...
            if (kategori.model === pasar_produk_inherent_dto_1.KategoriModel.TANPA_MODEL) {
                if (kategori.prinsip ||
                    kategori.jenis ||
        ...
        const lastParam = await this.parameterRepository.findOne({
            where: { pasarProdukOjkId: inherentId },
        ...
        const kategoriFormatted = createParamDto.kategori
            ? {
                ...
                prinsip: createParamDto.kategori.model !== pasar_produk_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? createParamDto.kategori.prinsip
                    : undefined,
                jenis: createParamDto.kategori.model === pasar_produk_inherent_dto_1.KategoriModel.OPEN_END
                    ? createParamDto.kategori.jenis
                    : undefined,
                underlying: createParamDto.kategori.model === pasar_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR
    ...
    async updateParameter(inherentId, parameterId, updateParamDto, userId) {
        ...
        const parameter = await this.parameterRepository.findOne({
            where: { id: parameterId, pasarProdukOjkId: inherentId },
        });
        if (!parameter) {
        ...
        if (updateParamDto.kategori) {
            ...
            parameter.kategori = {
                ...
                prinsip: kategori.model !== pasar_produk_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? kategori.prinsip
                    : undefined,
                jenis: kategori.model === pasar_produk_inherent_dto_1.KategoriModel.OPEN_END
                    ? kategori.jenis
                    : undefined,
                underlying: kategori.model === pasar_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR
                    ? kategori.underlying || []
                    : [],
            };
        }
    ...
    async reorderParameters(inherentId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.parameterIds.length; i++) {
                ...
                await queryRunner.manager.update(pasar_produk_parameter_entity_1.PasarParameter, { id: parameterId, pasarProdukOjkId: inherentId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async copyParameter(inherentId, parameterId, userId) {
        ...
        try {
            ...
            const savedParam = await queryRunner.manager.save(pasar_produk_parameter_entity_1.PasarParameter, newParam);
            if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
                const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
                    ...
                    return queryRunner.manager.save(pasar_produk_nilai_entity_1.PasarNilai, newNilai);
                });
    ...
    async removeParameter(inherentId, parameterId, userId) {
        ...
        try {
            ...
            await queryRunner.manager.delete(pasar_produk_parameter_entity_1.PasarParameter, { id: parameterId });
            await queryRunner.commitTransaction();
            await this.inherentRepository.update(inherentId, {
                updatedBy: userId,
                updatedAt: new Date(),
    ...
    async addNilai(inherentId, parameterId, createNilaiDto, userId) {
        ...
        const nilai = {
            ...
            judul: {
                type: createNilaiDto.judul?.type || pasar_produk_inherent_dto_1.JudulType.TANPA_FAKTOR,
                text: judulText.trim(),
    ...
    async updateNilai(inherentId, parameterId, nilaiId, updateNilaiDto, userId) {
        ...
        if (nilai.parameter.pasarProdukOjkId !== inherentId) {
    ...
    async reorderNilai(parameterId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.nilaiIds.length; i++) {
                ...
                await queryRunner.manager.update(pasar_produk_nilai_entity_1.PasarNilai, { id: nilaiId, parameterId: parameterId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async validateModelTerstruktur(inherentId) {
        ...
        const terstrukturParams = inherent.parameters?.filter((param) => param.kategori?.model === pasar_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR) || [];
    ...
    async importFromExcel(importData, userId) {
        ...
        await queryRunner.startTransaction();
        try {
            await queryRunner.manager.update(pasar_produk_ojk_entity_1.PasarProdukOjk, { isActive: true }, { isActive: false });
            const inherent = {
            ...
            const savedInherent = await queryRunner.manager.save(pasar_produk_ojk_entity_1.PasarProdukOjk, inherent);
            for (let i = 0; i < importData.parameters.length; i++) {
                ...
                const parameter = {
                    ...
                    pasarProdukOjkId: savedInherent.id,
                    orderIndex: paramData.orderIndex || i,
                };
                const savedParam = await queryRunner.manager.save(pasar_produk_parameter_entity_1.PasarParameter, parameter);
                    ...
                    for (let j = 0; j < paramData.nilaiList.length; j++) {
                        ...
                        const nilai = {
                            ...
                            judul: nilaiData.judul || {
                                type: pasar_produk_inherent_dto_1.JudulType.TANPA_FAKTOR,
                                text: '',
                                value: null,
                                pembilang: '',
                                valuePembilang: null,
                        ...
                        await queryRunner.manager.save(pasar_produk_nilai_entity_1.PasarNilai, nilai);
                    }
                }
            }
            await queryRunner.commitTransaction();
...
exports.PasarProdukOjkService = PasarProdukOjkService;
exports.PasarProdukOjkService = PasarProdukOjkService = PasarProdukOjkService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(pasar_produk_ojk_entity_1.PasarProdukOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(pasar_produk_parameter_entity_1.PasarParameter)),
    __param(2, (0, typeorm_1.InjectRepository)(pasar_produk_nilai_entity_1.PasarNilai)),
    __param(3, (0, typeorm_1.InjectRepository)(pasar_inherent_refetences_entity_1.InherentReferencePasar)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], PasarProdukOjkService);
//# sourceMappingURL=pasar-produk-ojk.service.js.map