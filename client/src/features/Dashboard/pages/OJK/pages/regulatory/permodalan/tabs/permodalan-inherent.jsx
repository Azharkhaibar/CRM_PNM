// src/ojk/permodalan-produk/permodalan-produk-ojk/permodalan-produk-ojk.service.tsx
...
import usePermodalanInherent from '../hooks/inherent/permodalan.hook';
...
export default function PermodalanInherentWrapper() {
  ...
  // PERBAIKAN: Gunakan usePermodalanInherent, bukan usePermodalanIntegration
  const { permodalan, parameters, loading, saving, error, currentPermodalanId, loadByYearQuarter, refreshData, addParameter, updateParameter, deleteParameter, copyParameter, addNilai, updateNilai, deleteNilai, copyNilai } =
    usePermodalanInherent();
  ...
  // PERBAIKAN: Data permodalan untuk info
  const currentPermodalanData = permodalan;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      loading,
      currentPermodalanId,
      currentPermodalanData: currentPermodalanData ? `ID: ${currentPermodalanData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentPermodalanId, currentPermodalanData, error]);
  ...
  const backendHandlers = useMemo(
    () => ({
      addAspek: async (createAspekDto) => {
        try {
          ...
          if (!currentPermodalanId) {
            throw new Error('Tidak ada data permodalan yang dipilih');
          }
          ...
          // Panggil API
          await addParameter(currentPermodalanId, createAspekDto);
      ...
      addPertanyaan: async (aspekId, createPertanyaanDto) => {
        try {
          ...
          await addNilai(currentPermodalanId, aspekId, createPertanyaanDto);
          await handleRefresh();
      ...
      copyAspek: async (aspekId) => {
        try {
          ...
          await copyParameter(currentPermodalanId, aspekId);
          await handleRefresh();
      ...
      copyPertanyaan: async (aspekId, pertanyaanId) => {
        try {
          ...
          await copyNilai(currentPermodalanId, aspekId, pertanyaanId);
          await handleRefresh();
    ...
    [currentPermodalanId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Permodalan - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentPermodalanId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentPermodalanId).substring(0, 8)}...</span>}
            </p>
            {permodalan?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(permodalan.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {permodalan?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{permodalan.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <PermodalanInherent rows={safeRows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={loading || saving || isRefreshing} isLocked={permodalan?.isLocked || false} />
    </div>
  );
}

// Komponen utama
function PermodalanInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data Permodalan - Inherent</h1>
