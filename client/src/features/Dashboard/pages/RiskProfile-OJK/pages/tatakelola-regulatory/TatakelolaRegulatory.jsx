...
async function exportInherentToExcel({
  ...
  year,
  quarter,
  categoryLabel = "TatakelolaRegulatory",
}) {
  if (!Array.isArray(rows) || rows.length === 0) {
...
export default function TatakelolaRegulatory() {
  const {
    year,
    activeQuarter,
    search,
  ...
  const CATEGORY_ID = "tatakelola-regulatory";
  ...
  const handleExportToExcel = useCallback(async () => {
    if (activeTab === "inherent") {
      await exportInherentToExcel({
        ...
        year,
        quarter: activeQuarter,
        categoryLabel: "Tatakelola Regulatory",
      });
  ...
  return (
    <div className="w-full space-y-4">
      <Header title="Risk Profile – Tata kelola" onExportClick={handleExportToExcel} showQuarterActions={true}/>
