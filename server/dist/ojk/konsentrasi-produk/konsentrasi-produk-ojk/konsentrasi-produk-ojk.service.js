...
var KonsentrasiProdukOjkService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KonsentrasiProdukOjkService = void 0;
...
let KonsentrasiProdukOjkService = KonsentrasiProdukOjkService_1 = class KonsentrasiProdukOjkService {
    inherentRepository;
    parameterRepository;
    nilaiRepository;
    referenceRepository;
    dataSource;
    logger = new common_1.Logger(KonsentrasiProdukOjkService_1.name);
    ...
    async create(createDto, userId) {
        try {
            ...
            this.logger.error(`Error creating KonsentrasiProdukOjk: ${error.message}`, error.stack);
            throw error;
        }
    }
    async findActive() {
    ...
    async updateActiveStatus(id, isActive, userId) {
        ...
        if (isActive) {
            ...
            await this.inherentRepository
                .createQueryBuilder()
                .update(konsentrasi_produk_ojk_entity_1.KonsentrasiProdukOjk)
                .set({ isActive: false })
                .execute();
        }
    ...
    async remove(id) {
        ...
        try {
            for (const parameter of inherent.parameters || []) {
                await queryRunner.manager.delete(konsentrasi_produk_nilai_entity_1.KonsentrasiNilai, {
                    parameterId: parameter.id,
                });
            }
            await queryRunner.manager.delete(konsentrasi_produk_paramter_entity_1.KonsentrasiParameter, {
                konsentrasiProdukOjkId: id,
            });
            await queryRunner.manager.delete(konsentrasi_produk_ojk_entity_1.KonsentrasiProdukOjk, { id });
    ...
    async addParameter(inherentId, createParamDto, userId) {
        ...
        if (createParamDto.kategori) {
            ...
            if (kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.OPEN_END) {
                if (!kategori.jenis) {
            ...
            if (kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR) {
                if (kategori.jenis) {
            ...
            if (kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.TANPA_MODEL) {
                if (kategori.prinsip ||
                    kategori.jenis ||
        ...
        const lastParam = await this.parameterRepository.findOne({
            where: { konsentrasiProdukOjkId: inherentId },
        ...
        const kategoriFormatted = createParamDto.kategori
            ? {
                ...
                prinsip: createParamDto.kategori.model !== konsentrasi_produk_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? createParamDto.kategori.prinsip
                    : undefined,
                jenis: createParamDto.kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.OPEN_END
                    ? createParamDto.kategori.jenis
                    : undefined,
                underlying: createParamDto.kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR
        ...
        try {
            const parameter = this.parameterRepository.create({
                ...
                konsentrasiProdukOjkId: inherentId,
    ...
    async updateParameter(inherentId, parameterId, updateParamDto, userId) {
        ...
        const parameter = await this.parameterRepository.findOne({
            where: { id: parameterId, konsentrasiProdukOjkId: inherentId },
        });
        if (!parameter) {
        ...
        if (updateParamDto.kategori) {
            ...
            parameter.kategori = {
                ...
                prinsip: kategori.model !== konsentrasi_produk_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? kategori.prinsip
                    : undefined,
                jenis: kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.OPEN_END
                    ? kategori.jenis
                    : undefined,
                underlying: kategori.model === konsentrasi_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR
                    ? kategori.underlying || []
                    : [],
            };
        }
    ...
    async reorderParameters(inherentId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.parameterIds.length; i++) {
                ...
                await queryRunner.manager.update(konsentrasi_produk_paramter_entity_1.KonsentrasiParameter, { id: parameterId, konsentrasiProdukOjkId: inherentId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async copyParameter(inherentId, parameterId, userId) {
        ...
        try {
            ...
            const savedParam = await queryRunner.manager.save(konsentrasi_produk_paramter_entity_1.KonsentrasiParameter, newParam);
            if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
                const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
                    ...
                    return queryRunner.manager.save(konsentrasi_produk_nilai_entity_1.KonsentrasiNilai, newNilai);
                });
    ...
    async addNilai(inherentId, parameterId, createNilaiDto, userId) {
        ...
        const nilai = {
            ...
            judul: {
                type: createNilaiDto.judul?.type || konsentrasi_produk_inherent_dto_1.JudulType.TANPA_FAKTOR,
                text: judulText.trim(),
    ...
    async updateNilai(inherentId, parameterId, nilaiId, updateNilaiDto, userId) {
        ...
        if (nilai.parameter.konsentrasiProdukOjkId !== inherentId) {
    ...
    async reorderNilai(parameterId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.nilaiIds.length; i++) {
                ...
                await queryRunner.manager.update(konsentrasi_produk_nilai_entity_1.KonsentrasiNilai, { id: nilaiId, parameterId: parameterId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async validateModelTerstruktur(inherentId) {
        ...
        const terstrukturParams = inherent.parameters?.filter((param) => param.kategori?.model === konsentrasi_produk_inherent_dto_1.KategoriModel.TERSTRUKTUR) || [];
    ...
    async importFromExcel(importData, userId) {
        ...
        await queryRunner.startTransaction();
        try {
            await queryRunner.manager.update(konsentrasi_produk_ojk_entity_1.KonsentrasiProdukOjk, { isActive: true }, { isActive: false });
            const inherent = {
            ...
            const savedInherent = await queryRunner.manager.save(konsentrasi_produk_ojk_entity_1.KonsentrasiProdukOjk, inherent);
            for (let i = 0; i < importData.parameters.length; i++) {
                ...
                const parameter = {
                    ...
                    konsentrasiProdukOjkId: savedInherent.id,
                    orderIndex: paramData.orderIndex || i,
                };
                const savedParam = await queryRunner.manager.save(konsentrasi_produk_paramter_entity_1.KonsentrasiParameter, parameter);
                    ...
                    for (let j = 0; j < paramData.nilaiList.length; j++) {
                        ...
                        const nilai = {
                            ...
                            judul: nilaiData.judul || {
                                type: konsentrasi_produk_inherent_dto_1.JudulType.TANPA_FAKTOR,
                                text: '',
                                value: null,
                                pembilang: '',
                                valuePembilang: null,
                        ...
                        await queryRunner.manager.save(konsentrasi_produk_nilai_entity_1.KonsentrasiNilai, nilai);
                    }
                }
            }
            await queryRunner.commitTransaction();
...
exports.KonsentrasiProdukOjkService = KonsentrasiProdukOjkService;
exports.KonsentrasiProdukOjkService = KonsentrasiProdukOjkService = KonsentrasiProdukOjkService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(konsentrasi_produk_ojk_entity_1.KonsentrasiProdukOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(konsentrasi_produk_paramter_entity_1.KonsentrasiParameter)),
    __param(2, (0, typeorm_1.InjectRepository)(konsentrasi_produk_nilai_entity_1.KonsentrasiNilai)),
    __param(3, (0, typeorm_1.InjectRepository)(konsentrasi_inherent_references_entity_1.InherentReferenceKonsentrasi)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KonsentrasiProdukOjkService);
//# sourceMappingURL=konsentrasi-produk-ojk.service.js.map