...
let KpmrOperasionalService = KpmrOperasionalService_1 = class KpmrOperasionalService {
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(operasional_kpmr_aspek_entity_1.KpmrAspekOperasional, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(operasional_kpmr_pertanyaan_entity_1.KpmrPertanyaanOperasional, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        if (!kpmr.id) {
            throw new common_1.InternalServerErrorException('KPMR tidak memiliki ID');
        }
        const frontendKpmr = {
            id: kpmr.id.toString(),
            year: kpmr.year,
        ...
        try {
            return (0, class_transformer_1.plainToInstance)(operasional_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
        }
        catch (error) {
            this.logger.error(`❌ Error converting to DTO: ${error.message}`);
            throw new common_1.InternalServerErrorException('Gagal mengkonversi data KPMR');
        }
    }
    async createKpmr(createDto, createdBy = 'system') {
        ...
        try {
            ...
            if (!kpmrWithRelations) {
                throw new common_1.InternalServerErrorException('Gagal mengambil data KPMR setelah create');
            }
            this.logger.log(`📊 After create - aspekList length: ${kpmrWithRelations.aspekList?.length || 0}`);
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(operasional_kpmr_dto_1.FrontendAspekResponseDto, {
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(operasional_kpmr_aspek_entity_1.KpmrAspekOperasional, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(operasional_kpmr_dto_1.FrontendPertanyaanResponseDto, {
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(operasional_kpmr_pertanyaan_entity_1.KpmrPertanyaanOperasional)
                    .findOne({
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(operasional_kpmr_pertanyaan_entity_1.KpmrPertanyaanOperasional, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(operasional_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrOperasionalService = KpmrOperasionalService = KpmrOperasionalService_1 = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(operasional_kpmr_ojk_entity_1.KpmrOperasionalOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(operasional_kpmr_aspek_entity_1.KpmrAspekOperasional)),
    __param(2, (0, typeorm_1.InjectRepository)(operasional_kpmr_pertanyaan_entity_1.KpmrPertanyaanOperasional)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KpmrOperasionalService);
//# sourceMappingURL=operasional-kpmr-ojk.service.js.map