...
export function exportInvestasiToExcel(filteredRows, viewYear, viewQuarter) {
    const headers1 = [
        "No", "Bobot", "Parameter atau Indikator", "", "", "Bobot",
        "Sumber Risiko", "Dampak", "Low", "", "Moderate", "Moderate to High", "High",
    ...
    sorted.forEach((r) => {
        const weightedVal = r.weighted === "" ? "" : Number(r.weighted);
        ...
        dataRows.push([
            ...
            r.sumberRisiko,
            r.dampak,
            r.low, r.lowToModerate, r.moderate, r.moderateToHigh, r.high,
        ...
        // pembilang
        dataRows.push([
            "", "", "", "", String(r.numeratorLabel || "Pembilang"), "", "", "", "", "", "", "", "",
            r.numeratorValue === "" ? "" : Number(r.numeratorValue || 0),
            "", "", "", ""
        ]);
        // penyebut
        dataRows.push([
            "", "", "", "", String(r.denominatorLabel || "Penyebut"), "", "", "", "", "", "", "", "",
            r.denominatorValue === "" ? "" : Number(r.denominatorValue || 0),
            "", "", "", ""
        ]);
    });
        ...
        for (let c = 0; c <= 16; c++) {
            ...
            if (c === 14) {
                ...
                if (hasValue) {
                    ...
                    let bg = null, fg = "#000";
                    if (v === 1) { bg = COLORS.darkGreen; fg = "#fff"; }
                    else if (v === 2) { bg = COLORS.lightGreen; }
                    else if (v === 3) { bg = COLORS.yellow; }
                    else if (v === 4) { bg = COLORS.orange; }
                    else if (v === 5) { bg = COLORS.red; fg = "#fff"; }
                    if (bg) {
    ...
    if (ws[valAddr]) { ws[valAddr].t = "n"; ws[valAddr].z = "0.00"; }
