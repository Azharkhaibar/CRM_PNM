...
import useOperasional from '../hooks/operational/operasional.hook.js';
...
// Transform functions untuk operasional
const transformIndicatorToBackend = (indicatorData, year, quarter, sectionId) => {
  ...
  return {
    ...
    // Hasil dihitung otomatis di backend, tidak perlu dikirim
...
// ===================== FUNGSI KALKULASI SAMA PERSIS DENGAN ORIGIN =====================
...
// total baris per indikator (untuk rowSpan section) - SAMA DENGAN ORIGIN
const rowsPerIndicator = (ind) => {
  return 1 + (ind.mode === 'RASIO' ? 2 : 1);
};

export default function OperationalTab({ viewYear, setViewYear, viewQuarter, setViewQuarter, query, setQuery }) {
  // ====== Hooks ======
  const { user: authUser } = useAuth();
  const { logUpdate, logDelete, logExport, logCreate } = useAuditLog();
  const { loading, error, getSectionsByPeriod, getAllSections, createSection, updateSection, deleteSection, createIndikator, updateIndikator, deleteIndikator, getSummaryByPeriod, calculateHasil, calculateWeighted, clearError } =
    useOperasional();
  ...
  const loadData = useCallback(async () => {
    console.log('🔄 [OPERASIONAL LOAD] Starting to load data...', viewYear, viewQuarter);
    ...
    try {
      ...
      console.log('📥 [OPERASIONAL LOAD] Raw data length:', sectionsData.length);
      ...
      console.log('✅ [OPERASIONAL LOAD] Setting new sections:', transformedSections.length);
      setSections([...transformedSections]);
    } catch (err) {
      console.error('❌ [OPERASIONAL LOAD] Error:', err);
      setLocalError(err.message || 'Gagal memuat data operasional');
    } finally {
      setLoadingData(false);
      console.log('🏁 [OPERASIONAL LOAD] Finished');
    }
  ...
  const loadAllSections = useCallback(async () => {
    ...
    try {
      console.log('🔄 [OPERASIONAL] Loading all sections from database...');
      ...
      console.log('✅ [OPERASIONAL] Total sections in database:', transformedAllSections.length);
      setAllSections(transformedAllSections);
    } catch (err) {
      console.error('❌ [OPERASIONAL] Error loading all sections:', err);
      // Tidak set error state agar tidak mengganggu UI utama
    } finally {
  ...
  useEffect(() => {
    loadData();
    loadAllSections(); // Load semua section saat komponen mount
  ...
  // Effect untuk auto-calculate - SAMA DENGAN ORIGIN
  useEffect(() => {
    if (sectionForm.id) {
  ...
  const getUniqueSections = useMemo(() => {
    ...
    return Array.from(uniqueMap.values()).sort((a, b) => {
      // Coba parsing sebagai angka untuk sorting yang lebih baik
  ...
  const getUnusedSections = useMemo(() => {
    ...
    return getUniqueSections.filter(
      (section) =>
        ...
        // Jika section tidak punya ID (belum disimpan), cek berdasarkan no dan nama
  ...
  const handleAuditLog = async (action, description, isSuccess = true, metadata = {}) => {
    try {
      ...
      switch (action) {
        case 'CREATE':
          await logCreate('OPERASIONAL', description, {
            isSuccess,
            userId: userId || null,
            metadata: {
              year: viewYear,
          ...
          break;
        case 'UPDATE':
          await logUpdate('OPERASIONAL', description, {
            isSuccess,
            userId: userId || null,
            metadata: {
              year: viewYear,
          ...
          break;
        case 'DELETE':
          await logDelete('OPERASIONAL', description, {
            isSuccess,
            userId: userId || null,
            metadata: {
              year: viewYear,
          ...
          break;
        case 'EXPORT':
          await logExport('OPERASIONAL', description, {
            isSuccess,
            userId: userId || null,
            metadata: {
              year: viewYear,
      ...
      console.error('❌ [OPERASIONAL AUDIT] Audit failed:', error);
    }
  };
  ...
  const handleEditIndicator = (indikator) => {
    console.log('📝 [OPERASIONAL EDIT] HandleEditIndicator:', indikator);
    ...
    console.log('📝 [OPERASIONAL EDIT] Setting form data:', formData);
  ...
  const handleCreateSection = async () => {
    ...
    try {
      ...
      handleAuditLog('CREATE', `Menambahkan section operasional - No: ${sectionForm.no}, Nama: "${sectionForm.parameter}", Bobot: ${sectionForm.bobotSection}%`, true, {
      ...
      handleAuditLog('CREATE', `Gagal menambah section operasional - No: ${sectionForm.no}, Nama: "${sectionForm.parameter}"`, false, {
        section_no: sectionForm.no,
        section_name: sectionForm.parameter,
        error: err.message,
      });
  ...
  const handleUpdateSection = async () => {
    ...
    try {
      ...
      handleAuditLog('UPDATE', `Mengupdate section operasional - No: ${sectionForm.no}, Nama: "${sectionForm.parameter}", Bobot: ${sectionForm.bobotSection}%`, true, {
      ...
      handleAuditLog('UPDATE', `Gagal update section operasional - No: ${sectionForm.no}, Nama: "${sectionForm.parameter}"`, false, {
        section_id: sectionForm.id,
        section_no: sectionForm.no,
        section_name: sectionForm.parameter,
        error: err.message,
  ...
  const handleDeleteSection = async (sectionId) => {
    ...
    try {
      ...
      handleAuditLog('DELETE', `Menghapus section operasional - No: ${sectionToDelete?.no}, Nama: "${sectionToDelete?.parameter}"`, true, {
      ...
      handleAuditLog('DELETE', `Gagal menghapus section operasional - No: ${sectionToDelete?.no}, Nama: "${sectionToDelete?.parameter}"`, false, {
        section_id: sectionId,
        section_no: sectionToDelete?.no,
        section_name: sectionToDelete?.parameter,
        error: err.message,
  ...
  const handleCreateIndicator = async () => {
    ...
    try {
      ...
      handleAuditLog('CREATE', `Menambahkan indikator operasional - "${indicatorForm.indikator}", Bobot: ${indicatorForm.bobotIndikator}%`, true, {
        section_id: sectionForm.id,
        section_no: sectionForm.no,
      ...
      handleAuditLog('CREATE', `Gagal menambah indikator operasional - "${indicatorForm.indikator}"`, false, {
        section_id: sectionForm.id,
        section_no: sectionForm.no,
        indicator_name: indicatorForm.indikator,
        error: err.message,
  ...
  const handleUpdateIndicator = async () => {
    console.log('🔄 [OPERASIONAL UPDATE] Starting update for indicator ID:', indicatorForm.id);
    ...
    try {
      ...
      console.log('📤 [OPERASIONAL UPDATE] Data untuk update:', updateData);

      await updateIndikator(indicatorForm.id, updateData);

      console.log('✅ [OPERASIONAL UPDATE] API call successful');

      resetIndicatorForm();

      setTimeout(async () => {
        console.log('🔄 [OPERASIONAL UPDATE] Reloading data...');
        await loadData();
        console.log('✅ [OPERASIONAL UPDATE] Data reloaded');

        alert('✅ Data berhasil diupdate!');
      }, 300);
    } catch (err) {
      console.error('❌ [OPERASIONAL UPDATE] Error updating:', err);
  ...
  const handleDeleteIndicator = async (indikatorId) => {
    ...
    try {
      ...
      handleAuditLog('DELETE', `Menghapus indikator operasional - "${indicatorToDelete.indikator}"`, true, {
      ...
      handleAuditLog('DELETE', `Gagal menghapus indikator operasional - "${indicatorToDelete.indikator}"`, false, {
        indicator_id: indikatorId,
        section_id: sectionForm.id,
        section_no: sectionForm.no,
        indicator_name: indicatorToDelete.indikator,
  ...
  const handleExportOperasional = () => {
    ...
    try {
      ...
      handleAuditLog('EXPORT', `Export data Excel operasional - Periode: ${viewYear}-${viewQuarter}, Section: ${selectedSection.no}, Jumlah Data: ${rows.length}`, true, {
        year: viewYear,
        quarter: viewQuarter,
      ...
      handleAuditLog('EXPORT', `Gagal export data Excel operasional - Periode: ${viewYear}-${viewQuarter}`, false, {
        year: viewYear,
        quarter: viewQuarter,
        section_id: selectedSection?.id,
        section_no: selectedSection?.no,
  ...
  return (
    <>
      <header className="px-4 py-4 flex items-center justify-between gap-3">
        <h2 className="text-xl sm:text-2xl font-semibold">Form – Risiko Operasional</h2>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 transform -translate-y-1">
            ...
            <button
              onClick={handleExportOperasional}
      ...
      <div className="relative rounded-2xl overflow-hidden mb-6 shadow-sm bg-gradient-to-r from-[#0076C6]/90 via-[#00A3DA]/90 to-[#33C2B5]/90">
        <div className="rounded-2xl border-2 border-gray-300 p-4 shadow-sm mb-6">
          <h2 className="text-white font-semibold text-lg sm:text-xl mb-6">Form Section – Operasional</h2>
          ...
          <div className="rounded-xl border-2 border-gray-300 shadow-sm p-6 mb-6">
            ...
            <div className="grid grid-cols-12 gap-4 mb-4">
              <div className="col-span-3">
                ...
                <input className="w-full rounded-lg border-2 border-gray-300 px-3 py-3 text-sm bg-white" value={indicatorForm.subNo} onChange={(e) => setIndicatorField('subNo', e.target.value)} placeholder="4.1.1" />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  placeholder={indicatorForm.mode === 'RASIO' ? 'Pendapatan operasional (juta rupiah)' : 'Jumlah kejadian, jumlah kasus, dll.'}
                />
              </div>
              <div>
      ...
      {/* table area - BAGIAN INI YANG DIPERBAIKI */}
                ...
                <tbody>
                  {loadingData ? (
                    ...
                    filteredSections.map((s) => {
                      ...
                      // ====== PERBAIKAN UTAMA: Hitung sectionRowSpan seperti origin ======
                      ...
                      return (
                        <React.Fragment key={s.id}>
                          {inds.map((it, idx) => {
                            ...
                            // ====== PERBAIKAN: Format hasil display seperti origin ======
                            const hasilDisplay = (() => {
                              ...
                              if (transformed.mode === 'RASIO') {
                                ...
                                // NILAI_TUNGGAL: di origin kosong di baris utama
                                return ''; // Kosongkan untuk NILAI_TUNGGAL
                              }
                            })();
                            ...
                            return (
                              <React.Fragment key={transformed.id}>
                                {/* BARIS UTAMA INDIKATOR - SAMA PERSIS DENGAN ORIGIN */}
                                <tr>
                                  {firstOfSection && (
                                    <>
                                  ...
                                  {/* Hasil: untuk RASIO berisi rasio, untuk NILAI_TUNGGAL kosong */}
