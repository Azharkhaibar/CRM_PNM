// src/ojk/strategis-produk/strategis-produk-ojk/strategis-produk-ojk.service.tsx
...
import useStrategisInherent from '../hooks/inherent/strategis.hook';
...
export default function StrategisInherentWrapper() {
  ...
  // PERBAIKAN: Gunakan useStrategisInherent, bukan useStrategisIntegration
  const { strategis, parameters, loading, saving, error, currentStrategisId, loadByYearQuarter, refreshData, addParameter, updateParameter, deleteParameter, copyParameter, addNilai, updateNilai, deleteNilai, copyNilai } =
    useStrategisInherent();
  ...
  // PERBAIKAN: Data strategis untuk info
  const currentStrategisData = strategis;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      loading,
      currentStrategisId,
      currentStrategisData: currentStrategisData ? `ID: ${currentStrategisData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentStrategisId, currentStrategisData, error]);
  ...
  const backendHandlers = useMemo(
    () => ({
      addAspek: async (createAspekDto) => {
        try {
          ...
          if (!currentStrategisId) {
            throw new Error('Tidak ada data strategis yang dipilih');
          }
          ...
          // Panggil API
          await addParameter(currentStrategisId, createAspekDto);
      ...
      addPertanyaan: async (aspekId, createPertanyaanDto) => {
        try {
          ...
          await addNilai(currentStrategisId, aspekId, createPertanyaanDto);
          await handleRefresh();
      ...
      copyAspek: async (aspekId) => {
        try {
          ...
          await copyParameter(currentStrategisId, aspekId);
          await handleRefresh();
      ...
      copyPertanyaan: async (aspekId, pertanyaanId) => {
        try {
          ...
          await copyNilai(currentStrategisId, aspekId, pertanyaanId);
          await handleRefresh();
    ...
    [currentStrategisId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Strategis - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentStrategisId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentStrategisId).substring(0, 8)}...</span>}
            </p>
            {strategis?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(strategis.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {strategis?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{strategis.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <StrategisInherent rows={safeRows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={loading || saving || isRefreshing} isLocked={strategis?.isLocked || false} />
    </div>
  );
}

// Komponen utama
function StrategisInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
