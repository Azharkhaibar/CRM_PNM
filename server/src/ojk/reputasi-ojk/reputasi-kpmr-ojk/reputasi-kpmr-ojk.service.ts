...
export class KpmrReputasiService {
  ...
  private async getAspekWithRelations(id: number): Promise<KpmrAspekReputasi> {
    ...
    try {
      await this.dataSource.transaction(async (manager) => {
        for (let i = 0; i < reorderDto.aspekIds.length; i++) {
          await manager.update(KpmrAspekReputasi, reorderDto.aspekIds[i], {
            orderIndex: i,
          });
        }
      });
