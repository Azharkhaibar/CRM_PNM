...
export interface RentabilitasOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
...
export interface ParameterEntity {
  ...
  orderIndex: number;
  rentabilitasId: number; // ✅ BUKAN kpmrRentabilitasId
  createdAt: Date;
  updatedAt: Date;
}
...
export interface CreateRentabilitasDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateRentabilitasDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class RentabilitasService {
  private baseUrl = '/rentabilitas-ojk';
  ...
  async findOrCreate(
    ...
    success: boolean;
    data: RentabilitasOjkEntity | null;
    isNew: boolean;
    message: string;
  }> {
    ...
    try {
      ...
      const createDto: CreateRentabilitasDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
   ...
   */
  async ensureDataExists(year: number, quarter: number): Promise<RentabilitasOjkEntity> {
   ...
   */
  async loadOrCreateData(year: number, quarter: number): Promise<RentabilitasOjkEntity> {
   ...
   */
  private formatToFrontend(entity: RentabilitasOjkEntity | null): any[] {
    ...
    const result = parameters.map((parameter, index) => {
      ...
      const formattedParameter = {
        ...
        // Metadata dari rentabilitas
        metadata: {
          rentabilitasId: entity.id,
          year: entity.year,
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[RentabilitasService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<RentabilitasOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_rentabilitas_produk.get<RentabilitasOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<RentabilitasOjkEntity | null> {
  ...
  async getById(id: number): Promise<RentabilitasOjkEntity> {
  ...
  async create(createDto: CreateRentabilitasDto): Promise<RentabilitasOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateRentabilitasDto): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateRentabilitasDto['summary']): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_rentabilitas_produk.put<RentabilitasOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async getParameters(rentabilitasId: number): Promise<ParameterEntity[]> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters`;
    this.logApiCall('GET', url);

    try {
      const response = await api_rentabilitas_produk.get<ParameterEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addParameter(rentabilitasId: number, dto: CreateParameterDto): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters`;

    console.log('[Service] addParameter:', { url, rentabilitasId, dto });
    ...
    try {
      ...
      const response = await api_rentabilitas_produk.post<RentabilitasOjkEntity>(url, payload);
  ...
  async updateParameter(rentabilitasId: number, parameterId: number, dto: UpdateParameterDto): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}`;
    ...
    try {
      ...
      const response = await api_rentabilitas_produk.put<RentabilitasOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(rentabilitasId: number, parameterId: number): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(rentabilitasId: number, parameterId: number): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_rentabilitas_produk.delete<RentabilitasOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(rentabilitasId: number, parameterIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/reorder`;
  ...
  async getNilai(rentabilitasId: number, parameterId: number): Promise<NilaiEntity[]> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai`;
    this.logApiCall('GET', url);

    try {
      const response = await api_rentabilitas_produk.get<NilaiEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addNilai(rentabilitasId: number, parameterId: number, dto: CreateNilaiDto): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai`;
  ...
  async updateNilai(rentabilitasId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai/${nilaiId}`;
  ...
  async copyNilai(rentabilitasId: number, parameterId: number, nilaiId: number): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai/${nilaiId}/copy`;
    this.logApiCall('POST', url);
  ...
  async removeNilai(rentabilitasId: number, parameterId: number, nilaiId: number): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(rentabilitasId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${rentabilitasId}/parameters/${parameterId}/nilai/reorder`;
  ...
  async exportToExcel(rentabilitasId: number): Promise<any> {
    const url = `${this.baseUrl}/${rentabilitasId}/export`;
    this.logApiCall('GET', url);
  ...
  async importFromExcel(importData: any): Promise<RentabilitasOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_rentabilitas_produk.post<RentabilitasOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: RentabilitasOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<RentabilitasOjkEntity> {
    ...
    try {
      ...
      const fallbackData: RentabilitasOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: RentabilitasOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_rentabilitas_produk.get(this.baseUrl, {
        timeout: 5000,
      });
...
export const rentabilitasService = new RentabilitasService();
export default rentabilitasService;
