// src/ojk/tatakelola-produk/tatakelola-produk-ojk/tatakelola-produk-ojk.service.tsx
...
import useTataKelolaInherent from '../hooks/inherent/tatakelola.hook';
...
export default function TataKelolaInherentWrapper() {
  ...
  // PERBAIKAN: Gunakan useTataKelolaInherent, bukan useTataKelolaIntegration
  const { tatakelola, parameters, loading, saving, error, currentTataKelolaId, loadByYearQuarter, refreshData, addParameter, updateParameter, deleteParameter, copyParameter, addNilai, updateNilai, deleteNilai, copyNilai } =
    useTataKelolaInherent();
  ...
  // PERBAIKAN: Data tatakelola untuk info
  const currentTataKelolaData = tatakelola;
  ...
  useEffect(() => {
    log.debug('State changed', {
      ...
      loading,
      currentTataKelolaId,
      currentTataKelolaData: currentTataKelolaData ? `ID: ${currentTataKelolaData.id}` : 'null',
      error,
    });
  }, [year, quarter, isInitialLoading, localRows, loading, currentTataKelolaId, currentTataKelolaData, error]);
  ...
  const backendHandlers = useMemo(
    () => ({
      addAspek: async (createAspekDto) => {
        try {
          ...
          if (!currentTataKelolaId) {
            throw new Error('Tidak ada data tatakelola yang dipilih');
          }
          ...
          // Panggil API
          await addParameter(currentTataKelolaId, createAspekDto);
      ...
      addPertanyaan: async (aspekId, createPertanyaanDto) => {
        try {
          ...
          await addNilai(currentTataKelolaId, aspekId, createPertanyaanDto);
          await handleRefresh();
      ...
      copyAspek: async (aspekId) => {
        try {
          ...
          await copyParameter(currentTataKelolaId, aspekId);
          await handleRefresh();
      ...
      copyPertanyaan: async (aspekId, pertanyaanId) => {
        try {
          ...
          await copyNilai(currentTataKelolaId, aspekId, pertanyaanId);
          await handleRefresh();
    ...
    [currentTataKelolaId, localRows, addParameter, updateParameter, deleteParameter, addNilai, updateNilai, deleteNilai, copyParameter, copyNilai, handleRefresh, loadByYearQuarter],
  );
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-lg border shadow p-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Tata Kelola - Inherent Risk</h1>
            <p className="text-gray-600">
              Tahun: <span className="font-semibold">{year}</span> | Quarter: <span className="font-semibold">Q{quarter}</span>
              {currentTataKelolaId && <span className="ml-4 text-xs bg-gray-100 px-2 py-1 rounded">ID: {String(currentTataKelolaId).substring(0, 8)}...</span>}
            </p>
            {tatakelola?.isLocked && (
              <p className="text-sm text-amber-600 mt-1">
                <TriangleAlert className="w-4 h-4 inline mr-1" />
                Data terkunci pada {new Date(tatakelola.lockedAt).toLocaleDateString()}
              </p>
            )}
          </div>
        ...
        <div className="rounded-lg p-3 bg-blue-50 border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm text-blue-800">
              ...
              {tatakelola?.summary?.totalWeighted && (
                <span className="ml-2">
                  | Total Weighted: <span className="font-bold">{tatakelola.summary.totalWeighted.toFixed(2)}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* Komponen utama */}
      <TataKelolaInherent rows={safeRows} setRows={setRows} search={search} active={active} backendHandlers={backendHandlers} isLoading={loading || saving || isRefreshing} isLocked={tatakelola?.isLocked || false} />
    </div>
  );
}

// Komponen utama
function TataKelolaInherent({ rows, setRows, search, active, backendHandlers, isLoading, isLocked = false }) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-2 pr-2">
        <div>
          <h1 className="text-2xl font-semibold">Data TataKelola - Inherent</h1>
