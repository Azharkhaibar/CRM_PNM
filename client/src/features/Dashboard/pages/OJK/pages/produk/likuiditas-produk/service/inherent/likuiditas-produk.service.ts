// services/likuiditas-produk.service.ts
...
export interface CreateLikuiditasParameterDto {
  nomor?: string;
  judul: string;
  bobot: number;
  kategori?: {
...
export interface UpdateLikuiditasParameterDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  kategori?: {
...
export interface CreateLikuiditasNilaiDto {
  nomor?: string;
  judul: {
    type?: string;
    text?: string;
...
export interface UpdateLikuiditasNilaiDto {
  nomor?: string;
  judul?: {
    type?: string;
    text?: string;
...
export class LikuiditasProdukService {
  private baseUrl = '/likuiditas-produk-ojk';
   ...
   */
  private formatToFrontend(entity: LikuiditasProdukOjkEntity | null): any[] {
  ...
  async addParameter(inherentId: number, dto: CreateLikuiditasParameterDto): Promise<LikuiditasProdukOjkEntity> {
    ...
    try {
      ...
      const payload: CreateLikuiditasParameterDto = {
        ...dto,
  ...
  async updateParameter(inherentId: number, parameterId: number, dto: UpdateLikuiditasParameterDto): Promise<LikuiditasProdukOjkEntity> {
    const url = `${this.baseUrl}/${inherentId}/parameters/${parameterId}`;

    try {
      // Format payload
      const payload: UpdateLikuiditasParameterDto = { ...dto };
  ...
  async addNilai(inherentId: number, parameterId: number, dto: CreateLikuiditasNilaiDto): Promise<LikuiditasProdukOjkEntity> {
  ...
  async updateNilai(inherentId: number, parameterId: number, nilaiId: number, dto: UpdateLikuiditasNilaiDto): Promise<LikuiditasProdukOjkEntity> {
   ...
   */
  formatNilaiJudul(judul: any): CreateLikuiditasNilaiDto['judul'] {
    if (!judul) {
      return {
        type: 'Tanpa Faktor',
        text: '',
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_likuiditas_produk.get(this.baseUrl, {
        timeout: 5000,
      });
