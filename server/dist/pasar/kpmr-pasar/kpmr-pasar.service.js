...
let KPMRPasarService = KPMRPasarService_1 = class KPMRPasarService {
    definitionRepo;
    scoreRepo;
    aspectRepo;
    questionRepo;
    logger = new common_1.Logger(KPMRPasarService_1.name);
    ...
    async deleteAspect(id) {
        ...
        this.logger.log(`✅ Aspect ID ${id} and all related data deleted (cascade)`);
        return {
            success: true,
    ...
    async createQuestion(createDto) {
        ...
        if (!aspect) {
            throw new common_1.NotFoundException(`Aspek dengan nomor "${createDto.aspekNo}" untuk tahun ${createDto.year} tidak ditemukan. Silakan buat aspek terlebih dahulu.`);
        }
        try {
    ...
    async deleteQuestion(id) {
        const question = await this.findQuestionById(id);
        this.logger.log(`🗑️ Deleting question ID ${id} (${question.sectionNo} - ${question.sectionTitle}) for year ${question.year}`);
        ...
        this.logger.log(`✅ Question ID ${id} and ${definitionsCount} definitions permanently deleted`);
        return {
            success: true,
    ...
    async createOrUpdateDefinition(createDto, createdBy) {
        ...
        if (!question) {
            throw new common_1.NotFoundException(`Pertanyaan dengan aspek ${createDto.aspekNo} dan section ${createDto.sectionNo} untuk tahun ${createDto.year} tidak ditemukan`);
        }
    ...
    async updateDefinition(id, updateDto, updatedBy) {
        ...
        if (updateDto.aspekNo || updateDto.sectionNo) {
            ...
            if (existing) {
                throw new common_1.ConflictException(`Definition dengan aspek ${checkAspekNo} dan section ${checkSectionNo} sudah ada untuk tahun ${definition.year}`);
            }
        }
    ...
    async deleteDefinition(definitionId, year) {
        ...
        try {
            ...
            if (!definition) {
                this.logger.warn(`Definition ${definitionId} for year ${year} not found`);
                return {
                    success: true,
    ...
    async deleteScoreByTarget(definitionId, year, quarter) {
        this.logger.log(`Deleting score: definitionId=${definitionId}, year=${year}, quarter=${quarter}`);
        this.validateQuarter(quarter);
        const score = await this.scoreRepo.findOne({
            where: {
                definitionId,
        ...
        for (const aspek of result) {
            ...
            ['Q1', 'Q2', 'Q3', 'Q4'].forEach((quarter) => {
                ...
                quarterAverages[quarter] = allScores.length
                    ? Number((allScores.reduce((a, b) => a + b, 0) /
                        allScores.length).toFixed(2))
                    : null;
            });
...
exports.KPMRPasarService = KPMRPasarService = KPMRPasarService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(kpmr_pasar_definisi_entity_1.KPMRPasarDefinition)),
    __param(1, (0, typeorm_1.InjectRepository)(kpmr_pasar_skor_entity_1.KPMRPasarScore)),
    __param(2, (0, typeorm_1.InjectRepository)(kpmr_pasar_aspek_entity_1.KPMRPasarAspect)),
    __param(3, (0, typeorm_1.InjectRepository)(kpmr_pasar_pertanyaan_entity_1.KPMRPasarQuestion)),
