...
export function exportInherent({
  rows = [],
  year,
  quarter,
  ...
  subHeaders.forEach((label, i) => {
    ...
    const style =
      riskStyles[label] ??
      (["Hasil", "Peringkat", "Weighted", "Keterangan"].includes(label)
        ? headerBlueDark
        : headerBlue);
  ...
  rows.forEach((row, i) => {
    ...
    const nilai =
      Array.isArray(row.nilaiList) && row.nilaiList.length > 0
        ? row.nilaiList[0]
        : null;
    ...
    const values = [
      ...
      String(nilai?.nomor ?? ""),
      nilai
        ? resolveNilaiLabel(nilai.judul ?? nilai.nilai)
        : "Belum ada nilai",
  ...
  ws["!cols"] = [
    { wch: 6 }, { wch: 8 }, { wch: 32 },
    { wch: 6 }, { wch: 32 }, { wch: 8 }, { wch: 18 },
    { wch: 16 }, { wch: 18 }, { wch: 16 }, { wch: 20 }, { wch: 16 },
  ...
  saveAs(
    ...
    `Inherent_${categoryLabel}_${year}_${quarter.toUpperCase()}.xlsx`
  );
}
