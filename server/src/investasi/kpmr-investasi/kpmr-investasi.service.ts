...
@Injectable()
export class KPMRService {
  private readonly logger = new Logger(KPMRService.name);

  constructor(
    @InjectRepository(KPMRDefinition)
    private readonly definitionRepo: Repository<KPMRDefinition>,

    @InjectRepository(KPMRScore)
    private readonly scoreRepo: Repository<KPMRScore>,

    @InjectRepository(KPMRAspect)
    private readonly aspectRepo: Repository<KPMRAspect>,

    @InjectRepository(KPMRQuestion)
    private readonly questionRepo: Repository<KPMRQuestion>,
  ) {}
  ...
  // ========== ASPECT SERVICES ==========
  async createAspect(createDto: CreateKPMRAspectDto): Promise<KPMRAspect> {
    this.logger.log(
  ...
  async findAllAspects(year?: number): Promise<KPMRAspect[]> {
    const where: any = {};
    if (year) {
      where.year = year;
    }
  ...
  async findAspectById(id: number): Promise<KPMRAspect> {
  ...
  async createQuestion(
    createDto: CreateKPMRQuestionDto,
  ): Promise<KPMRQuestion> {
    this.logger.log(
  ...
  async findAllQuestions(year?: number): Promise<KPMRQuestion[]> {
    const where: any = {};
    if (year) {
      where.year = year;
    }
  ...
  async findQuestionById(id: number): Promise<KPMRQuestion> {
  ...
  async updateQuestion(
    id: number,
    updateDto: UpdateKPMRQuestionDto,
  ): Promise<KPMRQuestion> {
  ...
  async createOrUpdateDefinition(
    createDto: CreateKPMRDefinitionDto,
    createdBy?: string,
  ): Promise<KPMRDefinition> {
    this.logger.log(
  ...
  async findAllDefinitions(): Promise<KPMRDefinition[]> {
  ...
  async findDefinitionsByYear(year: number): Promise<KPMRDefinition[]> {
    return await this.definitionRepo.find({
      where: { year },
      relations: ['question', 'scores'],
  ...
  async findDefinitionById(id: number): Promise<KPMRDefinition> {
    const definition = await this.definitionRepo.findOne({
      where: { id },
      relations: ['question', 'scores'],
    });
  ...
  async updateDefinition(
    id: number,
    updateDto: UpdateKPMRDefinitionDto,
    updatedBy?: string,
  ): Promise<KPMRDefinition> {
  ...
  async findAllScores(): Promise<KPMRScore[]> {
  ...
  async findScoresByDefinition(definitionId: number): Promise<KPMRScore[]> {
  ...
  async findScoreById(id: number): Promise<KPMRScore> {
