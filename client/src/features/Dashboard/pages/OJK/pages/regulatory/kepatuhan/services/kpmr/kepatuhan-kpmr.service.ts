...
// ========== CREATE DTOs ==========
export interface CreateKpmrKepatuhanOjkDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface CreateKpmrAspekKepatuhanDto {
  nomor?: string;
  judul: string;
  bobot: number;
  deskripsi?: string;
...
export interface CreateKpmrPertanyaanKepatuhanDto {
  nomor?: string;
  pertanyaan: string;
  skor?: KpmrSkorDto;
...
// ========== UPDATE DTOs ==========
export interface UpdateKpmrAspekKepatuhanDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  deskripsi?: string;
...
export interface UpdateKpmrPertanyaanKepatuhanDto {
  nomor?: string;
  pertanyaan?: string;
  skor?: KpmrSkorDto;
  indicator?: KpmrIndicatorDto;
...
class KpmrKepatuhanApiService {
  private readonly BASE_PATH = '/kpmr-kepatuhan';
  ...
  private handleApiError(error: any, defaultMessage: string): never {
    console.error(`❌ [KPMR Kepatuhan Service] Error:`, error);
  ...
  async createKpmr(data: CreateKpmrKepatuhanOjkDto): Promise<FrontendKpmrResponse> {
    try {
      console.log(`🆕 [KPMR Kepatuhan Service] Creating KPMR:`, data);
      ...
      console.log('📦 [KPMR Kepatuhan Service] Sending payload:', payload);

      const response = await api_kepatuhan_produk.post(this.BASE_PATH, payload);

      if (!response || !response.data) {
        throw new Error('Response dari server tidak valid');
      }

      console.log('✅ [KPMR Kepatuhan Service] Response:', response.data);
      return this.convertBackendToFrontend(response.data);
    } catch (error: any) {
      console.error('❌ [KPMR Kepatuhan Service] Create error:', {
        message: error.message,
  ...
  async createAspek(kpmrId: number | string, data: CreateKpmrAspekKepatuhanDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_kepatuhan_produk.post(url, payload);
  ...
  async updateAspek(id: number | string, data: UpdateKpmrAspekKepatuhanDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_kepatuhan_produk.patch(url, data);
  ...
  async deleteAspek(id: number | string): Promise<void> {
    try {
      ...
      await api_kepatuhan_produk.delete(url);
    } catch (error: any) {
  ...
  async createPertanyaan(aspekId: number | string, data: CreateKpmrPertanyaanKepatuhanDto): Promise<FrontendPertanyaanResponse> {
    try {
  ...
  async updatePertanyaan(id: number | string, data: UpdateKpmrPertanyaanKepatuhanDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanId = this.cleanId(id);
      ...
      const response = await api_kepatuhan_produk.patch(url, payload);
...
export default kpmrKepatuhanApiService;
