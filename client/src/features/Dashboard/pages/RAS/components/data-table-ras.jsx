...
import { calculateRasValue, getQuarterMonthName } from '../utils/ras-utils';
const fmtNumber = (v) => {
  if (v === '' || v == null) return '';
...
export default function RasTable({ rows, year, quarter, onEdit, onDelete }) {
      ...
      <table className="w-full min-w-[1400px] text-left border-collapse text-sm">
        <thead className="bg-[#1f4e79] text-white">
          <tr>
            ...
            <th rowSpan={2} className="py-3 px-2 border border-gray-400 font-semibold w-12 align-middle text-center">
              No
            </th>
            <th rowSpan={2} className="py-3 px-4 border border-gray-400 font-semibold min-w-[250px] align-middle">
              Parameter
            </th>
            <th rowSpan={2} className="py-3 px-2 border border-gray-400 font-semibold w-24 align-middle text-center">
              RKAP
              <br />
              Des {year}
            </th>
            <th rowSpan={2} className="py-3 px-2 border border-gray-400 font-semibold w-32 align-middle text-center">
              Penjelasan Tipe Data
            </th>

            {/* Group Header RAS */}
            <th colSpan={3} className="py-2 px-2 border border-gray-400 font-semibold text-center bg-[#163a5c]">
              RAS {year}
            </th>

            <th rowSpan={2} className="py-3 px-4 border border-gray-400 font-semibold w-40 align-middle text-center">
              Keterangan
            </th>
            <th rowSpan={2} className="py-3 px-4 border border-gray-400 font-semibold w-32 align-middle text-center">
              {currentMonthName}
            ...
            <th rowSpan={2} className="py-3 px-2 border border-gray-400 font-semibold w-20 align-middle text-center">
              Aksi
            </th>
          </tr>
          <tr>
              ...
              return (
                <React.Fragment key={category + groupIndex}>
                  {items.map((item, itemIndex) => {
                    ...
                    const finalValue = calculateRasValue(item.numeratorValue, item.denominatorValue, item.unitType, item.manualQuarterValue);
                    ...
                    return (
                      <React.Fragment key={item.id}>
                        ...
                        <tr className="hover:bg-blue-50 transition-colors">
                          ...
                          {isFirstItemInGroup && (
                            <td rowSpan={totalGroupRows} className="px-4 py-3 font-bold align-top bg-gray-50 border border-gray-300">
                              {category}
                            </td>
                          )}
                          ...
                          {/* Parameter Utama */}
                          <td className="px-4 py-3 align-top border border-gray-300 font-medium">{item.parameter}</td>

                          <td className="px-2 py-3 text-center align-top border border-gray-300 text-xs">{item.rkapTarget || 'N/A'}</td>

                          <td className="px-2 py-3 text-xs align-top border border-gray-300 italic">{item.dataTypeExplanation}</td>

                          <td className="px-2 py-3 text-center font-bold text-red-600 align-top border border-gray-300">{item.rasLimit}</td>

                          <td className="px-2 py-3 text-center align-top border border-gray-300">
                            <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold border ${getStanceBadgeColor(item.riskStance)}`}>{item.riskStance}</span>
                          </td>

                          <td className="px-3 py-3 text-xs align-top border border-gray-300 text-justify leading-relaxed min-w-[200px]">{item.statement}</td>
                          ...
                          */}
                          ...
                          <td className="px-3 py-3 text-center font-bold text-blue-800 align-top border border-gray-300 bg-blue-50/50">{finalValue}</td>
                        ...
                        {hasCalc && (
                          <tr className="bg-gray-50/30 text-xs text-gray-600">
                            ...
                            <td className="px-4 py-2 border border-gray-300 border-t-0 pl-8 italic">{item.numeratorLabel || '(Pembilang)'}</td>
                            ...
                            {/* Keterangan (Merged) */}
                            {/* Value Pembilang */}
                            <td className="px-3 py-2 text-center border border-gray-300 font-mono text-gray-700">{fmtNumber(item.numeratorValue)}</td>
                            {/* Aksi (Merged) */}
                          </tr>
                        )}
                        ...
                        {hasCalc && (
                          <tr className="bg-gray-50/30 text-xs text-gray-600 border-b-2 border-gray-300">
                            ...
                            <td className="px-4 py-2 border border-gray-300 border-t-0 pl-8 italic">{item.denominatorLabel || '(Penyebut)'}</td>
                            ...
                            {/* Value Penyebut */}
                            <td className="px-3 py-2 text-center border border-gray-300 font-mono text-gray-700">{fmtNumber(item.denominatorValue)}</td>
                            {/* Aksi (Merged) */}
                          </tr>
                        )}
                      </React.Fragment>
