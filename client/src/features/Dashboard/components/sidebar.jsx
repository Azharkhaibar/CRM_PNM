    ...
    <div className="bg-white w-64 h-screen border-r p-4 flex flex-col">
        ...
        <ul className="space-y-1">
          <li>
            <Link
              ...
              className={`block py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard', true) ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              Dashboard
            </Link>
          </li>
          ...
          <li>
            <Link
              to="#"
              className={`block py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard/ras') ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              RAS
            </Link>
          </li>
          ...
          <li>
            <button
              ...
              className={`w-full flex items-center justify-between py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard/risk-form') ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
          ...
          <li>
            <Link
              to="/dashboard/maturasi"
              className={`block py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard/maturasi', true) ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              Maturasi
            </Link>
          </li>
          ...
          <li>
            <Link
              to="/dashboard/report"
              className={`block py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard/report', true) ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              Report
            </Link>
          </li>
          ...
          <li>
            <Link
              ...
              className={`block py-2 px-4 rounded-lg text-[18px] font-medium transition
                ${isActive('/dashboard/settings', true) ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              Settings
            </Link>
          </li>
      ...
      <div className="mt-auto relative flex items-center gap-3 p-3 border-t border-gray-200" ref={menuRef}>
        <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-start gap-3 w-full">
          <Avatar src={user?.photoURL} name={user?.userID || 'User'} size="40" round color="#2563EB" />
        ...
        <AnimatePresence>
          {menuOpen && (
            ...
            >
              <button onClick={() => nvg('profile')} className="flex items-center gap-2 w-full px-2 py-1 hover:bg-gray-100 rounded">
                <FaUserCircle />
