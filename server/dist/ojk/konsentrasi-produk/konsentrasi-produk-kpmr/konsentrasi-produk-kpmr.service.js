...
var KpmrKonsentrasiService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KpmrKonsentrasiService = void 0;
...
let KpmrKonsentrasiService = KpmrKonsentrasiService_1 = class KpmrKonsentrasiService {
    kpmrRepository;
    aspekRepository;
    pertanyaanRepository;
    dataSource;
    logger = new common_1.Logger(KpmrKonsentrasiService_1.name);
    MAX_BOBOT_TOTAL = 100;
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(konsentrasi_kpmr_aspek_entity_1.KpmrAspekKonsentrasi, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(konsentrasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanKonsentrasi, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(konsentrasi_kpmr_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(konsentrasi_kpmr_dto_1.FrontendAspekResponseDto, {
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(konsentrasi_kpmr_aspek_entity_1.KpmrAspekKonsentrasi, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async createPertanyaan(aspekId, createDto) {
        try {
            ...
            return {
                ...
                indicator: {
                    strong: savedPertanyaan.indicator?.strong || '',
                    satisfactory: savedPertanyaan.indicator?.satisfactory || '',
                    fair: savedPertanyaan.indicator?.fair || '',
                    marginal: savedPertanyaan.indicator?.marginal || '',
                    unsatisfactory: savedPertanyaan.indicator?.unsatisfactory || '',
                },
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(konsentrasi_kpmr_dto_1.FrontendPertanyaanResponseDto, {
                ...
                indicator: {
                    strong: updatedPertanyaan.indicator?.strong || '',
                    satisfactory: updatedPertanyaan.indicator?.satisfactory || '',
                    fair: updatedPertanyaan.indicator?.fair || '',
                    marginal: updatedPertanyaan.indicator?.marginal || '',
                    unsatisfactory: updatedPertanyaan.indicator?.unsatisfactory || '',
                },
    ...
    async updateSkor(id, updateSkorDto) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(konsentrasi_kpmr_dto_1.FrontendPertanyaanResponseDto, {
                ...
                indicator: {
                    strong: saved.indicator?.strong || '',
                    satisfactory: saved.indicator?.satisfactory || '',
                    fair: saved.indicator?.fair || '',
                    marginal: saved.indicator?.marginal || '',
                    unsatisfactory: saved.indicator?.unsatisfactory || '',
                },
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(konsentrasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanKonsentrasi)
                    .findOne({
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(konsentrasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanKonsentrasi, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(konsentrasi_kpmr_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrKonsentrasiService = KpmrKonsentrasiService;
exports.KpmrKonsentrasiService = KpmrKonsentrasiService = KpmrKonsentrasiService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(konsentrasi_produk_kpmr_entity_1.KpmrKonsentrasiOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(konsentrasi_kpmr_aspek_entity_1.KpmrAspekKonsentrasi)),
    __param(2, (0, typeorm_1.InjectRepository)(konsentrasi_kpmr_pertanyaan_entity_1.KpmrPertanyaanKonsentrasi)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], KpmrKonsentrasiService);
//# sourceMappingURL=konsentrasi-produk-kpmr.service.js.map