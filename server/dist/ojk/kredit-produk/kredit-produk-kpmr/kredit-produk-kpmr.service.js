...
var KpmrKreditService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KpmrKreditService = void 0;
...
let KpmrKreditService = KpmrKreditService_1 = class KpmrKreditService {
    kpmrRepository;
    aspekRepository;
    pertanyaanRepository;
    dataSource;
    logger = new common_1.Logger(KpmrKreditService_1.name);
    MAX_BOBOT_TOTAL = 100;
    ...
    async getAspekWithRelations(id) {
        const aspek = await this.aspekRepository.findOne({
            where: { id },
            relations: ['kpmrKredit', 'pertanyaanList'],
        });
        if (!aspek) {
    ...
    async getPertanyaanWithRelations(id) {
        const pertanyaan = await this.pertanyaanRepository.findOne({
            where: { id },
            relations: ['aspek', 'aspek.kpmrKredit'],
        });
        if (!pertanyaan) {
    ...
    async validateTotalBobot(kpmrId, newBobot, excludeAspekId) {
        const query = this.aspekRepository
            ...
            .where('aspek.kpmrKreditId = :kpmrId', { kpmrId });
        if (excludeAspekId) {
    ...
    async reorderRemainingAspek(kpmrId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < aspekList.length; i++) {
                await manager.update(kredit_kpmr_aspek_entity_1.KpmrAspekKredit, aspekList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    async reorderRemainingPertanyaan(aspekId) {
        ...
        await this.dataSource.transaction(async (manager) => {
            for (let i = 0; i < pertanyaanList.length; i++) {
                await manager.update(kredit_kpmr_pertanyaan_entity_1.KpmrPertanyaanKredit, pertanyaanList[i].id, {
                    orderIndex: i,
                });
            }
        });
    ...
    convertToFrontendFormat(kpmr) {
        ...
        return (0, class_transformer_1.plainToInstance)(kredit_produk_dto_1.FrontendKpmrResponseDto, frontendKpmr);
    }
    ...
    async createAspek(kpmrId, createDto) {
        try {
            ...
            const kpmrKredit = await this.kpmrRepository.findOne({
                where: { id: kpmrId },
            });
            if (!kpmrKredit) {
                throw new common_1.NotFoundException(`KPMR with ID ${kpmrId} not found`);
            }
            this.checkKpmrLocked(kpmrKredit, 'menambah aspek');
    ...
    async updateAspek(id, updateDto, updatedBy) {
        ...
        this.checkKpmrLocked(aspek.kpmrKredit, 'mengupdate aspek');
        if (updateDto.bobot !== undefined) {
            this.validateBobot(updateDto.bobot);
            await this.validateTotalBobot(aspek.kpmrKreditId, updateDto.bobot, id);
        }
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(kredit_produk_dto_1.FrontendAspekResponseDto, {
    ...
    async deleteAspek(id) {
        ...
        const aspek = await this.getAspekWithRelations(id);
        this.checkKpmrLocked(aspek.kpmrKredit, 'menghapus aspek');
        try {
            await this.aspekRepository.remove(aspek);
            await this.reorderRemainingAspek(aspek.kpmrKreditId);
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.aspekIds.length; i++) {
                    await manager.update(kredit_kpmr_aspek_entity_1.KpmrAspekKredit, reorderDto.aspekIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async createPertanyaan(aspekId, createDto) {
        try {
            ...
            const aspek = await this.aspekRepository.findOne({
                where: { id: aspekId },
                relations: ['kpmrKredit'],
            });
            if (!aspek) {
                throw new common_1.NotFoundException(`Aspek with ID ${aspekId} not found`);
            }
            this.checkKpmrLocked(aspek.kpmrKredit, 'menambah pertanyaan');
    ...
    async updatePertanyaan(id, updateDto) {
        ...
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKredit, 'mengupdate pertanyaan');
        if (updateDto.skor) {
        ...
        try {
            ...
            return (0, class_transformer_1.plainToInstance)(kredit_produk_dto_1.FrontendPertanyaanResponseDto, {
    ...
    async updateSkor(id, updateSkorDto) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKredit, 'mengupdate skor');
        ...
        try {
            for (const update of bulkDto.updates) {
                const pertanyaan = await queryRunner.manager
                    .getRepository(kredit_kpmr_pertanyaan_entity_1.KpmrPertanyaanKredit)
                    .findOne({
                ...
                if (pertanyaan.aspek.kpmrKredit.isLocked) {
    ...
    async deletePertanyaan(id) {
        ...
        const pertanyaan = await this.getPertanyaanWithRelations(id);
        this.checkKpmrLocked(pertanyaan.aspek.kpmrKredit, 'menghapus pertanyaan');
        try {
    ...
    async reorderPertanyaan(aspekId, reorderDto) {
        ...
        this.checkKpmrLocked(aspek.kpmrKredit, 'mengubah urutan pertanyaan');
        ...
        try {
            await this.dataSource.transaction(async (manager) => {
                for (let i = 0; i < reorderDto.pertanyaanIds.length; i++) {
                    await manager.update(kredit_kpmr_pertanyaan_entity_1.KpmrPertanyaanKredit, reorderDto.pertanyaanIds[i], {
                        orderIndex: i,
                    });
                }
            });
    ...
    async findAllAspek(kpmrId) {
        ...
        return aspekList.map((aspek) => (0, class_transformer_1.plainToInstance)(kredit_produk_dto_1.FrontendAspekResponseDto, {
            id: aspek.id.toString(),
            nomor: aspek.nomor || '',
            judul: aspek.judul,
            bobot: aspek.bobot.toString(),
...
exports.KpmrKreditService = KpmrKreditService;
exports.KpmrKreditService = KpmrKreditService = KpmrKreditService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(kredit_produk_kpmr_entity_1.KpmrKreditOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(kredit_kpmr_aspek_entity_1.KpmrAspekKredit)),
    __param(2, (0, typeorm_1.InjectRepository)(kredit_kpmr_pertanyaan_entity_1.KpmrPertanyaanKredit)),
