...
// Transform functions untuk strategik - SESUAI dengan service
const transformIndicatorToBackend = (indicatorData, year, quarter, sectionId, sectionData) => {
...
// Transform functions untuk section
const transformSectionToBackendStrategik = (sectionData, year, quarter) => {
  return {
    no: sectionData.no,
    parameter: sectionData.parameter,
    bobotSection: Number(sectionData.bobotSection || 0),
...
const computeWeightedAuto = (ind, sectionBobot) => {
  // Normalisasi: pastikan semua dalam format yang benar
  const sectionB = Number(sectionBobot || 0);
  const bobotInd = Number(ind.bobotIndikator || 0);
  const peringkat = Number(ind.peringkat || 1); // Default 1

  // Formula STANDAR: (Bobot Section * Bobot Indikator * Peringkat) / 10000
  // Bobot Section dan Bobot Indikator dalam persentase (misal: 10, 25)
  const res = (sectionB * bobotInd * peringkat) / 10000;
...
// ===================== MAIN COMPONENT =====================
export default function StratejikTab() {
  // ====== STATES UTAMA ======
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setLocalError(err.message || 'Gagal memuat data strategik');
      showNotification('error', err.message || 'Gagal memuat data strategik');
      setSections([]);
    } finally {
      setLoadingData(false);
    }
  ...
  const loadAllSections = useCallback(async () => {
    setLoadingAllSections(true);
    try {
      console.log('🔄 [STRATEJIK] Loading all sections...');
      ...
      console.log('✅ [STRATEJIK] All sections loaded:', transformedSections.length);
    } catch (err) {
      console.error('❌ [STRATEJIK] Error loading all sections:', err);
  ...
  const handleCreateSection = async () => {
    ...
    setIsCreating(true);
    try {
      const sectionData = transformSectionToBackendStrategik(sectionForm, viewYear, viewQuarter);
  ...
  const handleUpdateSection = async () => {
    ...
    setIsUpdating(true);
    try {
      const updateData = transformSectionToBackendStrategik(sectionForm, viewYear, viewQuarter);
  ...
  // ====== Export Function ======
  const handleExportStrategik = () => {
    exportStrategikToExcel({
      year: viewYear,
      quarter: viewQuarter,
      sections: sections,
      filename: `Strategik_${viewYear}_${viewQuarter}`,
    });
  };
  ...
  const hasilPreview = useMemo(() => {
    ...
    if (indicatorForm.isPercent) {
      ...
      // PERBAIKAN: Hitung selisih persentase untuk KPI
      // Jika mode RASIO dan menggunakan rumus default (pemb/peny)
      // Maka persentase = (pembilang/penyebut) * 100
      if (indicatorForm.mode === CalculationMode.RASIO && !indicatorForm.formula) {
        return `${pct.toFixed(2)}%`;
      }
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl">
        ...
        <header className="flex items-center justify-between gap-3 px-4 py-4">
          <h2 className="text-xl sm:text-2xl font-semibold">Form – Strategik</h2>
          <div className="flex items-end gap-4">
            ...
            <div className="flex items-end gap-2">
              ...
              <button onClick={handleExportStrategik} disabled={loadingData || sections.length === 0} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 border bg-gray-900 text-white hover:bg-black disabled:opacity-50">
                <Download size={18} />
              ...
              <div className="grid grid-cols-12 gap-4 mb-4">
                ...
                <div className="col-span-7">
                  ...
                  <input
                    ...
                    value={indicatorForm.indikator}
                    onChange={(e) => setIndicatorField('indikator', e.target.value)}
                    placeholder="Teks indikator risiko strategik…"
                  />
                </div>
                <div className="col-span-3">
                  <label className="text-sm font-medium mb-2 block text-right text-white">Bobot Indikator (%)</label>
