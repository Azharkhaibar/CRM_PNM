...
let KepatuhanOjkService = KepatuhanOjkService_1 = class KepatuhanOjkService {
    kepatuhanRepository;
    parameterRepository;
    nilaiRepository;
    referenceRepository;
    dataSource;
    logger = new common_1.Logger(KepatuhanOjkService_1.name);
    constructor(kepatuhanRepository, parameterRepository, nilaiRepository, referenceRepository, dataSource) {
        this.kepatuhanRepository = kepatuhanRepository;
    ...
    async create(createDto, userId) {
        try {
            ...
            this.logger.error(`Error creating KepatuhanOjk: ${error.message}`, error.stack);
            throw error;
        }
    }
    async findActive() {
    ...
    async findByYearQuarter(year, quarter) {
        ...
        try {
            ...
            this.logger.log(`findByYearQuarter: Data ditemukan - ID: ${kepatuhan.id}`);
            return kepatuhan;
        }
        catch (error) {
    ...
    async update(id, updateDto, userId) {
        ...
        if (updateDto.notes !== undefined)
            kepatuhan.notes = updateDto.notes;
    ...
    async updateActiveStatus(id, isActive, userId) {
        ...
        if (isActive) {
            ...
            await this.kepatuhanRepository
                .createQueryBuilder()
                .update(kepatuhan_ojk_entity_1.KepatuhanOjk)
                .set({ isActive: false })
                .execute();
        }
    ...
    async remove(id) {
        ...
        try {
            for (const parameter of kepatuhan.parameters || []) {
                await queryRunner.manager.delete(kepatuhan_nilai_entity_1.KepatuhanNilai, {
                    parameterId: parameter.id,
                });
            }
            await queryRunner.manager.delete(kepatuhan_paramater_entity_1.KepatuhanParameter, {
                kepatuhanId: id,
            });
            await queryRunner.manager.delete(kepatuhan_ojk_entity_1.KepatuhanOjk, { id });
    ...
    async addParameter(kepatuhanId, createParamDto, userId) {
        this.logger.log(`addParameter: Menambahkan parameter - Kepatuhan ID: ${kepatuhanId}`);
        const kepatuhan = await this.kepatuhanRepository.findOne({
        ...
        if (createParamDto.kategori) {
            ...
            if (kategori.model === kepatuhan_inherent_dto_1.KategoriModel.OPEN_END) {
                if (!kategori.jenis) {
            ...
            if (kategori.model === kepatuhan_inherent_dto_1.KategoriModel.TERSTRUKTUR) {
                if (kategori.jenis) {
            ...
            if (kategori.model === kepatuhan_inherent_dto_1.KategoriModel.TANPA_MODEL) {
                if (kategori.prinsip ||
                    kategori.jenis ||
        ...
        const kategoriFormatted = createParamDto.kategori
            ? {
                ...
                prinsip: createParamDto.kategori.model !== kepatuhan_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? createParamDto.kategori.prinsip
                    : undefined,
                jenis: createParamDto.kategori.model === kepatuhan_inherent_dto_1.KategoriModel.OPEN_END
                    ? createParamDto.kategori.jenis
                    : undefined,
                underlying: createParamDto.kategori.model === kepatuhan_inherent_dto_1.KategoriModel.TERSTRUKTUR
    ...
    async updateParameter(kepatuhanId, parameterId, updateParamDto, userId) {
        this.logger.log(`updateParameter: Mengupdate parameter - ID: ${parameterId}`);
        const parameter = await this.parameterRepository.findOne({
            where: { id: parameterId, kepatuhanId: kepatuhanId },
        });
        if (!parameter) {
        ...
        if (updateParamDto.kategori) {
            ...
            parameter.kategori = {
                ...
                prinsip: kategori.model !== kepatuhan_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? kategori.prinsip
                    : undefined,
                jenis: kategori.model === kepatuhan_inherent_dto_1.KategoriModel.OPEN_END
                    ? kategori.jenis
                    : undefined,
                underlying: kategori.model === kepatuhan_inherent_dto_1.KategoriModel.TERSTRUKTUR
                    ? kategori.underlying || []
                    : [],
            };
        }
    ...
    async reorderParameters(kepatuhanId, reorderDto) {
        this.logger.log(`reorderParameters: Mengurutkan parameter - Kepatuhan ID: ${kepatuhanId}`);
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            for (let i = 0; i < reorderDto.parameterIds.length; i++) {
                ...
                await queryRunner.manager.update(kepatuhan_paramater_entity_1.KepatuhanParameter, { id: parameterId, kepatuhanId: kepatuhanId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
            this.logger.log(`reorderParameters: Parameter berhasil diurutkan - Kepatuhan ID: ${kepatuhanId}`);
    ...
    async copyParameter(kepatuhanId, parameterId, userId) {
        ...
        try {
            ...
            const savedParam = await queryRunner.manager.save(kepatuhan_paramater_entity_1.KepatuhanParameter, newParam);
            if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
                const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
                    ...
                    return queryRunner.manager.save(kepatuhan_nilai_entity_1.KepatuhanNilai, newNilai);
                });
    ...
    async removeParameter(kepatuhanId, parameterId, userId) {
        ...
        try {
            ...
            await queryRunner.manager.delete(kepatuhan_paramater_entity_1.KepatuhanParameter, { id: parameterId });
            await queryRunner.commitTransaction();
            await this.kepatuhanRepository.update(kepatuhanId, {
                updatedBy: userId,
                updatedAt: new Date(),
    ...
    async addNilai(kepatuhanId, parameterId, createNilaiDto, userId) {
        ...
        const nilai = {
            ...
            judul: {
                type: createNilaiDto.judul?.type || kepatuhan_inherent_dto_1.JudulType.TANPA_FAKTOR,
                text: judulText.trim(),
    ...
    async updateNilai(kepatuhanId, parameterId, nilaiId, updateNilaiDto, userId) {
        ...
        if (nilai.parameter.kepatuhanId !== kepatuhanId) {
            throw new common_1.BadRequestException('Nilai tidak termasuk dalam kepatuhan yang dimaksud');
        }
    ...
    async reorderNilai(parameterId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.nilaiIds.length; i++) {
                ...
                await queryRunner.manager.update(kepatuhan_nilai_entity_1.KepatuhanNilai, { id: nilaiId, parameterId: parameterId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async copyNilai(kepatuhanId, parameterId, nilaiId, userId) {
    ...
    async removeNilai(kepatuhanId, parameterId, nilaiId, userId) {
    ...
    async validateModelTerstruktur(kepatuhanId) {
        const result = {
            isValid: true,
            warnings: [],
            errors: [],
        ...
        if (!kepatuhan) {
            result.errors.push(`Data dengan ID ${kepatuhanId} tidak ditemukan`);
            result.isValid = false;
            return result;
        }
        const terstrukturParams = kepatuhan.parameters?.filter((param) => param.kategori?.model === kepatuhan_inherent_dto_1.KategoriModel.TERSTRUKTUR) || [];
    ...
    async exportToExcel(kepatuhanId) {
        this.logger.log(`exportToExcel: Mengekspor ke Excel - ID: ${kepatuhanId}`);
        const kepatuhan = await this.kepatuhanRepository.findOne({
            where: { id: kepatuhanId },
            relations: ['parameters', 'parameters.nilaiList'],
            order: {
        ...
        const exportData = {
            metadata: {
                ...
                totalNilai: kepatuhan.parameters?.reduce((total, param) => total + (param.nilaiList?.length || 0), 0) || 0,
            },
            parameters: kepatuhan.parameters?.map((param) => ({
                id: param.id,
                nomor: param.nomor,
                judul: param.judul,
    ...
    async importFromExcel(importData, userId) {
        ...
        await queryRunner.startTransaction();
        try {
            await queryRunner.manager.update(kepatuhan_ojk_entity_1.KepatuhanOjk, { isActive: true }, { isActive: false });
            const kepatuhan = {
            ...
            const savedKepatuhan = await queryRunner.manager.save(kepatuhan_ojk_entity_1.KepatuhanOjk, kepatuhan);
            for (let i = 0; i < importData.parameters.length; i++) {
                ...
                const savedParam = await queryRunner.manager.save(kepatuhan_paramater_entity_1.KepatuhanParameter, parameter);
                    ...
                    for (let j = 0; j < paramData.nilaiList.length; j++) {
                        ...
                        const nilai = {
                            ...
                            judul: nilaiData.judul || {
                                type: kepatuhan_inherent_dto_1.JudulType.TANPA_FAKTOR,
                                text: '',
                                value: null,
                                pembilang: '',
                                valuePembilang: null,
                        ...
                        await queryRunner.manager.save(kepatuhan_nilai_entity_1.KepatuhanNilai, nilai);
                    }
                }
            }
            await queryRunner.commitTransaction();
            this.logger.log(`importFromExcel: Data berhasil diimpor - ID: ${savedKepatuhan.id}, Jumlah parameter: ${importData.parameters.length}`);
...
exports.KepatuhanOjkService = KepatuhanOjkService = KepatuhanOjkService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(kepatuhan_ojk_entity_1.KepatuhanOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(kepatuhan_paramater_entity_1.KepatuhanParameter)),
    __param(2, (0, typeorm_1.InjectRepository)(kepatuhan_nilai_entity_1.KepatuhanNilai)),
    __param(3, (0, typeorm_1.InjectRepository)(kepatuhan_inherent_references_entity_1.KepatuhanReference)),
