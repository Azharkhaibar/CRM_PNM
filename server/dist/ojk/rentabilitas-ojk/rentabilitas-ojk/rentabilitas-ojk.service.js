...
var RentabilitasProdukOjkService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.RentabilitasProdukOjkService = void 0;
...
let RentabilitasProdukOjkService = RentabilitasProdukOjkService_1 = class RentabilitasProdukOjkService {
    inherentRepository;
    parameterRepository;
    nilaiRepository;
    referenceRepository;
    dataSource;
    logger = new common_1.Logger(RentabilitasProdukOjkService_1.name);
    ...
    async create(createDto, userId) {
        try {
            ...
            this.logger.error(`Error creating RentabilitasProdukOjk: ${error.message}`, error.stack);
            throw error;
        }
    }
    async findActive() {
    ...
    async updateActiveStatus(id, isActive, userId) {
        ...
        if (isActive) {
            ...
            await this.inherentRepository
                .createQueryBuilder()
                .update(rentabilitas_ojk_entity_1.RentabilitasProdukOjk)
                .set({ isActive: false })
                .execute();
        }
    ...
    async remove(id) {
        ...
        try {
            for (const parameter of inherent.parameters || []) {
                await queryRunner.manager.delete(rentabilitas_nilai_entity_1.RentabilitasNilai, {
                    parameterId: parameter.id,
                });
            }
            await queryRunner.manager.delete(rentabilitas_parameter_entity_1.RentabilitasParameter, {
                rentabilitasProdukOjkId: id,
            });
            await queryRunner.manager.delete(rentabilitas_ojk_entity_1.RentabilitasProdukOjk, { id });
    ...
    async addParameter(inherentId, createParamDto, userId) {
        ...
        if (createParamDto.kategori) {
            ...
            if (kategori.model === rentabilitas_inherent_dto_1.KategoriModel.OPEN_END) {
                if (!kategori.jenis) {
            ...
            if (kategori.model === rentabilitas_inherent_dto_1.KategoriModel.TERSTRUKTUR) {
                if (kategori.jenis) {
            ...
            if (kategori.model === rentabilitas_inherent_dto_1.KategoriModel.TANPA_MODEL) {
                if (kategori.prinsip ||
                    kategori.jenis ||
        ...
        const lastParam = await this.parameterRepository.findOne({
            where: { rentabilitasProdukOjkId: inherentId },
        ...
        const kategoriFormatted = createParamDto.kategori
            ? {
                ...
                prinsip: createParamDto.kategori.model !== rentabilitas_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? createParamDto.kategori.prinsip
                    : undefined,
                jenis: createParamDto.kategori.model === rentabilitas_inherent_dto_1.KategoriModel.OPEN_END
                    ? createParamDto.kategori.jenis
                    : undefined,
                underlying: createParamDto.kategori.model === rentabilitas_inherent_dto_1.KategoriModel.TERSTRUKTUR
        ...
        try {
            const parameter = this.parameterRepository.create({
                ...
                rentabilitasProdukOjkId: inherentId,
    ...
    async updateParameter(inherentId, parameterId, updateParamDto, userId) {
        ...
        const parameter = await this.parameterRepository.findOne({
            where: { id: parameterId, rentabilitasProdukOjkId: inherentId },
        });
        if (!parameter) {
        ...
        if (updateParamDto.kategori) {
            ...
            parameter.kategori = {
                ...
                prinsip: kategori.model !== rentabilitas_inherent_dto_1.KategoriModel.TANPA_MODEL
                    ? kategori.prinsip
                    : undefined,
                jenis: kategori.model === rentabilitas_inherent_dto_1.KategoriModel.OPEN_END
                    ? kategori.jenis
                    : undefined,
                underlying: kategori.model === rentabilitas_inherent_dto_1.KategoriModel.TERSTRUKTUR
                    ? kategori.underlying || []
                    : [],
            };
        }
    ...
    async reorderParameters(inherentId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.parameterIds.length; i++) {
                ...
                await queryRunner.manager.update(rentabilitas_parameter_entity_1.RentabilitasParameter, { id: parameterId, rentabilitasProdukOjkId: inherentId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async copyParameter(inherentId, parameterId, userId) {
        ...
        try {
            ...
            const savedParam = await queryRunner.manager.save(rentabilitas_parameter_entity_1.RentabilitasParameter, newParam);
            if (originalParam.nilaiList && originalParam.nilaiList.length > 0) {
                const nilaiPromises = originalParam.nilaiList.map(async (nilai) => {
                    ...
                    return queryRunner.manager.save(rentabilitas_nilai_entity_1.RentabilitasNilai, newNilai);
                });
    ...
    async addNilai(inherentId, parameterId, createNilaiDto, userId) {
        ...
        const nilai = {
            ...
            judul: {
                type: createNilaiDto.judul?.type || rentabilitas_inherent_dto_1.JudulType.TANPA_FAKTOR,
                text: judulText.trim(),
    ...
    async updateNilai(inherentId, parameterId, nilaiId, updateNilaiDto, userId) {
        ...
        if (nilai.parameter.rentabilitasProdukOjkId !== inherentId) {
    ...
    async reorderNilai(parameterId, reorderDto) {
        ...
        try {
            for (let i = 0; i < reorderDto.nilaiIds.length; i++) {
                ...
                await queryRunner.manager.update(rentabilitas_nilai_entity_1.RentabilitasNilai, { id: nilaiId, parameterId: parameterId }, { orderIndex: i });
            }
            await queryRunner.commitTransaction();
    ...
    async validateModelTerstruktur(inherentId) {
        ...
        const terstrukturParams = inherent.parameters?.filter((param) => param.kategori?.model === rentabilitas_inherent_dto_1.KategoriModel.TERSTRUKTUR) || [];
    ...
    async importFromExcel(importData, userId) {
        ...
        await queryRunner.startTransaction();
        try {
            await queryRunner.manager.update(rentabilitas_ojk_entity_1.RentabilitasProdukOjk, { isActive: true }, { isActive: false });
            const inherent = {
            ...
            const savedInherent = await queryRunner.manager.save(rentabilitas_ojk_entity_1.RentabilitasProdukOjk, inherent);
            for (let i = 0; i < importData.parameters.length; i++) {
                ...
                const parameter = {
                    ...
                    rentabilitasProdukOjkId: savedInherent.id,
                    orderIndex: paramData.orderIndex || i,
                };
                const savedParam = await queryRunner.manager.save(rentabilitas_parameter_entity_1.RentabilitasParameter, parameter);
                    ...
                    for (let j = 0; j < paramData.nilaiList.length; j++) {
                        ...
                        const nilai = {
                            ...
                            judul: nilaiData.judul || {
                                type: rentabilitas_inherent_dto_1.JudulType.TANPA_FAKTOR,
                                text: '',
                                value: null,
                                pembilang: '',
                                valuePembilang: null,
                        ...
                        await queryRunner.manager.save(rentabilitas_nilai_entity_1.RentabilitasNilai, nilai);
                    }
                }
            }
            await queryRunner.commitTransaction();
...
exports.RentabilitasProdukOjkService = RentabilitasProdukOjkService;
exports.RentabilitasProdukOjkService = RentabilitasProdukOjkService = RentabilitasProdukOjkService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(rentabilitas_ojk_entity_1.RentabilitasProdukOjk)),
    __param(1, (0, typeorm_1.InjectRepository)(rentabilitas_parameter_entity_1.RentabilitasParameter)),
    __param(2, (0, typeorm_1.InjectRepository)(rentabilitas_nilai_entity_1.RentabilitasNilai)),
    __param(3, (0, typeorm_1.InjectRepository)(rentabilitas_inherent_references_entity_1.InherentReferenceRentabilitas)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource])
], RentabilitasProdukOjkService);
