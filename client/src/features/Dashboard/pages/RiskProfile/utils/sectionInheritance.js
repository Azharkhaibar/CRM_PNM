...
export function getSectionsForPeriod(year, quarter, allSections) {
  ...
  const directSections = allSections.filter(
    s => s.year === year && s.quarter === quarter && !s.isVirtual
  );
...
export function deleteSectionFromPeriod(sectionId, year, quarter, allSections) {
  ...
  return allSections.filter(
    s => !(s.id === sectionId && s.year === year && s.quarter === quarter)
  );
}
...
export function cloneSectionsFromPeriod(fromYear, fromQuarter, toYear, toQuarter, allSections, deletedSections = new Map()) {
  const sourceSections = allSections.filter(
    s => s.year === fromYear && s.quarter === fromQuarter
  );
  ...
  const existingTargetSections = allSections.filter(
    s => s.year === toYear && s.quarter === toQuarter
  );
  const existingNoParamSet = new Set(
    existingTargetSections.map(s => `${s.no}|${s.parameter}`)
  );
...
export function autoCloneSectionsIfNeeded(viewYear, viewQuarter, allSections) {
  ...
  const sourceSections = allSections.filter(
    s => s.year === prevPeriod.year && s.quarter === prevPeriod.quarter && !s.isVirtual
  );
