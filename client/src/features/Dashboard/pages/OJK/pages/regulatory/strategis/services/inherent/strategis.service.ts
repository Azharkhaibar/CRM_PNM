...
export interface StrategisOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
...
export interface ParameterEntity {
  ...
  orderIndex: number;
  strategisId: number; // ✅ BUKAN kpmrStrategisId
  createdAt: Date;
  updatedAt: Date;
}
...
export interface CreateStrategisDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateStrategisDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class StrategisService {
  ...
  async findOrCreate(
    ...
    try {
      ...
      const createDto: CreateStrategisDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
   ...
   */
  async ensureDataExists(year: number, quarter: number): Promise<StrategisOjkEntity> {
   ...
   */
  async loadOrCreateData(year: number, quarter: number): Promise<StrategisOjkEntity> {
   ...
   */
  private formatToFrontend(entity: StrategisOjkEntity | null): any[] {
    ...
    const result = parameters.map((parameter, index) => {
      ...
      const formattedParameter = {
        ...
        // Metadata dari strategis
        metadata: {
          strategisId: entity.id,
          year: entity.year,
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[StrategisService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<StrategisOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_strategis_produk.get<StrategisOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<StrategisOjkEntity | null> {
  ...
  async getById(id: number): Promise<StrategisOjkEntity> {
  ...
  async create(createDto: CreateStrategisDto): Promise<StrategisOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_strategis_produk.post<StrategisOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateStrategisDto): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_strategis_produk.put<StrategisOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_strategis_produk.put<StrategisOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateStrategisDto['summary']): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_strategis_produk.put<StrategisOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async getParameters(strategisId: number): Promise<ParameterEntity[]> {
    const url = `${this.baseUrl}/${strategisId}/parameters`;
    this.logApiCall('GET', url);

    try {
      const response = await api_strategis_produk.get<ParameterEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addParameter(strategisId: number, dto: CreateParameterDto): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters`;

    console.log('[Service] addParameter:', { url, strategisId, dto });
    ...
    try {
      ...
      const response = await api_strategis_produk.post<StrategisOjkEntity>(url, payload);
  ...
  async updateParameter(strategisId: number, parameterId: number, dto: UpdateParameterDto): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}`;
    ...
    try {
      ...
      const response = await api_strategis_produk.put<StrategisOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(strategisId: number, parameterId: number): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_strategis_produk.post<StrategisOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(strategisId: number, parameterId: number): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_strategis_produk.delete<StrategisOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(strategisId: number, parameterIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${strategisId}/parameters/reorder`;
  ...
  async getNilai(strategisId: number, parameterId: number): Promise<NilaiEntity[]> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai`;
    this.logApiCall('GET', url);

    try {
      const response = await api_strategis_produk.get<NilaiEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addNilai(strategisId: number, parameterId: number, dto: CreateNilaiDto): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai`;
  ...
  async updateNilai(strategisId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai/${nilaiId}`;
  ...
  async copyNilai(strategisId: number, parameterId: number, nilaiId: number): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai/${nilaiId}/copy`;
    this.logApiCall('POST', url);
  ...
  async removeNilai(strategisId: number, parameterId: number, nilaiId: number): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(strategisId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${strategisId}/parameters/${parameterId}/nilai/reorder`;
  ...
  async exportToExcel(strategisId: number): Promise<any> {
    const url = `${this.baseUrl}/${strategisId}/export`;
    this.logApiCall('GET', url);
  ...
  async importFromExcel(importData: any): Promise<StrategisOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_strategis_produk.post<StrategisOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: StrategisOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<StrategisOjkEntity> {
    ...
    try {
      ...
      const fallbackData: StrategisOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: StrategisOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_strategis_produk.get(this.baseUrl, {
        timeout: 5000,
      });
...
export const strategisService = new StrategisService();
export default strategisService;
