...
export interface TataKelolaOjkEntity {
  id: number;
  year: number;
  quarter: number;
  isActive: boolean;
...
export interface ParameterEntity {
  ...
  orderIndex: number;
  tatakelolaId: number; // ✅ BUKAN kpmrTataKelolaId
  createdAt: Date;
  updatedAt: Date;
}
...
export interface CreateTataKelolaDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface UpdateTataKelolaDto {
  year?: number;
  quarter?: number;
  isActive?: boolean;
  summary?: {
...
export class TataKelolaService {
  private baseUrl = '/tatakelola-ojk';
  ...
  async findOrCreate(
    ...
    try {
      ...
      const createDto: CreateTataKelolaDto = {
        year,
        quarter,
        isActive: true,
        createdBy: 'system',
   ...
   */
  async ensureDataExists(year: number, quarter: number): Promise<TataKelolaOjkEntity> {
   ...
   */
  async loadOrCreateData(year: number, quarter: number): Promise<TataKelolaOjkEntity> {
   ...
   */
  private formatToFrontend(entity: TataKelolaOjkEntity | null): any[] {
    ...
    const result = parameters.map((parameter, index) => {
      ...
      const formattedParameter = {
        ...
        // Metadata dari tatakelola
        metadata: {
          tatakelolaId: entity.id,
          year: entity.year,
  ...
  private handleError(error: any, operation: string, url?: string): never {
    ...
    console.error(`[TataKelolaService] Error in ${operation}:`, errorDetails);
  ...
  async findActive(): Promise<TataKelolaOjkEntity | null> {
    const url = `${this.baseUrl}/active`;
    this.logApiCall('GET', url);

    try {
      const response = await api_tatakelola_produk.get<TataKelolaOjkEntity>(url);
  ...
  async findByYearQuarter(year: number, quarter: number): Promise<TataKelolaOjkEntity | null> {
  ...
  async getById(id: number): Promise<TataKelolaOjkEntity> {
  ...
  async create(createDto: CreateTataKelolaDto): Promise<TataKelolaOjkEntity> {
    const url = this.baseUrl;
    this.logApiCall('POST', url, undefined, createDto);

    try {
      const response = await api_tatakelola_produk.post<TataKelolaOjkEntity>(url, createDto);
      return response.data;
    } catch (error: any) {
  ...
  async update(id: number, updateDto: UpdateTataKelolaDto): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${id}`;
    this.logApiCall('PUT', url, undefined, updateDto);

    try {
      const response = await api_tatakelola_produk.put<TataKelolaOjkEntity>(url, updateDto);
      return response.data;
    } catch (error: any) {
  ...
  async updateActiveStatus(id: number, isActive: boolean): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${id}/status`;
    this.logApiCall('PUT', url, undefined, { isActive });

    try {
      const response = await api_tatakelola_produk.put<TataKelolaOjkEntity>(url, { isActive });
      return response.data;
    } catch (error: any) {
  ...
  async updateSummary(id: number, summary: UpdateTataKelolaDto['summary']): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${id}/summary`;
    this.logApiCall('PUT', url, undefined, { summary });

    try {
      const response = await api_tatakelola_produk.put<TataKelolaOjkEntity>(url, { summary });
      return response.data;
    } catch (error: any) {
  ...
  async getParameters(tatakelolaId: number): Promise<ParameterEntity[]> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters`;
    this.logApiCall('GET', url);

    try {
      const response = await api_tatakelola_produk.get<ParameterEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addParameter(tatakelolaId: number, dto: CreateParameterDto): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters`;

    console.log('[Service] addParameter:', { url, tatakelolaId, dto });
    ...
    try {
      ...
      const response = await api_tatakelola_produk.post<TataKelolaOjkEntity>(url, payload);
  ...
  async updateParameter(tatakelolaId: number, parameterId: number, dto: UpdateParameterDto): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}`;
    ...
    try {
      ...
      const response = await api_tatakelola_produk.put<TataKelolaOjkEntity>(url, payload);
      return response.data;
    } catch (error: any) {
  ...
  async copyParameter(tatakelolaId: number, parameterId: number): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/copy`;
    this.logApiCall('POST', url);

    try {
      const response = await api_tatakelola_produk.post<TataKelolaOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async removeParameter(tatakelolaId: number, parameterId: number): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}`;
    this.logApiCall('DELETE', url);

    try {
      const response = await api_tatakelola_produk.delete<TataKelolaOjkEntity>(url);
      return response.data;
    } catch (error: any) {
  ...
  async reorderParameters(tatakelolaId: number, parameterIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/reorder`;
  ...
  async getNilai(tatakelolaId: number, parameterId: number): Promise<NilaiEntity[]> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai`;
    this.logApiCall('GET', url);

    try {
      const response = await api_tatakelola_produk.get<NilaiEntity[]>(url);
      return response.data || [];
    } catch (error: any) {
  ...
  async addNilai(tatakelolaId: number, parameterId: number, dto: CreateNilaiDto): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai`;
  ...
  async updateNilai(tatakelolaId: number, parameterId: number, nilaiId: number, dto: UpdateNilaiDto): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai/${nilaiId}`;
  ...
  async copyNilai(tatakelolaId: number, parameterId: number, nilaiId: number): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai/${nilaiId}/copy`;
    this.logApiCall('POST', url);
  ...
  async removeNilai(tatakelolaId: number, parameterId: number, nilaiId: number): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai/${nilaiId}`;
    this.logApiCall('DELETE', url);
  ...
  async reorderNilai(tatakelolaId: number, parameterId: number, nilaiIds: number[]): Promise<{ message: string }> {
    const url = `${this.baseUrl}/${tatakelolaId}/parameters/${parameterId}/nilai/reorder`;
  ...
  async exportToExcel(tatakelolaId: number): Promise<any> {
    const url = `${this.baseUrl}/${tatakelolaId}/export`;
    this.logApiCall('GET', url);
  ...
  async importFromExcel(importData: any): Promise<TataKelolaOjkEntity> {
    const url = `${this.baseUrl}/import`;
    this.logApiCall('POST', url, undefined, importData);

    try {
      const response = await api_tatakelola_produk.post<TataKelolaOjkEntity>(url, importData);
      return response.data;
    } catch (error: any) {
  ...
  async checkExists(year: number, quarter: number): Promise<{ exists: boolean; data: TataKelolaOjkEntity | null }> {
    try {
  ...
  async loadOrCreate(year: number, quarter: number): Promise<TataKelolaOjkEntity> {
    ...
    try {
      ...
      const fallbackData: TataKelolaOjkEntity = {
        id: -1,
        year,
        quarter,
        isActive: true,
  ...
  async getFormattedData(year?: number, quarter?: number): Promise<any[]> {
    ...
    try {
      let data: TataKelolaOjkEntity | null = null;
  ...
  async validateConnection(): Promise<boolean> {
    try {
      const response = await api_tatakelola_produk.get(this.baseUrl, {
        timeout: 5000,
      });
...
export const tatakelolaService = new TataKelolaService();
export default tatakelolaService;
