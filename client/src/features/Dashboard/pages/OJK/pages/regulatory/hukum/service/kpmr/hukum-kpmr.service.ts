...
// ========== CREATE DTOs ==========
export interface CreateKpmrHukumOjkDto {
  year: number;
  quarter: number;
  isActive?: boolean;
  createdBy?: string;
...
export interface CreateKpmrAspekHukumDto {
  nomor?: string;
  judul: string;
  bobot: number;
  deskripsi?: string;
...
export interface CreateKpmrPertanyaanHukumDto {
  nomor?: string;
  pertanyaan: string;
  skor?: KpmrSkorDto;
...
// ========== UPDATE DTOs ==========
export interface UpdateKpmrAspekHukumDto {
  nomor?: string;
  judul?: string;
  bobot?: number;
  deskripsi?: string;
...
export interface UpdateKpmrPertanyaanHukumDto {
  nomor?: string;
  pertanyaan?: string;
  skor?: KpmrSkorDto;
  indicator?: KpmrIndicatorDto;
...
class KpmrHukumApiService {
  private readonly BASE_PATH = '/kpmr-hukum';
  ...
  private handleApiError(error: any, defaultMessage: string): never {
    console.error(`❌ [KPMR Hukum Service] Error:`, error);
  ...
  async getKpmrByYearQuarter(year: number, quarter: number, withRelations: boolean = true): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_hukum.get(`${url}${params}`);
  ...
  async getKpmrWithRelations(id: number | string): Promise<FrontendKpmrResponse> {
    try {
      ...
      const response = await api_hukum.get(`${this.BASE_PATH}/${cleanId}?withRelations=true`);
      return response.data;
    } catch (error: any) {
  ...
  async createAspek(kpmrId: number | string, data: CreateKpmrAspekHukumDto): Promise<FrontendAspekResponse> {
    try {
      ...
      const response = await api_hukum.post(`${this.BASE_PATH}/${cleanKpmrId}/aspek`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updateAspek(id: number | string, data: UpdateKpmrAspekHukumDto): Promise<FrontendAspekResponse> {
    try {
      const cleanId = this.cleanId(id);
      const response = await api_hukum.patch(`${this.BASE_PATH}/aspek/${cleanId}`, data);
      return response.data;
    } catch (error: any) {
  ...
  async deleteAspek(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_hukum.delete(`${this.BASE_PATH}/aspek/${cleanId}`);
    } catch (error: any) {
  ...
  async createPertanyaan(aspekId: number | string, data: CreateKpmrPertanyaanHukumDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_hukum.post(`${this.BASE_PATH}/aspek/${cleanAspekId}/pertanyaan`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async updatePertanyaan(id: number | string, data: UpdateKpmrPertanyaanHukumDto): Promise<FrontendPertanyaanResponse> {
    try {
      const cleanId = this.cleanId(id);
      ...
      console.log('[KPMR Hukum Service] Update pertanyaan payload:', JSON.stringify(payload, null, 2));

      const response = await api_hukum.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}`, payload);
      return response.data;
    } catch (error: any) {
  ...
  async deletePertanyaan(id: number | string): Promise<void> {
    try {
      const cleanId = this.cleanId(id);
      await api_hukum.delete(`${this.BASE_PATH}/pertanyaan/${cleanId}`);
    } catch (error: any) {
  ...
  async createKpmr(data: CreateKpmrHukumOjkDto): Promise<FrontendKpmrResponse> {
    try {
      console.log(`🆕 [KPMR Hukum Service] Creating KPMR:`, data);
      ...
      console.log('📦 [KPMR Hukum Service] Sending payload:', payload);

      const response = await api_hukum.post(this.BASE_PATH, payload);

      if (!response || !response.data) {
        throw new Error('Response dari server tidak valid');
      }

      console.log('✅ [KPMR Hukum Service] Response:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('❌ [KPMR Hukum Service] Create error:', {
  ...
  async updateSkor(id: number | string, data: UpdateSkorDto): Promise<FrontendPertanyaanResponse> {
    try {
      ...
      const response = await api_hukum.patch(`${this.BASE_PATH}/pertanyaan/${cleanId}/skor`, data);
      return response.data;
    } catch (error: any) {
...
export default kpmrHukumApiService;
