...
import InvestasiInherent from './investasi-inherent';
import InvestasiKpmrPage from './investasi-kpmr';
...
export default function InvestasiOJK() {
  ...
  // ========== HOOK KPMR ==========
  const { kpmr, loading: kpmrHookLoading, error: kpmrHookError, loadKpmrByYearQuarter, refreshKpmrData, createKpmr, addAspek } = useKpmrInvestasi();
  ...
  const createNewKpmr = useCallback(async () => {
    ...
    try {
      console.log(`🆕 [Investasi] Creating new KPMR for year ${year}`);
      ...
      if (data) {
        console.log(`✅ [Investasi] KPMR created: ID ${data.id}`);
      ...
      console.error('❌ [Investasi] Error creating KPMR:', error);
      return null;
    } finally {
  ...
  useEffect(() => {
    ...
    if (loadedYear === year && kpmrDataLoaded) {
      console.log(`✅ [Investasi] Data untuk tahun ${year} sudah dimuat, skip reload`);
      return;
    }
    ...
    const loadKpmrData = async () => {
      ...
      try {
        const targetQuarter = 1;
        console.log(`📡 [Investasi] Loading KPMR for year ${year} Q${targetQuarter}`);

        const data = await loadKpmrByYearQuarter(year, targetQuarter);

        if (data) {
          console.log(`✅ [Investasi] KPMR loaded: ID ${data.id}`);
          setKpmrDataLoaded(true);
          setKpmrLoadError(null);
          setLoadedYear(year);
          setKpmrId(data.id);
          ...
          console.log(`ℹ️ [Investasi] KPMR not found for year ${year}`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
        ...
        if (error?.response?.status === 404) {
          console.log(`ℹ️ [Investasi] KPMR not found for year ${year} (404)`);
          setKpmrRows([]);
          setKpmrId(null);
          setKpmrDataLoaded(true);
          setLoadedYear(year);
          ...
          console.error('❌ [Investasi] KPMR load error:', error);
  ...
  useEffect(() => {
    ...
    if (kpmrId !== kpmr.id) {
      console.log(`🆔 [Investasi] kpmrId berubah: ${kpmrId} -> ${kpmr.id}`);
      setKpmrId(kpmr.id);
    }
  ...
  const handleKpmrRefresh = useCallback(async () => {
    if (!kpmrId) {
      console.log('⚠️ [Investasi] Cannot refresh: kpmrId is null');
      return [];
    }

    console.log('🔄 [Investasi] handleKpmrRefresh dipanggil');
    try {
      ...
      console.error('❌ [Investasi] Error refreshing:', error);
      return [];
    }
  ...
  const kpmrPage = useMemo(() => {
    console.log(`🏗️ [Investasi] Membangun KPMR Page untuk tahun ${year} dengan ID: ${kpmrId || 'null'}`);

    return <InvestasiKpmrPage key={`kpmr-page-${year}`} rows={kpmrRows} setRows={setKpmrRows} search={search} kpmrId={kpmrId} onRefreshData={handleKpmrRefresh} onCreateKpmr={createNewKpmr} />;
  ...
  useEffect(() => {
    ...
    if (currentRowsJson !== newRowsJson) {
      console.log(`📝 [Investasi] Update rows: ${kpmrRows.length} -> ${formattedRows.length}`);
      setKpmrRows(formattedRows);
    }
  }, [kpmr]);
  ...
  // ========== LOAD INHERENT DATA ==========
  useEffect(() => {
    console.log('🚀 [Investasi] LOADING INHERENT DATA', { year, quarter: activeQuarter });
    ...
    console.log('✅ [Investasi] Inherent data loaded:', normalizedInh.length, 'rows');
  }, [year, activeQuarter]);
  ...
  if (!year) {
    return (
      <div className="w-full space-y-4">
        <Header title="Risk Profile – Investasi" />
        <RiskTabs
          value={activeTab}
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="w-full">
        {activeTab === 'inherent' && <InvestasiInherent rows={inherentRows} setRows={setInherentRows} search={search} active onSaveData={saveInherentData} />}
